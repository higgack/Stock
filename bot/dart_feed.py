...
def _budget_add(n: int = 1) -> int:
    ...
    try:
        ...
        pass
    return total
# 사이클(1분)당 신규 enrich 시도 상한 — 16/분(=960/시간). 장 마감(15:30 KST)
# 후 16~18시 KST 공시 폭주를 더 빨리 흡수(8→16, 사용자 2026-06-15 '미파싱
# 비율 높음' = 폭주 처리 lag, 진짜 파서 공백 아님 — 지난날치 미파싱 0 확인).
# 최신순 소진(오늘치 우선), 나머지는 다음 사이클 점진 백필. ⚠️ 일일 예산
# 소비는 공시 '총량'에 묶임(정상 ~250건/일×2콜=500, 하드캡 15k 훨씬 아래) —
# per-cycle 상향은 같은 총량을 더 적은 사이클에 처리할 뿐 일일 예산을 안
# 늘린다. _BUDGET_HARD 가 대량 backfill 시 총량 폭주를 여전히 차단.
_ENRICH_MAX_PER_CYCLE = 16

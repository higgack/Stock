...
_VKOSPI_TICKER_CACHE = "vkospi_ticker.json"
_VKOSPI_TICKER_CACHE_TTL_HOURS = 24 * 7   # 지수 카탈로그는 사실상 불변


def _find_vkospi_ticker() -> Optional[str]:
    """pykrx 지수 카탈로그에서 '변동성' 이 이름에 들어간 티커를 **이름으로
    동적 탐색**(get_kr_beta_60m 의 "1028" 처럼 숫자 코드 하드코딩 안 함 —
    라이브 검증 불가한 코드를 추측해 박아넣는 건 이 프로젝트의 '추측보고
    금지' 원칙 위반, 2026-08-06 사용자 요청으로 신규 추가). 7일 캐시(카탈로그
    거의 안 바뀜). 매치 없음/조회 실패 시 None(호출부 graceful)."""
    cache_file = _CACHE_DIR / _VKOSPI_TICKER_CACHE
    if cache_file.exists():
        try:
            age_h = (time.time() - cache_file.stat().st_mtime) / 3600
            if age_h < _VKOSPI_TICKER_CACHE_TTL_HOURS:
                cached = json.loads(cache_file.read_text())
    ...
    for market in ("KOSPI", "KOSDAQ", "KRX", "테마"):
        try:
            ...
            log.warning("pykrx: 지수 티커 목록 조회 실패(%s): %s", market, exc)
            continue
        for tk in tickers or []:
            try:
                name = stock.get_index_ticker_name(tk)
            except Exception:
                continue
            if name and "변동성" in name:
                try:
                    _CACHE_DIR.mkdir(parents=True, exist_ok=True)
                    cache_file.write_text(
                        json.dumps({"ticker": tk, "name": name}, ensure_ascii=False))
                except Exception:
                    pass
                log.info("pykrx: 변동성지수 티커 탐색 — %s (%s)", tk, name)
                return tk
    log.warning("pykrx: 변동성지수(VKOSPI) 티커를 카탈로그에서 못 찾음 — "
               "이름 매칭 실패 또는 KRX 스키마 변경 의심")
    return None


def get_kr_volatility_index_series(days_back: int = 200) -> Optional[list[tuple[str, float]]]:
    """VKOSPI(코스피 200 변동성지수) 일별 종가 시계열 [(date, close)] — 오름차순.
    코스피 신용/예탁금(KOFIA) 과 무관한 별개 소스(KRX, pykrx). 미인증
    (krx_login_ready)/티커 미탐색/조회 실패 시 None(호출부 graceful —
    대시보드 카드가 자연히 생략됨). 6h 캐시(당일 값은 장중 갱신 여지)."""
    if not krx_login_ready():
        return None
    ...
    if not ticker:
        return None
    today_str = date.today().isoformat()
    cache_file = _CACHE_DIR / f"vkospi_series_{today_str}.json"
    if cache_file.exists():
        try:
            age_h = (time.time() - cache_file.stat().st_mtime) / 3600
            if age_h < 6:
    ...
    try:
        ...
        log.warning("pykrx: VKOSPI 시계열 조회 실패(ticker=%s): %s", ticker, exc)
        return None
    ...
    for idx, row in df.iterrows():
        try:
            # 예탁금·신용잔고 차트(YYYY.MM.DD, _fsc_date 규약)와 표기 통일
            # — pykrx 인덱스가 Timestamp/원시 YYYYMMDD 문자열 어느 쪽이어도
            # 같은 포맷으로 정규화.
            if hasattr(idx, "strftime"):
                d = idx.strftime("%Y.%m.%d")
            else:
                s = str(idx)
                d = f"{s[:4]}.{s[4:6]}.{s[6:8]}" if len(s) >= 8 else s
            out.append((d, float(row["종가"])))
        except Exception:
            continue
    if not out:
        return None
    try:
        _CACHE_DIR.mkdir(parents=True, exist_ok=True)
        cache_file.write_text(json.dumps({"series": out}, ensure_ascii=False))
    except Exception:
        pass
    return out

...
def _compute_tw_highlow() -> None:
    ...
    try:
        ...
        out = _compute_highlow_from(
            uni, names, _CACHE,
            "TWSE 전종목 산출(yfinance · 52주 고저 1% 근접)", "TW")
...
def fetch_tw_highlow() -> dict:
    """TW 52주 신고가/신저가 — **동기 계산 안 함**. 신선(30분) 즉시 / 스테일
    + 백그라운드 킥 / 캐시 부재 시 building. 실패 5분 백오프·진행중 30분 dedup
    (US movers 미러). {high,low,ts,source,building,status}."""
    from bot.finviz_client import _cached

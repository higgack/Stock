...
<b>【대시보드】</b> 🌍 Main 단일 entry — 그 외(분석아카이브·자산·Screener·레딧·Daily Byte·블로그·밸류체인·🏭PPI·🛒CPI·💧유동성·🚦시장타이밍(분산일·FTD·섹터breadth·변동성 VKOSPI/VIX/MOVE 전일~1년)·🧭Breadth전략(4구간: 역추세/회복/비추세/추세 — 섹터 MA120 상회비율 기준, 월말 확정)·📅경제캘린더(CPI·Core CPI·PPI·고용·AHE·실업률·실업수당(신규/연속)·소매·ECI·GDP·PCE·Core PCE·FOMC·JOLTS·소비자심리·산업생산, 분류: 5일변동성/정책민감/침체조기경보, 방향성: 최근발표대비·1M·3M·6M·1Y)·🏆Market cap·부동산(청약 포함)·수출입)는 Main nav, 워치·도메인목록은 Screener nav
 ...
 📊 <b>분기실적</b> 탭 (종목 상세 · 전 시장) — 최근 5분기 매출·영업이익·순이익 추이(막대 값 라벨) + 단일분기 YoY/QoQ(이익률은 %p) + 수주잔고·재고자산 막대(KR·공시하는 종목만, 파서 미스는 격주 금 16시 자동 리포트) + TTM·Forward PER·PSR 스코어카드(시총 기준 라이브, 시간 단위 갱신) + 주요 제품 및 서비스·생산능력/생산실적/가동률 원본 표(KR·공시하는 종목만, 최신 보고서 기준 자동 롤링). 소스: KR=DART 정기보고서 · 그 외=yfinance 분기 손익. 성장동력·리스크 카드(각 최대 6개)는 DART 공시 원문 근거라 <b>한국 종목만</b>, 탭 열면 자동 생성(분기당 1회 과금, 이후 캐시)

...
<b>【9. 대시보드】</b> 3개 entry — 나머지(Screener·레딧·Daily Byte·📝블로그(글 요약+원문 아카이브)·부동산·청약·수출입)는 🌍Main nav, 워치·도메인목록은 Screener nav 에서
 🌍 <b>Main</b> — 글로벌스냅샷·Macro(금리·물가·환율·센티먼트) · 다가오는실적(한국yfinance+미국Finnhub) · 리서치액션(7일치·한국네이버목표가/원문+미국TP 종목당3) · 관심종목(시총·PER(적자표시)·EPS FY라벨·등락·정렬/필터/순서) · 📋DART공시(18종 구조화 카드·🔥중요/⚠️미파싱 색상·조회공시/공개매수/부도·생산중단 등 리스크 수집·지분 노이즈컷) · 업종등락(KR테마·상한가 + 미국TOP10→업종별시세·신고저 전량) · 종목검색(헤더→탭→차트 즉시) · 1분 갱신

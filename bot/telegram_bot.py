...
<b>【대시보드】</b> 위 <b>Main</b> 이 단일 entry — 나머지는 전부 Main nav 에서 이동
 🌍 <b>Main</b> — 글로벌스냅샷·매크로(금리·물가·환율) · 다가오는 실적(한·미·일·대·중·홍) · 리서치 액션(한국 기업/산업/전략 + 미국 TP) · 관심종목(한글명·시총·현재/예상 PER·등락·★중요표시+중요만 필터·정렬/필터 · 5분 주기 갱신) · 📋DART 공시(40+종 구조화 카드 · 🔥중요/⚠️미파싱 색상 + 카테고리 필터 · CSV) · 업종등락 + 🏯ASIA(신고저·급등락·거래량상위·KRX장후·장전/시간외·NXT) · 종목검색·스크롤복원 · 새 데이터 하단알림(1분 체크·반영은 사용자 선택)
...
━━━━━━━━━
<b>【채널 알림】</b>
🚀 배포 · ⚠️ hang · ❌ 실패 · 📋 관심종목 DART공시(/dart_alert) · ⏰ 카드 알람 · 📰 Daily Byte(한 평일19:00·미 08:00·주간 한·미 일22:00) · 🎟️ 청약(평일10·14시) · 🏠 부동산(금09:00·1일) · 📝 블로그(30분) · 📣 DAJU 실적예정(실시간) · 📨 레딧(1분) · 🔍 대시보드 감사(매일08:00·이상 있을 때만)
...
_SITES_TEXT = """🔗 <b>참고 사이트</b>
 ...
 • <a href="https://stocks.allreview.kr/">싹다분석</a>
 ...
 • <a href="https://siliconanalysts.com/">Siliconanalysts</a>
 • <a href="https://quantflow.io.kr/dashboard">Quantflow</a>
 • <a href="https://mosobamboocompany.github.io/">모소밤부</a>"""
...
def _blog_list_text() -> str:
    ...
    import html as _h
    try:
        from bot.blog_watch import (
            _BLOGS, category_label as _cat_label, title_label as _ttl_label)
    except Exception:
        _BLOGS = ()
    ...
    for b in _BLOGS:
        ...
        # 필터는 **두 축**(카테고리·제목)이고 화면은 둘 다 말해야 한다 —
        # 한쪽만 적으면 "왜 안 올라오나"에 답을 못 한다(#43·#34).
        suffix = _h.escape(_cat_label(b.get("categories")) + _ttl_label(b))
        ...
        if not nav_off:                                  # 네이버 경량 보드
            ...
            if kr_ext:                                   # KR 장전후(NXT) — KST 08-20
                jobs += [
                    ...
                    # KRX 애프터마켓(16:00–20:00) — 같은 연장창 게이트.
                    ("krafter", lambda: _warm_render(

...
def _strip_meta_commentary(text: str) -> tuple[str, int]:
    ...
    # ⚠️ quantifier 중첩 금지 — '(?:[가-힣A-Za-z]+\s*,?\s*)*정량...' 형태는
    # 긴 한국어 본문에서 catastrophic backtracking 으로 CPU 무한 hang 발생
    # (2026-06-01 Hospitality screener run CPU 73% 정지 진범). 인접 토큰만
    # 고정 길이로 매칭하고 \s* → \s? (0~1 공백) 로 제한.
    patterns = (
        r"정량\s?데이터\s?추가\s?확인\s?필요",
        r"데이터\s?깊이\s?부족",
        r"데이터\s?미수집",

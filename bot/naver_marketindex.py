...
_IDXHIST_BASE = "https://stock.naver.com/api/securityService/index"


def fetch_naver_index_history(reuters_code: str, count: int = 30) -> list[float]:
    """securityService/index/{reutersCode}/price → 지수 일별 close 시계열(오래된→최신).
    reutersCode = .INX(S&P)·.IXIC(나스닥)·.VIX 등 (사용자 VM Network 탭 확인 2026-06-14).
    원자재와 동일 securityService 패밀리 → closePrice schema·pageSize 캡(ps30) 동일.
    응답 bare list 또는 {datas:[]} 양형 수용. 1h 캐시·naver_paused·graceful []."""
    if not reuters_code:
        return []
    from bot.finviz_client import _cache_write, _cached, naver_paused
    ck = f"naver_idxhist_{reuters_code}_{count}.json"
    c = _cached(ck, ttl=3600)
    if isinstance(c, list) and c:
        return c
    if naver_paused():
        ...
        return s if isinstance(s, list) else []
    import requests
    _PS = 30                               # 네이버 캡 — 30씩 페이지(원자재와 동일)
    rows: list = []
    for _pg in range(1, (count + _PS - 1) // _PS + 1):
        try:
            r = requests.get(f"{_IDXHIST_BASE}/{reuters_code}/price",
                             headers=_HDRS, params={"page": _pg, "pageSize": _PS},
                             timeout=10)
            raw = r.json() if r.status_code == 200 else []
        except Exception as exc:
            log.warning("naver idxhist %s p%s: %s", reuters_code, _pg, exc)
            break

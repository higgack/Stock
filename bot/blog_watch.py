...
_BLOGS = (
    ...
    # 사용자 2026-08-27 (blog.naver.com/hwasikyuljeon, 표시명 사용자 확정).
    # 첫 run 은 per-blog init 으로 기존 글 seen 처리만 — 백필 없이 새 글부터.
    ...
    {"id": "ggbbvv", "title": "간동", "categories": None,
     ...
     # ⚠️ 1차는 `title_any=("결산",)` 이었는데 **VM 실측이 그걸 반증했다** —
     # 50건 중 32건이 통과했고 거기엔 `운동 & 독서 결산 (2/48)` 처럼 주식과
     # 무관한 결산이 섞였다(주간 결산은 제외어가 잡았지만 이건 못 잡는다).
     # 실측 제목으로 다시 고른 판별어가 **"투자 결산"** 이고, 같은 10건에
     # 10/10 정확하다(`26년 8월/7월 투자 결산` 만 통과).
     #   ✅ 26년 8월 투자 결산 · 26년 7월 투자 결산
     #   ⛔ 주간 결산 (9.6-9.12) · 운동 & 독서 결산 (2/48) · 260708 누구에게나…
     # `title_none` 은 남긴다 — "8월 2주차 투자 결산" 같은 조합이 오면
     # 필수어만으론 못 막는다(관측된 적 없는 백스톱이고, 오면 `--check` 가
     # 보여 준다). 카테고리 축으로는 못 가른다: 월별 결산은 `2026년 투자`
     # 인데 그 카테고리엔 결산이 아닌 글도 함께 있다.
     "title_any": ("투자 결산",), "title_none": ("주차", "주간", "주별")},
    # 사용자 2026-09-13 추가(표시명 사용자 확정). 채널 제목은 `--check` 실측.
    # ⚠️ 여기서 박아도 되는 이유 — blogId 가 **사용자가 준 URL 그대로**라
    # 정체성이 출처로 확정된다(`hempty` 때는 내가 두 후보 중 골랐고 그래서
    # 확인이 필요했다, #362). 박는 값은 정체성 주장이 아니라 **드리프트
    # 기준선**이다: 이 blogId 가 다른 곳을 가리키기 시작하면 기계가 잡는다.
    # 필터 없음 — 사용자가 요청하지 않았다. 실측 50건: 생각들 35(시장정리) ·
    # 투자복기·마인드 1 · 운동 6 · 스타일 4 · 열심히살기 4 = 주식 무관 14건.
    # 보고만 하고 붙이지 않는다(사용자 결정 사항).
    {"id": "chcmg2022", "title": "너쟁이", "categories": None,
     "channel": "성실히 나아가는 작은 발걸음"},
...
def _check_banner() -> str:
    """`--check` 시작 줄 — **어느 코드가 돌았는지** 출력이 스스로 말한다(#21).

    ⚠️ 왜 필요한가(2026-09-13 실측): 판별어를 `결산` → `투자 결산` 으로 좁혀
    배포했는데 사용자가 돌린 `--check ggbbvv` 출력이 **옛 라벨·옛 통과 수**
    였다. VM auto-update 가 아직 안 돌았던 것인데, **출력만 봐선 나도
    사용자도 그걸 알 수 없어** 고친 것이 안 먹은 것처럼 보였다(#11 배포완료
    ...
    return f"# blog_watch --check · 코드 지문 {sig} · 등록 {len(_BLOGS)}개"
...
def main(argv: list[str] | None = None) -> int:
    ...
    if argv and argv[0] == "--check":
        # ⚠️ 배너는 **두 갈래 모두**에 — 무인자 표도 `_BLOGS` 에서 나오므로
        # 낡은 체크아웃이면 등록 목록부터 옛것이다. 한 갈래에만 달면 그
        # 갈래의 증상만 설명한다(#359 그 사각을 적은 바로 그 커밋에서 또
        # 낼 뻔했다 · #38 한 곳을 고치면 형제를 즉시 grep).
        print(_check_banner())
        if len(argv) < 2:

...
class Site:
    ...
    url: str
    # ⚠️ **nav 전용** 항목. 기본은 두 표면 모두(그게 지금까지의 전부였다).
    # 사용자가 "대시보드에만" 이라고 명시하면 False — 그때 `/sites` 파티
    # 회귀는 이 항목을 요구하지 않는다(사용자 2026-09-16 네이버증권).
    # 축을 안 두고 회귀만 느슨하게 하면 **어느 항목이 의도적 제외인지**
    # 기계가 모른다 = 다음에 진짜 누락이 생겨도 조용하다(#24·#54).
    in_sites: bool = True
...
SITES: tuple[Site, ...] = (
    ...
    # 사용자 2026-09-16: "네이버증권을 메인대시보드에 연결해줘. 제목은

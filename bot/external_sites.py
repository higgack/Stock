...
CLAUDE.md 는 원래 "외부 third-party 사이트는 `/sites` 만, 메인 nav 추가
금지" 였는데 사용자가 2026-08-21 그 규칙을 **폐기**했다("외부사이트도 앞으로는
메인 nav 에 등재할거야"). 이제 새 외부 사이트는 기본으로 여기와 `/sites`
둘 다 등재한다. 기존 `/sites` 전량(~30)을 소급 이전하진 않는다 — nav 가
감당 못 하고, 사용자가 옮기라고 한 적도 없다(요청 시 개별 이전).

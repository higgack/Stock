...
⚠️ CLAUDE.md 는 원래 "외부 third-party 사이트는 `/sites` 만, 메인 nav 추가
금지" 였다. 사용자가 이 다섯 개는 **자주 본다**며 명시적으로 nav 를 요청해
규칙을 바꿨다(같은 커밋에서 CLAUDE.md 갱신). 규칙이 없어진 게 아니라
**여기 등록된 것만** 예외다 — 아무 사이트나 nav 에 붙이지 않는다.

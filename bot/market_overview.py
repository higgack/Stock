...
# ⚠️ 라벨은 **짧게**. 핵심 지표 카드는 300px 그리드라 라벨이 길면 두 줄로
# 접히고 옆 카드와 행 높이가 어긋난다(사용자 2026-08-18 "(금리) 같은 글자
# 빼서 옆과 높이를 맞춰줘"). 카드 제목이 이미 "금리/경제" 라 중복이었다.
FRED_INDICATORS = [
    ("US 미국채 2년", "DGS2", "%", 90),
    ("US 미국채 10년", "DGS10", "%", 90),
    ("US 미국채 30년", "DGS30", "%", 90),
...
# 2차 리뷰 L6) — 한 카드의 두 절반이 같은 재생성에서 함께 넘어간다. 그러니 이 값을 다시
# 늘리면 둘이 같이 늦어진다(갈라지진 않는다). 일별 집합(`_FRED_DAILY_SIDS`)은 남긴다 — 이
# 값을 다시 늘리는 날 그 집합이 일별 시리즈를 짧게 지키는 유일한 선이다.

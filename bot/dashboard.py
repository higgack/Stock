        ...
        if (j && j.ok && j.chart) {
          ...
          if (ivf) {
            status('ℹ️ ' + intervalLabel(ivf.requested) + ' 데이터가 없어 '
                   ...
                   + ' — 국내 종목은 분봉 미제공, 해외도 최근 구간만 보관됩니다.');
          } else { status(''); }

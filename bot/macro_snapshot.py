...
GLOBAL = [
    ...
    # 솔라나 — 코멘트 의도(2026-06-14 '코인은 비트·이더·솔만')대로 복원. _MACRO_NAVER
    # 에 SOL-USD→업비트 SOL 매핑은 이미 존재했으나 카드 목록에서 누락돼 있던 것
    # (사용자 2026-06-23 '카드 3개 빠짐'). 값=업비트, 스파크=네이버/yf.
    ("sol", "솔라나", "$", "yf", "SOL-USD", 0),
]
    ...
    def _build(defs: list) -> list[dict]:
        ...
        for key, label, unit, src, sid, dec in defs:
            ...
            if src == "yf":
                ...
                # 가격을 공개적으로 노출하지 않아 그 시도는 잘못된 전제였고

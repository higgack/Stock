...
def _ago(gap: timedelta) -> str:
    """경과 시간 한 마디. ⚠️ **내림**이다 — 반올림하면 35시간이 '1일 전'이
    아니라 '1일 전'인데 상한 36시간은 '2일'로 올라가 여유가 넉넉해 보인다
    (독립 리뷰 2026-09-08: 인접 두 칸이 서로 다른 방향으로 반올림했다).
    같은 줄에 나란히 놓이는 값은 **같은 방향으로** 자른다(#33)."""
    h = gap.total_seconds() / 3600
    return f"{int(h // 24)}일 전" if h >= 24 else f"{int(h)}시간 전"


def _cap_label(cap: float) -> str:
    """상한 표기 — 경과와 **같은 방향(내림)** 으로 자른다.

    이 두 값은 `(4일 전 · 상한 8일)` 처럼 한 줄에 나란히 놓여 'N/M 만큼 지났다'
    로 읽히므로, 서로 다른 방향으로 반올림하면 비교 자체가 거짓이 된다
    (독립 리뷰 2026-09-08: 36시간 상한이 '2일'로 올라가 35시간 경과가 여유
    있어 보였다 — 실제로는 1시간 남았다, #33 나란히 놓인 칸은 산수가 맞아야).
    내림은 상한을 실제보다 **빡빡하게** 보이게 하므로 안전한 방향이다.
    """
    return f"{int(cap // 24)}일" if cap >= 24 else f"{int(cap)}시간"

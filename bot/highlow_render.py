...
def stock_panel(title: str, items: list, tid: str, market: str,
    ...
    ⚠️ 호출부가 **재서 켠다** — 원천이 안 주는 보드는 아예 False 로 두 칸을
    없앤다(사용자 2026-09-17 "못가져오는거면 아예 빼주고"). 매번 같은 각주로
    빈 칸을 설명하면 화면만 시끄럽다(#25·#260). 켜 놓고 일부 행만 비는 경우는
    종전대로 '—' 이고 사유는 호출부 각주가 말한다(#43·#181).

...
def dry_run() -> int:
    ...
    print("   ⚠️ 브리프 아카이브·인포그래픽은 **안 남깁니다** — 진단이 자기가 "
          "읽을 신호를 오염시키면 안 된다(#264)")
    print("   ⚠️ 단 **실제 과금된 비용은 사용량 로그에 기록**됩니다(대시보드 "
          "비용 카드에 반영) — 쓴 돈을 장부에서 빼면 그게 더 큰 거짓말이다")
    try:
        from dotenv import load_dotenv
        from pathlib import Path
        load_dotenv(Path.home() / "stock" / ".env")
    ...
    print(f"   | {head}")
    print("④ 푸시·아카이브 둘 다 안 했다 — 남긴 것은 **실제 과금된 비용 "
          "기록**뿐이다. 정기 실행(daily-byte.timer)이 같은 경로로 보내고 "
          "기록한다")
    return 0

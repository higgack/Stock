...
# ⚠️ 옛 `_L4_DOMAIN_KO`(272줄) 를 여기서 제거했다(2026-08-18). 그 표는 L4 만
# 덮었고 값도 `Railroads(Transportation & Logistics)` 처럼 괄호 안이 부모
# **영문**이라 형식이 네 갈래로 갈려 있었다. 표시 라벨은 전 계층 하나의
# 레지스트리 `bot/screener_domain_labels` 로 통일 — 사용자 2026-08-18
# "L1~L4 를 영어 (한글) 형식으로 통일".
...
def list_domains() -> list[dict]:
    ...
    for slug in sorted(_THEMES):
        theme = _THEMES[slug]
        # 표시 라벨 = `영문 (한글)` 단일 규약(전 계층). 원본 THEME["domain"]
        # 은 프롬프트·아카이브가 쓰므로 건드리지 않는다.
        from bot.screener_domain_labels import display_domain
        domain = display_domain(slug, theme.get("domain", slug))
        out.append({
            "slug": slug,
            "domain": domain,

...
def fetch_intl_sector_movers_naver(market: str, top_n: int = 10) -> dict:
    ...
    업종 상위 종목(시총순 20) 시총가중 등락 → 랭킹. ETF 업종 제외. **장중 30초 디스크
    캐시**(사용자 2026-06-15 '네이버 다 실시간'·session-aware 라 장 밖 0). graceful·
    ...
    if isinstance(stale, dict) and (stale.get("up") or stale.get("down")):
        try:
            ...
            _mt = 0.0
        if _session_fresh(market, _mt, 30):   # 30초(사용자 2026-06-15 '업종 30초')
            return stale
    import requests
    try:

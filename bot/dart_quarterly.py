...
def _attach_fcf(entries: list[dict] | None) -> int:
    ...
    for e in entries or []:
        ...
        # 것이다(#50).
        # ⚠️ 옛 주석은 여기에 "이 정의는 yfinance `Capital Expenditure`
        # 와도 같아 시장 간 정의가 하나로 맞는다" 고 이어 적었는데 그건
        # 재지 않은 단언이었다(#165). 2026-09-21 181710.KS 8기간 실측에서
        # `|yf CAPEX| = 유형 + 무형` — yfinance 는 무형을 묶어 준다. 산식은
        # 그대로(FnGuide 기준) 두고 **정의가 다르다는 사실**을 화면이
        # 말한다(밸류에이션↔재무제표 탭 상호 각주, #102·#186·#220).

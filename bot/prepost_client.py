...
def stored_note(data: dict | None, window: str = "") -> str:
    ...
    US·KR 두 화면이 같은 문구를 쓰게 한 곳에서 만든다(화면마다 적으면 한쪽만
    고쳐져 갈린다, #38·#147). 판정은 payload 가 실어 준 것(`stale`·`stale_min`·
    `in_window`·`status`)을 그대로 따르고 여기서 다시 재지 않는다(#136·#35).
    나이 라벨은 형제 위젯(TW 업종 #306 · 네이버 업종 #335)과 같은
    `naver_diag.stale_label` 규약.

    ⚠️ **평상시엔 아무 줄도 안 낸다**(사용자 2026-09-20 "이 코멘트는 대시보드에
    없어도 될것 같아. 당연한거니까"). 창 밖이라 저장분인 것은 부제가 이미
    `💾 저장분(28시간 전) · 실시간 아님 · <창> · 2026-09-18 19:54 기준` 으로 다
    말하므로, 같은 사실을 본문에 한 번 더 적으면 소음이다(#25 늘 뜨는 배지는
    아무것도 안 재는 것과 같다 · #45 두 줄이 같은 모집단을 말한다).
    ⚠️ 지우지 않고 **좁힌다** — 마지막 집계 시도가 **실패**했다는 사실은 부제의
    어느 칸도 말하지 않고, 그때 "다음 창에 갱신된다"는 지킬 수 없는 약속이
    된다(#380·#43). 그리고 US 는 실패 배너가 `building` 분기 **안**이라 표가
    ...
    if scan_state(d.get("status")) != "failed":
        return ""                      # 평상시 — 부제가 이미 다 말한다
    from bot.naver_diag import stale_label
    m = d.get("stale_min")
    ago = stale_label(m * 60 if isinstance(m, int) else None)
    ts = str(d.get("ts") or "")
    when = " · ".join(x for x in ((f"{ts} 집계" if ts else ""), ago) if x)
    # 여기까지 온 것은 **실패 갈래뿐**이다(위 조기반환). '왜 저장분인가'(창
    # 밖이라)는 실패 사실이 이미 설명하므로 적지 않는다 — 한 줄이 같은 말을
    # 두 번 하면 그게 소음이다(사용자 2026-09-20 "당연한거니까").
    # ⚠️ 언제 다시 시도하는지는 **창 판정에 달렸다** — 창 안인데 '다음 창'이라
    #    적으면 거짓이다(#55). 창 판정을 못 받았으면 시점을 단정하지 않는다(#165).

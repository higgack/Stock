...
def create_fundamentals_analyst(llm):
    def fundamentals_analyst_node(state):
        ...
        system_message = (
            ...
            " exact mistake). ✅ RIGHT: 'LNVGY' (ADR) or '0992.HK' (Hong"
            " Kong listing). Include exchange suffixes when the peer is"
            " non-US: '.HK', '.T' (Tokyo), '.KS' (KRX), '.L' (London), etc."
            " If you are not certain a ticker symbol is real, write only"
            " the company name in the ticker column and LEAVE THE MULTIPLES"
            " COLUMNS EMPTY rather than guess multiples for a fake symbol.\n"

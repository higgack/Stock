...
def get_macro_context(
    ...
    if suspect:
        ...
        out += (
            ...
            "올바른 처리:\n"
            "  ✅ 의심 시리즈는 리포트 본문에서 **완전 omit** — 보류 사유"
            " / 검증 상태 / 가드 존재 자체를 언급하지 말 것. '거시 frame"
            " 인용 보류' / '검증 미완료' / '데이터 이상' 같은 시스템"
            " 메타-텍스트가 리포트에 노출되면 RULE 위반.\n"
    ...
    # ⛔ SSoT 글자 단위 copy 강제 (IBM 2026-06-02 surfaced): 같은 리포트
    # 안에서 뉴스 에이전트가 '10Y 4.51%', 매크로/시장 에이전트가 '4.45%'
    # 처럼 같은 지표를 다르게 서술하는 data-drift. 이 스냅샷은 (market,
    # date) 단일 캐시라 모든 에이전트가 동일 값을 받지만, 본문 재서술 시
    # paraphrase / 반올림 / 다른 시점 추정으로 어긋난다. RSI/MACD SSoT 와
    # 동일하게 글자 단위 copy 를 강제해 cross-agent 일관성 보장.
    out += (
        "\n\n⛔ SINGLE SOURCE OF TRUTH — 위 거시 수치 글자 단위 copy 의무:\n"
        "  • 위 스냅샷의 각 지표 값(예: 美 10Y 4.45%, VIX 16.09, DXY"
        " 99.29, WTI $94.27)은 이 분석 전체(시장/감정/뉴스/펀더멘털/결정)"
        " 의 유일한 canonical 값.\n"
        "  • 본문·요약·결론에서 거시 지표를 인용할 때 위 값을 글자 단위로"
        " 정확히 copy. 반올림 / 자릿수 축약 / 다른 시점 추정값 인용 금지.\n"
        "  • 같은 리포트 안에서 뉴스 섹션 '10Y 4.51%' vs 매크로 섹션 '4.45%'"
        " 같은 불일치 = 위 snapshot 무시하고 paraphrase 한 위반"
        " (IBM 2026-06-02 사례). 모든 에이전트가 위 동일 문자열 copy 의무."
    )
    return out

...
def _build_instrument_context_impl(ticker: str, analyst_id: str | None = None,
    ...
    # Universal 글쓰기 스타일 (전 시장 — US/KR/JP/TW/CN_A/HK 공통, 2026-06-27).
    # 분석가 산출물은 LLM 한글 텍스트라 'AI 번역체'(영어 직역투·기계적 병렬·
    # 공허한 메타문구·AI 상투어)가 새기 쉬움 (im-not-ai 검토에서 도출). 별도
    # 후처리 모듈/비용 없이 프롬프트 한 블록으로 자연스러움을 강제. 시장 게이트
    # 없음 — 모든 분석에 동일 적용. 단, 수치/사실 보존이 스타일보다 절대 우선.
    base += (
        "\n\n=== 글쓰기 스타일 (전 시장 공통) ===\n"
        "한국 금융 독자가 자연스럽게 읽는 한국어로 작성. AI 산출물 특유의"
        " 번역체·기계적 표현을 피한다:\n"
        " • 영어 직역투 금지: '~을 제공한다 / ~에 위치한다 / 그것은 ~이다 /"
        " ~라고 말할 수 있다' 류 어색한 직역 → 한국어 자연 어순으로.\n"
        " • 기계적 병렬·과도한 불릿 나열 대신 핵심을 문장으로 압축.\n"
        " • 공허한 메타문구 금지: '결론적으로 / 요약하자면 / 주목할 만하다 /"
        " ~라는 점이 중요하다 / ~할 필요가 있다' 같은 빈 연결어.\n"
        " • AI 상투어·영어단어 직역 금지: 'leverage(활용)·utilize·robust·"
        " ~의 영역에서·~로서의 역할' 등.\n"
        "⚠️ 사실 보존 절대 우선: 모든 수치·티커·고유명사(회사명/인명)·통화·"
        " 단위·날짜·인용은 그대로 유지한다. 스타일 개선이 수치나 사실을"
        " 바꾸면 안 되며, 위 모든 단위/통화/종목명 표기 지시가 이 스타일"
        " 규칙보다 우선한다."
    )

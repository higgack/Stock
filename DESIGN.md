...
| `--accent-on` | `#fff` | `#0b1220` | **`--accent` 배경 위 글씨** — 활성 칩·버튼. 리터럴 `#fff` 금지(다크서 3.00:1) |
...
- 다크/라이트 둘 다 **WCAG AA(4.5:1) 이상** — 주장이 아니라 회귀가 잰다
  ...
  칩 배경 위 글씨는 **별도 축**이다(`--X-on` × `--X`,
  `test_accent_on_pairs_meet_wcag_aa`). 한 토큰이 '어두운 배경 위 글자'(밝아야
  함)와 '그 위에 놓이는 글자'(어두워야 함)를 겸할 수 없어서다 — 실제로 접근성을
  위해 다크 `--accent` 를 밝히자 그 위의 리터럴 흰 글씨가 3.65 → 3.00 으로
  **더 나빠졌다**(2026-09-12 독립 리뷰 실측). 그래서 `--accent-on` 을 뒀다.
  ⚠️ 가드가 **못 보는 축** — 범위를 숫자로 밝힌다(#274·#202):
  · **토큰이 나르는 글자는 전수 AA**(480쌍 · 미달 0 · 최저 4.54, 22개 모듈).
  · **리터럴이 나르는 글자는 아직 아니다.** 팔레트 모듈의 `color:#hex` 선언
    238건 중 상당수가 라이트 표면에서 AA 미달이다(2026-09-12 실측). 문맥
    (어느 배경 위인가·어느 테마인가)을 모르는 기계 검사는 오탐 투성이라
    (`#fff` 는 칩 위 글씨, 다크 리터럴은 다크 배경 위) **가드를 두지 않았다** —
    이번 라운드에서는 등락 숫자(`--up`/`--down` 14개 모듈)와 `--fg-soft`
    미정의 폴백만 토큰으로 옮겼다.
  · 비텍스트 대비(WCAG 1.4.11 — `--border` 는 `--card` 대비 1.22:1 이지만
    장식 구분선이라 **재지 않은 것을 단정하지 않는다**, #165).
  · `--X-on` 규약을 안 쓰는 칩 전경(`--tier-l-fg`↔`--tier-l-bg` 류).
  이미지 로고 = 흰 배경 타일(`background:#fff` — 다크서 투명로고 뭉개짐 방지).

...
| `--muted` | `#696e78` | `#8a8f98` | 보조·라벨·기준시각 |
| `--accent` | `#5a66d1` | `#7c84e8` | 링크·활성 필·강조 |
| `--pos` | `#04805a` | `#10B981` | 상승·긍정(▲) |
| `--neg` | `#dc2626` | `#f04d4d` | 하락·부정(▼) |
| `--neu` | `#8a8f98` | `#6B7280` | 중립 — **선언만, `var(--neu)` 참조 0건**(2026-09-12 실측) |
...
- 모바일 폭(≤640/700px)서 그리드 열 축소(`repeat(2,1fr)` 또는 1열), 패딩·
  ...
  body 가로 스크롤 금지).
- 다크/라이트 둘 다 **WCAG AA(4.5:1) 이상** — 주장이 아니라 회귀가 잰다
  (`bot/css_contrast.py` + `test_every_palette_meets_wcag_aa`). 2026-09-12 이
  가드를 켜자 라이트 `--muted` 3.06 · `--pos` 3.54 · `--accent` 4.42 ·
  trade `--tone-export` 2.04 이 미달이라 전부 교체했다(실수 #355). 새 토큰은
  값을 정하기 전에 그 회귀를 한 번 돌려 볼 것.
  ⚠️ 가드가 **못 보는 축**: 인라인 리터럴 색 · 비텍스트 대비(WCAG 1.4.11 —
  `--border` 는 `--card` 대비 1.22:1 이지만 장식 구분선이라 재지 않았다) ·
  칩 전경×자기 칩 배경. 이미지 로고 = 흰 배경 타일(`background:#fff` —
  다크서 투명로고 뭉개짐 방지).

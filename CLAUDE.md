...
294. **'미설정' 이라고만 적은 경고가 바로 다음 줄의 '로그인 완료' 와 모순됐다**
    ...
    ⚠️ 그리고 **모순을 만드는 메커니즘 자체가 갈래 하나였다**: `env_key` 는
    첫 조회 실패를 `_TRIED` 에 넣고 다시 안 읽으므로, 그 뒤 `.env` 가 읽히게
    되면(cwd 가 바뀌거나 파일이 나중에 생기거나) **경고만 남고 값은 있다**.
    추측하지 말고 **다시 읽어 재서** 이름을 붙인다("지금 다시 읽으면 있다 —
    첫 조회가 실패해 캐시됨"). 이걸 '미설정' 이라고만 적으면 운영자가 이미
    있는 키를 넣으러 간다(#187b 틀린 로그가 헛걸음을 만든다 · #23 의 재발).
    ⚠️ **같은 스캔이 세 곳에 복제돼 있었다** — `env_key`·`env_why` 가 각각
    `.env` 두 경로를 도는 루프를 갖고 있어 한쪽만 고치면 두 자리가 같은
    상태를 다르게 말한다(#38). `_dotenv_lookup` 하나로 합치고 회귀가
    `dotenv_values(p)` **1회**를 못박는다.

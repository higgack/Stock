...
- 캐시가 fix 를 가림: #18 #21b #95 #119 #124 #198 #212 #216 #233 #266 #267 #275 #299 #304 #343 #344
...
- 뮤테이션 통과 시 의심 순서: #68 #91 #155 #312 #313 #323 #325 #345 #347
...
348. **예산을 두면 그 예산 밖은 영원히 빈칸인지 물을 것 — 재료가 안 바뀌는 값은
    ...
    예산은 **미스에만** 쓴다(1주기 40 · 2주기 80 · 3주기 100% · 그 뒤 네트워크 0).
    성공은 30일 **실패는 15분만**(#303·#161) · 캐시 쓰기는 **가장 아래 계층**에
    ...
    ⚠️ 30일 TTL 캐시는 **테스트 오염이 운영을 한 달 앓게** 한다 — conftest 세션
    리다이렉트에 모듈을 넣고 **메모리 사본도 같이** 비울 것(#30·#344 — 격리 전
    실행에서 `저장분 1` 로 실제 관측됐다).
    → REFERENCE §실수 #348

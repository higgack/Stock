...
- 값을 비웠으면 사유도 말할 것: #43 #123 #129 #131 #189 #195 #228 #234 #282 #290 #292
...
- 같은 계산 하는 다른 화면 즉시 grep: #38 #147 #211 #241 #248 #254 #288 #289 #291
...
293. **'대조 0건은 통과가 아니다' 가드를 한 갈래에만 걸어 두면 나머지 갈래가
    그대로 통과한다**(2026-09-07 VM 실측 098070.KQ `① 재계산 ✅ 전 기간
    일치(0건)`): 현금흐름표 8행을 받았는데 **전부 원천이 `Free Cash Flow` 를
    직접 준 행**이라 재계산 대상이 0건이었고, 그런데도 축은 '전 기간 일치'
    라고 찍었다 — 아무것도 안 잰 자리가 통과로 보고된 것이다(#54). 2026-08-22
    에 같은 축에 넣은 가드는 `not yq and not ya`(스냅샷이 통째로 빈 경우)
    **하나만** 막고 있었다. 재료가 0인 이유는 갈래가 여럿이고(못 받음 /
    받았지만 잴 대상이 아님) 처방도 다르다 — 갈래마다 이름을 대야 한다(#82).
    같은 함수의 형제 축 ④·⑤ 는 이미 `❓ + 사유` 로 하고 있었다: **한 축을
    고칠 땐 옆 축이 어떻게 하는지 보고 같은 수준으로 맞출 것**(#291).
    ⚠️ ❓ 는 sweep 이 세지 않으므로(❌ 만 결함, ⚠️ 만 사람 확인) 이 정직함은
    알림 노이즈를 하나도 안 늘린다 — **정직하게 만들면 시끄러워지나**를 먼저
    재고 판단할 것(#25·#260 늘 뜨는 경고는 아무것도 안 재는 것과 같다).
    ⚠️ 회귀엔 **반대 증거**를 같이 둔다 — 실제로 잰 자리가 여전히 ✅ 와
    건수를 말하는지 안 보면 축을 통째로 ❓ 로 바꾸는 변형이 통과한다(#25).
    ⚠️ **첫 fix 의 사유가 틀린 라벨이었다**(배포전 셀프리뷰가 잡음): 루프는
    두 이유로 건너뛰는데(재료가 아예 없음 / 원천이 직접 줌) 사유를 하나로
    뭉뚱그려 "전부 원천이 직접 준 행" 이라고 적었다 — 재료가 통째로 없는
    종목에도 그렇게 찍힌다(#292 틀린 라벨은 라벨이 없는 것보다 나쁘다).
    **사유를 적기 전에 그 사유가 몇 갈래인지부터 셀 것**(#82).

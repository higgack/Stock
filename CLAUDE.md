...
- 캐시가 fix 를 가림: #18 #21b #95 #119 #124 #198 #212 #216 #233 #266 #267 #275 #299 #304 #343 #344 #343
- 값을 비웠으면 사유도 말할 것: #43 #123 #129 #131 #189 #195 #228 #234 #282 #290 #292 #295 #296 #297 #298 #304 #305 #307 #308 #315 #317 #318 #320 #322 #324 #325 #335 #342 #343 #344 #343
- 감사·프로브가 화면과 다른 경로: #35 #56 #80 #105 #114 #169 #275 #305 #313 #321 #322 #323 #335 #345 #346
- 소스 문자열 단언 금지: #19 #67 #89 #93 #117 #120 #150 #230
- 열거형 가드는 새 항목을 못 잡음: #24 #27 #31 #87 #269 #273 #299 #311
- 뮤테이션 통과 시 의심 순서: #68 #91 #155 #312 #313 #323 #325 #345
...
323. **공용 파일의 줄 수를 전용 DB 와 비교하면 판정이 영영 한 갈래에 고인다 —
    ...
    `inbox.jsonl` 은 16개 소스 **공용**인데 `store.db` 는 관세청 전용이라, 줄 수를
    그냥 세면 남의 트래픽이 늘 '안 들어간 줄' 로 잡힌다(#45). 갈래는 ingest 가
    **실제로 쓰는 게이트**(파서)로 가르고 화면은 두 모집단을 나란히 찍을 것(#35·#45);
    파서를 못 부르면 폴백하되 **폴백했다고 밝힌다**(#12·#165).
    ⚠️ **순간 상태로 지속 상태를 말하지 말 것** — `activating` 스냅샷 하나를 보고
    "물려 있다"고 단정했는데 정상 실행 중이었다. `ActiveState` 는 표본이고 지속
    여부는 `journalctl` 의 시작·종료 쌍이 답한다(#86 아는 쪽에 물어라).
    ⚠️ **'아직 처리 안 됨' 을 시각으로 재지 말 것** — inbox `date` 는 중계 시각,
    DB `posted_at` 은 원 게시 시각이라 **시계가 다르다**. 두 시각이 같은 시계인지
    먼저 묻고, 아니면 제품의 **멱등 키**(source_chat_id, source_message_id)로 셀 것.
    식별자가 없는 줄은 **식별 불가**지 미적재가 아니다 — 따로 세어 화면이 밝힌다(#54).
    ⚠️ 갈래를 맞게 뒤집어도 **사유 문구가 거짓이면 처방이 틀린다**(#292).
    ⚠️ '한쪽이 조용하다' 를 말하기 전에 **그 파일에 쓰는 다른 소스들도 조용한지**
    볼 것 — 전부 조용하면 원천이 아니라 공용 경로다. 중계 리스너까지 물을 것(#316).
    ⚠️ 픽스처는 **제품 삽입 경로**로 심고 **되읽어 단언**할 것(손으로 짠
    `INSERT OR IGNORE` 가 NOT NULL 위반을 조용히 삼켰다) + 반대 증거(진짜 지연은
    여전히 잡히는가)를 같이 둘 것(#25·#47·#155). 스텁만 두면 배선을 못 잰다(#20) —
    파일이 생기는지는 **파일로** 볼 것(진단이 빈 DB 를 만들고 있었다, #264).
    ⚠️ 키가 없어 **설계상 안 도는** 수집은 ❌ 가 아니라 ⚠️ 로 사실만 적을 것(#41·#260).
    → REFERENCE §실수 #323
...
343. **값이 없으면 카드가 조용히 사라지고, 고친 것은 캐시가 가리고, 죽은 경로는
    ...
    팔라듐·리서치 20건·5탭·재킥): 배치 캐시 키는 **요청 집합**으로 판정하고
    **저장도 같은 집합**으로(손 bump 는 다섯 번 졌다 #119 · 안 그러면 영구 미스로
    30초마다 download) · 캐시 키를 나누는 축은 **결과를 바꾸는 인자 전부**(감사가
    상세 없이 부르면 그 행이 공용 캐시에 굽힌다, #61 의 반대 방향) · 요청한 컬럼이
    없으면 **첫 컬럼을 주지 말 것**(달러인덱스 값이 팔라듐 카드에 앉는다, #46·#33) ·
    카드를 빼면 **뺐다는 사실과 갈래**를 말할 것(대조군까지 이름을 대야 '배치 전체
    실패'와 '심볼 문제'가 갈린다, #43·#82·#143) · 절단 경고 기준은 '상한에 닿았나'가
    아니라 **'창을 다 못 덮고 멈췄나'** 이고 그 사실은 **캐시에 같이 굽는다**(#342 의
    다음 층) · 중간 쪽의 429·타임아웃을 '원천에 그게 전부'로 분류하면 창이 조용히
    잘린다(#82·#136) · 쪽을 합치면 **정렬 뒤에 자르고** 행당 HTTP 가 곱해지니
    **예산을 같이**(#116) · 실패에 **백오프**를 달되 시각은 **기록**이 말하게(`now−now=0`
    이면 영원히 안 풀린다 #178) 하고 사유는 **디스크에**(별도 프로세스 진단은 모듈
    전역을 못 본다, #123 계열) · '갱신 중' 은 **진짜 진행 중일 때만**(#25·#35·#136) ·
    부제·패널 제목은 **같은 판정값**에서(#55·#38) · 감사는 부분문자열이 아니라 제품
    파서로 재고 점검 목록의 누락을 열거로 막지 말 것(#35·#75·#24) · **안 재는 기능을
    영향으로 적지 말 것**(#292).
    ⚠️ 의무 게이트(`make test`)와 도구 기본값(`pytest`)이 **다른 슈트**를 돌면 그
    슈트는 아무것도 막지 않는다 — 그런데 **합치면 깨진다**(`bot/tests/conftest.py` 가
    `sys.modules` 에 mock 을 모듈 레벨로 꽂아 같은 세션의 `tests/` 73건이 그걸 진짜로
    읽는다, #30 의 세션판). 게이트는 **별도 프로세스 둘**이고 그 이유를 pytest.ini 에
    적었다(안 적으면 다음 사람이 다시 합친다, #55).
    ⚠️ 독립 리뷰가 배포 전에 7건을 더 잡았고 **넷은 내가 그 턴에 넣은 가드가 스스로
...
344. **별표는 목록 속성인데 목록 캐시를 태웠고, 규칙 표가 결과 카드와 모순돼
    ...
    (a) **표시 속성 write 에 목록 캐시 무효화를 걸면 기능이 고장으로 보인다** —
        별 한 번에 139종목 가격이 `—` 가 됐다가 데몬이 수십 초에 걸쳐 채운다.
        쓸 땐 안 태우고(`invalidate_cache=False`) **읽는 시점에 디스크 정본에서
        덧입힌다** — 그래야 캐시(3분)·스냅샷(6시간) 어느 층이 행을 주든 별표가
        정본과 같다(#18·#21b·#95·#124·#198·#216 이 여섯 번 다 규율로 졌다 → #119).
    (b) **규칙 표와 결과 카드를 나란히 두면 모순으로 읽힌다** — 회복 구간 규칙은
        '총 50%' 인데 조건 충족 섹터가 0개면 결과는 0%/현금 100% 다. 규칙은 그대로
        두고 **활성 행에만 '→ 이번 판정 N%'** 를 덧붙일 것(#33·#34).
    (c) 그 답(`cash_reason`)을 **이력 표에만 배선**하고 카드엔 안 실어 사용자가
        물어야 했다(#123·#129·#189·#228·#234·#292 — 일곱 번째).
    ⚠️ 정의부터 창을 잘라 재는 소스 검사가 또 무너졌다(#60) — `_save` 에 스위치와
    사유 독스트링이 붙자 무효화 줄이 600자 창 밖으로 밀려 멀쩡한 코드가 빨간불.
    ⚠️ 칸을 **앞에** 하나 더하면 `split("</td>")[0]` 로 집던 회귀가 남의 칸을
    잰다 — 셀은 위치가 아니라 **내용으로** 찾을 것(#46).
    ⚠️ 그리고 독립 리뷰가 배포 전에 **Blocking 1 · High 1 · 그 밖 7**을 더
    잡았다(전부 반영): 락은 **전원이 참여할 때만** 상호배제다 —
    `add_favorite` 가 `_load()` 뒤 yfinance 로 **수 초**를 쓰고 그 낡은 목록을
    저장해, 그 사이 찍은 별표가 통째로 사라졌다(화면은 ★ 를 칠했다가 60초 뒤
    ☆ 로 되돌아간다). 네트워크는 락 **밖**에 두고 쓰기 직전 락 안에서 **다시
    읽는다**. 그리고 `장중` 을 `종가` 라고 적지 말 것 — 시장타이밍 보드가 같은
    원천에 대해 이미 `vol_asof_label` 로 그 판정을 갖고 있었는데 리터럴로
    복제해, 같은 카드가 기준일 줄엔 `🕒 장중(미확정)` 을 적고 VKOSPI 엔
    `종가` 를 적었다(#43a·#34·#38). **그리고 내 테스트가 그 리터럴을 못박아
    버그를 축복하고 있었다**(#19).
    ⚠️ 그리고 **운영 디스크 캐시 오염은 함수 스코프 fixture 로 못 막는다** —
    렌더가 띄운 daemon 스레드가 teardown **뒤에** 쓴다(#312 네트워크 차단이
    세션 스코프인 것과 같은 이유 · 실측: `make test` 가 업종맵 실패 도장을
    운영 캐시에 남겨 **다음 운영 실행이 15분 백오프**에 걸렸다, #30). 루트
    conftest 에서 import 시점에 갈아끼우고 **되돌리지 않는다**. 목록이
    완전하지 않으면 완전하다고 **주장하지 말 것**(#286).
    → REFERENCE §실수 #344
345. **감지기가 제 사건을 못 잡고, 사다리가 요구 대신 '아무거나'로 폴백하고,
    감사가 제 계약을 바꿨다 — 새 경로를 놓는 커밋의 세 가지 전형**(2026-09-12
    네이버 SPA 테마·리서치 상세 사다리 + 배포 drift, 셀프리뷰 6 + 독립 리뷰 18):
    (a) **drift 감지기가 자기 사건에서 'fresh' 라고 답했다** — `_STARTED =
        time.time()` 인데 호출부가 **핸들러 안에서 지연 import** 해서, 배포 직후
        첫 요청이 시각을 찍는다. 감지하려던 바로 그 상태에서 눈이 먼다(#91b 재는
        대상). 상태는 아는 쪽에 묻는다 — `/proc/self/stat` + `btime` 으로 **실측**
        하고, 폴백(import 시각)이 되살아나지 않게 호출부도 최상단 import.
    (b) **폴백 조건을 '실패했나' 로 잡으면 부분 성공이 폴백을 죽인다** — 새 JSON
        단이 투자의견만 주고 목표가는 모르는 키로 주자, 옛 HTML 에 **있는**
        목표가를 통째로 버렸다(지금 동작하는 경로보다 나빠진다). 조건은
        **'요구를 충족했나'** 다(#136) — 나는 그 규칙을 테마 쪽엔 적용하고
        상세 쪽엔 빠뜨렸다. **한 커밋에서 같은 규칙을 쓰는 자리를 전부 grep**할 것.
    (c) **#35('감사는 화면이 쓰는 그 경로를')에는 범위가 있다** — 원천 raw
        도달성을 재던 `/health` 행을 화면 경로(`fetch_themes`)로 바꿨더니 캐시를
        읽어 **34시간 낡은 스냅샷 위에서 ✅** 가 떴고(사용자가 신고한 그 상태를
        '건강' 으로 보고), 정지를 존중해 `/naverpause` 중에도 ✅ 였으며, 진단이
        운영 캐시에 **쓰기**까지 했다(#264·#283·#321). 그 행의 계약이 무엇인지
        먼저 답할 것 — 답이 '원천이 지금 답하나' 면 raw 여야 한다. 화면이 뭘
        들고 있나는 **별도 줄**로(#45 두 모집단을 한 줄에 섞지 말 것).
    ⚠️ 새 수집 경로엔 **형제의 가드를 그대로** 옮길 것 — 테마 사다리에
    `_UPJONG_DEFAULT_PAGE`/`_UPJONG_MIN_GROUPS` 에 해당하는 하한이 없어, 원천이
    `page` 를 무시하면 100개·`pageSize` 를 무시하면 20개만 받고 `partial=False`
    로 캐시에 구웠다(화면은 '전체 테마 N개' 라고 적는다, #45·#280·#341).
    ⚠️ **탐색은 '찾음' 을 '동작함' 으로 렌더하지 말 것** — 청크에서 읽은 경로의
    **호스트를 추측**해 `✅ 탐색됨` 으로 6시간 못박고 있었다. 후보 × 검증된
    호스트를 **실호출해** 값이 오는 것만 메모한다(#25·#79·#165).
    ⚠️ **일시정지 중엔 탐색하지 않는다** — 전 단이 `PAUSED` 로 실패하는 것은
    '죽었다' 가 아니라 '안 물어본 것' 이고(#79·#143), 그대로 두면 '못 찾음'
    메모가 6시간 냉각을 걸어 정지가 풀린 뒤에도 진짜 탐색이 막힌다.
    ⚠️ **테스트가 레포가 요구하는 설정에서 깨지면 그건 시한폭탄이다** — 소켓
    테스트가 `_authorize()` 를 타는데 CLAUDE.md §Stance 가 VM `.env` 에 요구하는
    `DASHBOARD_USER`/`PASSWORD` 가 있으면 401 이 되어 `make test` 가 무관한
    커밋에서 빨간불이었다(#294·#249·#291). 화면이 시키는 조치를 한 뒤에도
    green 인가를 물을 것.
    ⚠️ **경로 집계를 모듈 전역에 쌓지 말 것** — 풀 워커 6개가 동시에 증가시켜
    수를 잃고, 전역이라 시장·산업·종목 **세 화면의 건수가 한 통에 섞인다**
    (#45·#114). 그 행에 실어 화면이 자기 행에서 세게 한다.
    ⚠️ 그리고 **아직 측정된 적 없는 경로를 '정상' 으로 놓고 배지를 달면** 오늘
    동작하는 유일한 경로가 상시 경고가 된다(#25·#260) — 배지는 **경로가 갈렸을
    때**만.
    ⚠️ 반환 **모양**을 확인하고 쓸 것 — 프로브가 `collect_themes_json()` 을 3개로
    풀었는데 제품이 4개가 되자 `ValueError` 가 `except` 에 삼켜져 **멀쩡한
    사다리가 '실행 실패'** 로 찍혔다(#252 반환형 미확인 진단이 그럴듯한 거짓을 낸다).
    → REFERENCE §실수 #345

346. **이득 0인 파라미터가 400 을 부르고, 화면은 마지막 후보의 404 를 적고,
    전멸한 사다리를 클릭마다 처음부터 다시 걸었다**(2026-09-12 테마, 사용자
    "테마별 시세 들어가는데 시간이 꽤 걸리는데 이건 양이 많아서 그런건가?"):
    한 시간 전 100행을 주던 1순위 주소가 `HTTP 400` 인데 화면은 2·3순위
    (추측 후보)의 `HTTP 404` 를 적어 '주소가 사라졌다 → 새 주소를 찾아라' 로
    읽혔다 — 고칠 자리는 **우리 요청**이었다. 셋을 갈라 고쳤다: (a) 실측 ④ 가
    이 API 가족은 `page` 를 **무시**한다고 확정했으므로 얹어도 이득이 0이다 —
    **증명된 호출 모양**을 그대로 쓴다(이득 0인 파라미터는 거절의 후보일 뿐,
    #61) (b) 실패 사유는 `reason_rank`(0 일시정지 / 1 답했는데 거절·못 읽음 /
    2 주소 없음 / 3 판정 불가)로 **가장 행동 가능한 것**을 고르고 같은 순위면
    앞 후보가 이긴다(#275·#82) (c) 전멸한 결과는 캐시에 안 굽히므로(#280)
    클릭마다 사다리 + 죽은 HTML 7쪽을 다시 걸었다 — 느림의 원인은 **양이
    아니라 실패 재시도**다. 사유를 **10분만** 믿는다(#152·#161·#303).
    ⚠️ 냉각은 **제품 경로에만** — `--check`·프로브는 계속 잰다(#35 의 경계,
    #345c). 일시정지는 기록하지 않는다(안 물어본 것이다, #79·#143). 냉각 중엔
    `refreshing=False`(#25) 이고 화면이 "직전 기록 · N분 뒤 재시도" 라고
    밝힌다(#165·#43). 식으면 **반드시 다시 시도**한다(#178).
    ⚠️ 단마다 **소요를 싣는다** — 재지 않으면 '느리다'는 진단이 아니다(#69·#110).
    → REFERENCE §실수 #346

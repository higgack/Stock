...
- 외부 third-party 사이트는 `/sites`(`_SITES_TEXT`) 기본, 이모지 없는 plain text.
  메인 nav 는 **`bot/external_sites.SITES` 등재분만** 예외 허용(사용자 2026-08-21 "자주
  봐야겠어" — 부동산 뒤 `|` 구분). 그 목록이 /sites 와 nav 단일 출처이고 회귀가 양쪽
  실림을 강제. 아무 사이트나 nav 추가 금지 — 등재는 사용자 요청분만.

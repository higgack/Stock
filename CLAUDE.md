...
- 값을 비웠으면 사유도 말할 것: #43 #123 #129 #131 #189 #195 #228 #234 #282 #290 #292 #295 #296 #297 #298 #304 #305 #307 #308 #315 #317
...
- 같은 계산 하는 다른 화면 즉시 grep: #38 #147 #211 #241 #248 #254 #288 #289 #291 #293 #314
...
317. **비용 원장이 '공짜'라고 말할 수 있었다 — 단가표가 두 벌 + 모르는 모델은
    ...
    (a) 단가표(`_PRICING`)와 산식이 `bot/usage_tracker` 와 `trade/llm_usage` 에
        ...
        **둘을 합산**하므로 단가를 한쪽만 고치는 날 화면이 조용히 틀린다(#38).
        두 식의 차이는 부동소수 연산 순서뿐(1e-20)이라 위임해도 값이 안 바뀐다 —
        재고 나서 합쳤다. `KRW_PER_USD` 도 같은 계열이라 같이 묶었다.
    ...
    ⚠️ 그리고 그 표식을 **집계와 두 화면(대시보드 줄·텔레그램)까지** 배선했다 —
    계산해 두고 안 실으면 없는 것과 같고, 그게 이 레포에서 여섯 번째가 될
    자리였다(#43·#123·#189·#228). 0 건이면 조용하다.

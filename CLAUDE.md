...
- /ticker 상세 페이지 가격 차트 (2026-06-03, 사용자 요청 — 대시보드 주 소비) → `bot/chart_data.py` `build_price_chart(ticker)` 가 분석 종료 후 yfinance 1년 종가 + 10EMA/50SMA/200SMA 를 `_compute_technical_snapshot` **동일 계산식**으로 산출 (차트 MA = 본문 TECHNICAL SNAPSHOT SSoT 일치). `archive.py` SCHEMA_VERSION 2 로 `price_chart` 필드 저장 (parallel arrays: times/close/ema10/sma50/sma200, 통화·decimals 포함, ~11KB/분석, try/except 비치명적). `dashboard.py` `_render_detail` 이 `_render_chart_section` 으로 종가+3 MA 라인 차트 emit (lightweight-charts v4.2.0, 다크/라이트 테마 연동, JSON 은 `<script type=application/json>` 블록 — `</` defuse). 라이브러리는 `_ensure_chart_lib()` 가 regen 시 ARCHIVE_ROOT 에 1회 자동 다운로드 → **self-host (CDN 의존 제거, 오프라인 작동)**, 실패 시 텍스트만 graceful. **새 분석부터** 차트 (옛 v1 기록은 text-only — 재분석 또는 백필 시 생성). **Phase 2 TODO**: (a) entry/stop/target 수평선 마커 — trade plan 자유텍스트 파싱이 fragile (잘못된 라인 = 오독 유발) 해서 v1 제외, 포맷 확정 후 추가. (b) 옛 기록 소급 백필 일회성 스크립트. universal (전 시장).

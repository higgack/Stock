...
| `trade-bot-health.timer` | 1시간 | `trade.scripts.health_check` | BeOn 사이클 갭 감지 + **수신 누락**(#406 — 릴레이 유닛 저널의 `forwarded N` 과 trade-bot 저널의 받음(`ingested` + 출처 게이트 버림)을 최근 2시간 대조해(봇 저널은 30분 앞부터) 보낸 만큼 못 받았거나 **릴레이 원천의** 글을 게이트에서 버렸으면 알림 · 다른 출처(BeOn 이 되포워드한 남의 글)의 버림은 무음 · 같은 누락은 한 번(누락마다 표식) · 저널을 못 읽으면 판정 불가 경고 로그만) — 원인 가르기는 `cd ~/stock-trade && .venv/bin/python -m trade.bot_health`(읽기 전용 · getUpdates 안 부름 · 토큰 안 찍음) |

...
| `trade-bot-health.timer` | 1시간 | `trade.scripts.health_check` | BeOn 사이클 갭 감지 + **수신 누락**(#406 — 릴레이 유닛 저널의 `forwarded N` 과 trade-bot 저널의 `ingested` 를 최근 2시간 대조해 보낸 만큼 못 받았으면 알림 · 같은 누락은 6시간에 한 번 · 저널을 못 읽으면 판정 불가 경고 로그만) — 원인 가르기는 `cd ~/stock-trade && .venv/bin/python -m trade.bot_health`(읽기 전용 · getUpdates 안 부름 · 토큰 안 찍음) |

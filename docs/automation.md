...
| `trade-bot-badonion-sync.timer` | 6시간 | `trade/scripts/backfill_badonion.py` | 배도니언 소스 백필 동기화 — 기본 최근 3일. 관련성 필터(= 레지스트리 파서) 코드 지문이 마지막 성공 기록과 다르면 **40일 1회**(파서가 생기기 전에 리스너가 버린 캡션 회수, #403) · 기록 `~/.trade/badonion_sync_state.json` · 포워드가 일부 실패하거나 도중에 중단되면 기록 대신 재시도 표식을 남기고 다음 자동 동기화가 최근 40일을 다시 훑는다(지문이 이미 기록돼 있어도) — 자동 회수는 연속 실패로 끊지 않고 끝까지 시도하며, 도중 중단은 **긴 FloodWait 만** 표식만 남기고 안 센다(기다리면 풀린다) · 센 실행 3회째엔 기록하고 **실패한** 원본 msg id 와 **중단으로 시도 못 한** id 를 갈라 사람이 돌릴 명령과 함께 경고·알림으로 · 중단·완료 알림은 이 판정을 붙여 한 번 간다 · 프로세스가 죽은 실행은 셈도 표식도 남기지 않는다 · 자동 회수는 100유닛 상한 · 포워드할 유닛은 머리·소스 이름을 로그로 찍는다 · 캡션 하나가 **어디로** 갔는지는 `--dry-run --since YYYY-MM-DD --find TEXT`(원문·마크다운, 대소문자 무시 — `--since` 없으면 기본 창(평소 3일)만 본다), **왜** 버려졌는지는 그 출력이 찍는 `diagnose_badonion --grep` · dry-run 류는 라이브 세션이 있으면 그 복사본으로 접속해 타이머와 잠금 경합하지 않고(없거나 복사가 실패하면 라이브 경로 + 경고 · 인증이 풀린 복사본엔 로그인하지 않고 멈춘다), 후보 상한에서도 멈추지 않으며, 시작 실패도 폰에 알리지 않는다(타이머 장애로 읽힌다) · 클라이언트는 세션 형식 가드를 거친다(#404 — 못 읽는 형식이면 생성 전에 멈추고 깔 판을 알린다 · 운영 세션을 새 형식으로 올리는 것은 운영 venv(`.backfill-venv`)가 고정판일 때만 — 다른 인터프리터는 판이 같아도 막고, 형식을 못 잰 운영 세션도 막는다. 고정판 = `trade/scripts/requirements.txt`, 자동 배포는 pip 를 안 돌린다) · 재게시 글(나쁜양파가 다른 채널에서 퍼 온 글)은 포워드 **전에** (원래 채널, 원래 글번호)를 `~/.trade/relay_origins.json` 에 보증한다(#411 — 봇은 원래 출처로 받으므로 보증이 없으면 버린다 · 보증을 못 쓰면 그 유닛은 포워드하지 않고 완료 알림이 삭제·포워드 실패와 **갈라** 센다 · `to-forward`·`find` 줄이 원래 출처를 찍는다) |
...
| `trade-bot-health.timer` | 1시간 | `trade.scripts.health_check` | BeOn 사이클 갭 감지 + **수신 누락**(#406 — 릴레이 유닛 저널의 `forwarded N` 과 trade-bot 저널의 받음(`ingested` + 출처 게이트 버림)을 최근 2시간 대조해(봇 저널은 30분 앞부터) 보낸 만큼 못 받았거나 **릴레이 원천의** 글을 게이트에서 버렸거나 채널 글을 처리하다 **예외로 놓쳤으면**(수신 줄 없는 번호 — 수와 무관한 직접 증거, 2차 리뷰 H1) 알림 · 릴레이가 **보증한** 재게시 글을 보증 뒤에 버린 것도 알린다(#411 — 보증 기록 `~/.trade/relay_origins.json` 을 봇과 같은 데이터 디렉터리에서 읽는다 · 처방은 .env 가 아니라 보증 기록·데이터 디렉터리) · 보증 없는 다른 출처 포워드의 버림은 무음(BeOn 이 되포워드한 남의 글이 대부분 — ⚠️ 그래서 보증하기 **전의** 판이 포워드한 재게시 글의 버림은 알림엔 안 잡힌다, 진단 메모가 출처·건수와 가르는 명령을 적는다 · 3차 리뷰 M2) · 같은 **사실**은 한 번(**전달된** 알림의 포워드 사건·버림·예외 신원만 `~/.trade/.health-markers/delivery-alerted.json` 에 적는다 — 전달 실패는 다음 실행이 다시 알린다, 3차 리뷰 M4 · 다음 실행은 아직 안 알린 사건만 대조 — 끊김이 이어지면 새로 빠진 포워드만 알린다 · 2일 지난 기록은 지운다, 2차 리뷰 L4) · 저널을 못 읽으면 판정 불가 경고 로그만) — 원인 가르기는 `cd ~/stock-trade && .venv/bin/python -m trade.bot_health`(읽기 전용 · getUpdates 안 부름 · 토큰 안 찍음) |

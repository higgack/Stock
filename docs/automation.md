...
| `trade-bot-health.timer` | 1시간 | `trade.scripts.health_check` | BeOn 사이클 갭 감지 + **수신 누락**(#406 — 릴레이 유닛 저널의 `forwarded N` 과 trade-bot 저널의 받음(`ingested` + 출처 게이트 버림)을 최근 2시간 대조해(봇 저널은 30분 앞부터) 보낸 만큼 못 받았거나 **릴레이 원천의** 글을 게이트에서 버렸거나 채널 글을 처리하다 **예외로 놓쳤으면**(수신 줄 없는 번호 — 수와 무관한 직접 증거, 2차 리뷰 H1) 알림 · 다른 출처 포워드의 버림은 무음(BeOn 이 되포워드한 남의 글이 대부분 — ⚠️ 그래서 나쁜양파가 **재게시**한 관련 글의 버림도 알림엔 안 잡힌다, 진단 메모가 출처·건수와 가르는 명령을 적는다 · 3차 리뷰 M2) · 같은 **사실**은 한 번(**전달된** 알림의 포워드 사건·버림·예외 신원만 `~/.trade/.health-markers/delivery-alerted.json` 에 적는다 — 전달 실패는 다음 실행이 다시 알린다, 3차 리뷰 M4 · 다음 실행은 아직 안 알린 사건만 대조 — 끊김이 이어지면 새로 빠진 포워드만 알린다 · 2일 지난 기록은 지운다, 2차 리뷰 L4) · 저널을 못 읽으면 판정 불가 경고 로그만) — 원인 가르기는 `cd ~/stock-trade && .venv/bin/python -m trade.bot_health`(읽기 전용 · getUpdates 안 부름 · 토큰 안 찍음) |
...
| `trade-bot-badonion-listener.service` | `trade.scripts.listen_badonion` | 나쁜양파(태국·말련·필리핀·멕시코 등) 실시간 forward | 세션 미인증·세션 형식 불일치(exit 78, #404) |

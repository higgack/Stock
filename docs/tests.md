...
## #403 — 파서가 생기기 전에 버려진 캡션의 자동 회수 (`trade/tests/test_badonion_sources.py::TestSyncRecoveryWindow20260924` 31건 · `trade/tests/test_backfill_badonion_sync.py` 68건 — 그중 #404 몫 7건 · 2026-09-24 4차 리뷰 뒤 실측)
...
`iter_messages`(limit·reverse) 동작은 가짜의 흉내다. 수렴은 끝낸 실행과 **진전 없이**
끊긴 실행을 센다 — FloodWait·진전 있는 중단은 세지 않고, 프로세스가 죽은 실행(systemd
타임아웃 kill — 유닛 `TimeoutStartSec=600` 과 FloodWait 상한 600초가 겹친다, 4차 리뷰 L8
미반영)도 `finish_recovery` 에 닿지 못해 세지 않는 중단과 같다. 세지 않는 중단이 끝없이
반복되지 않는다는 근거(FloodWait 은 풀린다 · 진전은 남은 수로 유계)는 재지 않은 가정이다 —
텔레그램이 매 틱 긴 제한을 거는 동안엔 6시간마다 중단 알림이 온다(조용하지는 않다).

## #404 — 세션 형식 가드: 옛 telethon 이 새 형식 세션을 생성자에서 조용히 못 열었다 (`trade/tests/test_tg_session_format.py` 29건 · `trade/tests/test_backfill_badonion_sync.py` 7건 · 2026-09-24 4차 리뷰 뒤 실측)
...
| 축 | 무엇을 재나 | 테스트 |
|---|---|---|
| ① 형식 측정 | 세션 DB 버전을 **읽기 전용**(`mode=ro`)으로 · 없는 파일·쓰레기·빈 파일·**version 행이 없는 표**는 None · **핫 저널을 되돌리지 않는다** — 자식 프로세스로 쓰다 죽은 트랜잭션을 만들어 파일 바이트·`-journal` 이 그대로인지 잰다(4차 리뷰 T05·T06) · 설치된 telethon 판·형식은 가정이 아니라 측정 | `test_tg_session_format.py::test_the_session_version_is_read_read_only` · `::test_a_missing_or_foreign_file_is_not_a_version` · `::test_an_empty_version_table_is_not_a_version` · `::test_measuring_never_rolls_back_a_hot_journal` · `::test_the_installed_telethon_format_is_measured_not_assumed` |
...
| ⑤ 메커니즘 | 실물 telethon 이 v7 파일을 열면 제 형식으로 올린다 — 가정이 아니라 사고의 메커니즘 · 실물 telethon 을 **고정판으로 둔 채** 운영 venv 가 아닌 경로에서 v7 운영 세션을 가드로 열면 안 올라간다(4차 리뷰 M1 재현 — 옛 판은 판이 같아 통과했다) | `::test_a_newer_telethon_upgrades_an_old_session_on_open` · `::test_a_same_version_foreign_venv_cannot_upgrade_the_live_session_for_real` |
...
함께 알린다. ⑤ 의 두 테스트만 실물 telethon 이고(없는 환경이면 skip) 나머지는 가짜
...
분류했다 — 재지 않은 분류였다(4차 리뷰). 이제 ① 이 핫 저널을 만들어 잰다.

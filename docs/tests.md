...
## 장전·장후 저장분 (`TestPrepostStoredSnapshot20260919`, 14건)
...
| ④ 창 안·뒤처짐 | 저장분이되 '다음 창' 꼬리는 안 붙인다(그 문장이 거짓, #55) |
| ⑤ 문구 단일 출처 | 두 렌더가 `stored_note` 를 부르는지 AST + 나이를 숫자로(#202) + 안 잰 것 단정 금지(#165) |
| ⑥ 화면 E2E | 표가 그려지고 `sm-note` 칸에 집계 시각·나이가 실린다 + `.sm-note` CSS 정의(#201·#273) |
| ⑦ 옛 이름 | `_*_prepost_fresh` 가 되살아나지 않는다(창 밖을 '신선'이라 부르던 그 이름, #34) |

뮤테이션 10종(만료 24h 되살리기 · `stale` 상수화 · `stale_min` 을 늘 None ·
두 렌더의 저장분 줄 제거 · '다음 창' 꼬리 무조건 · 나이 미측정을 라이브로 ·
꼬리 반환에서 라벨 떼기 · '저장분도 없습니다' 제거 · 창밖 사유 제거) **전부
잡힘**. 베이스라인 green 확인 + 치환 1건 단언 + md5 복원 확인을 같이 했다
(#328·#358·#383). 옛 코드에서는 14건 중 13건이 빨간불이었다(재현 먼저,
§Pre-commit 9).
...
⚠️ **`grep | head` 로 '다른 호출부 없음' 을 판정하다 하나를 놓쳤다** —
`TestUsPrepost::test_prepost_fresh` 가 `head -25` 에 잘려 안 보였고 전체
게이트에서 ImportError 로 드러났다. 옛 계약은 지우지 않고 **새 계약으로 다시
썼다**(#222). 자르는 자리가 다음 결정을 가리지 않는지 먼저 물을 것
(#156·#338·#350 — 같은 함정 네 번째).

**못 보는 축**(#274): (a) 스냅샷이 **몇 세션 뒤처졌는지**는 재지 않는다 —
파이프라인이 죽으면 "N일 전" 나이 라벨과 `failed` 배너가 신호이고, 달력 기반
세션 격차 판정은 장전 창에서 상시 경고가 될 위험이 있어 안 넣었다.
(b) `ttl=86400` 을 쓰는 다른 20여 곳은 그대로다 — 그쪽은 창 밖에서도 kick
하므로 mtime 이 갱신돼 같은 구멍이 없다(형제 `fetch_us_movers`·`fetch_jp_stop`
으로 확인). "재스캔 안 함 + TTL" 조합이 또 생기면 같은 사고가 난다.

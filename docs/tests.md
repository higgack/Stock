...
| ⑫ 지금 프로세스(리뷰 H1) | 409·폴링·버림·예외 판정은 **MainPID 의 줄**로 — 재시작 전 프로세스의 것은 ⚠️ 메모(예외로 놓친 글 메모는 2차 리뷰 L6 이 '독스트링만 약속하고 없다' 고 짚어 더했다) · 대조군: 같은 줄이 지금 프로세스 것이면 ❌ · 막 뜬 프로세스는 첫 폴링 전이라 멈춤과 같은 창만큼 봐 준다(대조군: 오래 떠 있는데 0회면 ❌) | `::test_verdict_judges_the_running_process_and_notes_the_previous_one` · `::test_losses_to_exceptions_before_the_restart_are_a_note` · `::test_a_just_started_process_is_not_called_stalled_before_its_first_poll` |

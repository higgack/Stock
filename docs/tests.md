...
| ⑨ 상태도 만료 없음 | 26h 된 `failed` 도장이 살아 있어 배너가 뜨고, `stored_note` 가 '다음 창' **약속 대신** 실패 사실을 적는다(#380) + 기계 상세는 안 샌다(#391) |
...
뮤테이션 **18종 전부 잡힘**(만료 24h 되살리기[저장분·상태] · 창안 단락 제거 ·
...
배선 제거 · 백오프 리터럴 복귀). 베이스라인 green 확인 + 치환 1건 단언 + md5
...
신호는 "N일 전" 나이 라벨과 `failed` 배너 둘이고, 달력 기반 세션 격차 판정은

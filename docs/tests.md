...
## #403 — 파서가 생기기 전에 버려진 캡션의 자동 회수 (`trade/tests/test_badonion_sources.py` +28 · `trade/tests/test_backfill_badonion_sync.py` 55건 — 그중 #404 몫 4건)
...
`iter_messages`(limit·reverse) 동작은 가짜의 흉내다. 수렴은 **중단된 실행까지** 센다 —
프로세스가 죽은 실행(systemd 타임아웃 kill)은 `finish_recovery` 에 닿지 못해 못 센다.

## #404 — 세션 형식 가드: 옛 telethon 이 새 형식 세션을 생성자에서 조용히 못 열었다 (`trade/tests/test_tg_session_format.py` 19건 · `trade/tests/test_backfill_badonion_sync.py` 4건)
...
| 축 | 무엇을 재나 | 테스트 |
|---|---|---|
| ① 형식 측정 | 세션 DB 버전을 **읽기 전용**(`mode=ro`)으로 · 없는 파일·쓰레기·빈 파일은 None · 설치된 telethon 판·형식은 가정이 아니라 측정 | `test_tg_session_format.py::test_the_session_version_is_read_read_only` · `::test_a_missing_or_foreign_file_is_not_a_version` · `::test_the_installed_telethon_format_is_measured_not_assumed` |
...
| ③ 판정 | 파일 형식 > 라이브러리 형식이면 막고 **깔 판**을 말한다 · 고정판이 아닌 판이 **운영** 세션을 올리려 하면 막는다(복사본·고정판은 허용) · 같거나 못 재면 막지 않는다(#54 는 '말하라' 이지 '멈추라' 가 아니다) | `::test_a_session_newer_than_this_telethon_names_the_fix` · `::test_an_unpinned_telethon_may_not_upgrade_a_live_session` · `::test_matching_or_unmeasurable_formats_never_block` |
| ④ 생성 | 막히면 클라이언트를 **만들지 않는다** · 통과면 인자를 그대로 넘긴다 · 알림 사유는 처방 문장 그대로 | `::test_the_guard_refuses_before_building_and_passes_through_otherwise` · `::test_the_failure_note_carries_the_prescription` |
| ⑤ 메커니즘 | 실물 telethon 이 v7 파일을 열면 제 형식으로 올린다 — 가정이 아니라 사고의 메커니즘 | `::test_a_newer_telethon_upgrades_an_old_session_on_open` |
| ⑥ 배선 전수 | 트레이드 운영 코드의 `TelegramClient(...)` 직접 생성 0건(디렉터리 전수, #24) · 생성 지점이 가드를 거친다 · 가드가 말하는 venv 가 telethon 유닛 ExecStart 의 venv | `::test_no_production_code_builds_a_telegram_client_directly` · `::test_every_telethon_unit_runs_the_venv_the_guard_names` |
| ⑦ 리스너 | 형식 불일치는 알리고 **EX_CONFIG(78)** 로 끝나 재시작 루프를 막는다(두 리스너) · `--auth` 도 가드를 거친다 | `::test_a_listener_with_an_unreadable_session_stops_cleanly_and_says_why` · `::test_interactive_auth_also_goes_through_the_guard` |
| ⑧ 동기화 | 생성자가 던져도 알림 `try` 안(시작 실패 알림 + rc 1, 기록 없음) · dry-run 은 알리지 않는다 · 가드는 **생성 전에** 돈다 · 라이브 경로만 live 로 잰다 · BeOn 동기화도 같은 규약 | `test_backfill_badonion_sync.py::test_a_session_this_telethon_cannot_open_pages_instead_of_dying_silently` · `::test_a_dry_run_that_cannot_open_the_session_logs_but_does_not_page` · `::test_the_format_guard_runs_before_the_client_is_built` · `::test_only_the_live_session_is_guarded_as_live` · `test_tg_session_format.py::test_the_beon_backfill_pages_on_a_session_it_cannot_open` |
...
함께 알린다. ⑤ 만 실물 telethon 이고 나머지는 가짜 모듈 위라 진짜 생성자·잠금은 안
잰다. 파이썬이 아닌 자식이나 다른 라이브러리의 상태 파일은 이 가드 밖이다.
형식 측정의 `mode=ro` 는 **핫 저널**(쓰다 죽은 트랜잭션의 `-journal`)이 있을 때만
관측 차이가 난다 — 읽기·쓰기 연결이면 측정이 그 저널을 되돌려 파일을 바꾼다. 회귀는
그 상태를 만들지 않아 `mode=ro` 를 지우는 뮤테이션이 생존한다(동등으로 분류).

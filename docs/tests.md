...
| ⑤ 문구 단일 출처 | 두 렌더가 `stored_note` 를 부르는지 AST + 나이를 숫자로(#202) + 재시도 시점은 창 판정이 있을 때만(#165) + **정상 경로에선 빈 문자열**(사용자 2026-09-20) |
| ⑥ 화면 E2E | 표가 그려지고 **부제**가 `💾 저장분(N시간 전)`·기준시각을 말한다 + 본문 `sm-note` 엔 되풀이 줄이 **없다** + `.sm-note` CSS 정의(#201·#273) |
...
| ⑨ 상태도 만료 없음 | 26h 된 `failed` 도장이 살아 있어 배너가 뜨고, `stored_note` 가 **그때만** 한 줄을 낸다 — `자동 갱신` 은 금지어이고 '다시 시도합니다' 만(#380·#375) + 기계 상세는 안 샌다(#391) |
...
뮤테이션 **23종 전부 잡힘**(만료 24h 되살리기[저장분·상태] · 창안 단락 제거 ·
...
나이·기준시각 떼기). 베이스라인 green 확인 + 치환 1건 단언 + md5
...
신호는 **부제**의 "N일 전" 나이 라벨과 `failed` 배너 둘이고, 달력 기반 세션 격차 판정은
장전 창에서 상시 경고가 될 위험이 있어 안 넣었다. ⚠️ 첫 판은 그 `failed`

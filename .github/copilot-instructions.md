...
> **충돌하면 `/CLAUDE.md` 가 이깁니다** — 이 요약이 더 느슨해 보이면 그건 요약이
> 낡은 것이지 예외가 생긴 것이 아닙니다.

<!-- covers-claude-md-sections: 아래는 CLAUDE.md 가 **스스로 '⛔/의무'로 표시한**
     섹션 목록이고, 이 파일이 그 각각을 요약한다는 선언이다.
     tests/test_docs_consistency.py 가 CLAUDE.md 의 실제 헤딩에서 파생해 대조하므로,
     CLAUDE.md 에 새 ⛔/의무 섹션이 생기면 이 목록과 아래 요약을 같이 갱신해야 한다
     (누가 갱신하나 = 사람이 아니라 그 테스트다).
- ⛔ 과거 실수
...
## 자주 반복된 실수 (수정 전 확인) — **발췌**

> ⚠️ 아래는 CLAUDE.md `## ⛔ 과거 실수` 의 **초기 항목 발췌**다(원본은 현재 수백 개).
> 여기 없다고 해서 그 실패모드가 없는 것이 아니다 — 그 섹션 머리의 **§주제 색인**이
> 같은 병이 어디에 흩어져 있는지 계열별로 모아 두었으니 작업 전에 그것부터 볼 것.
...
## Pre-commit 검증 — 의무 (요약)
- 위 회귀 테스트·syntax 체크가 그 **일부**입니다. 전체 항목(로직 스모크 ·
  `_HELP_TEXT` 길이 · 룰 이동 시 orphan `grep` · 다단계 항목별 검증 · 배포전
  셀프리뷰 · 비자명 변경의 독립 리뷰 게이트 · **버그 수정은 재현 테스트 먼저**)은
  `/CLAUDE.md` 의 `## Pre-commit 검증 — 의무` 를 열어서 확인하세요.
- 스킵은 "명시적 커밋 요청 위반"과 같은 급으로 취급됩니다.

...
### ⚠️ 데드라인 정정 (2026-09-22, 사용자가 붙인 **Google Cloud 공식 공지**)

아래 "2026-10-16 전면 종료"는 **3자 소스 추정이었고 공식 일정과 다르다**.
이 절 자체가 "착수 전 반드시 공식 가격표/일정으로 재검증" 이라고 적어 뒀고
(#12·#165), 그 재검증이 2026-09-22 에 이뤄졌다. **현행 일정은 이것이다:**

| 단계 | 날짜 | 무슨 일이 일어나나 |
|---|---|---|
| Phase 1 공개 지원 중단 | **2026-10-20** | **활성 프로젝트는 그대로 동작**(현행 가격·SLA 유지). 신규/비활성 프로젝트만 차단 — '비활성' = 2026-07-22~10-20 에 그 모델로 API 호출 0건 |
| Phase 2 종료 ① | **2027-01-28** | gemini-2.5-**flash-lite** 종료 |
| Phase 2 종료 ② | **2027-03-31** | gemini-2.5-**flash**/**pro** 종료 (데이터 레지던시 지역 중 **대한민국·브라질·프랑스 제외**) |
| Phase 2 종료 ③ | **2027-08-06** | 위 3개 지역의 flash/pro 종료 |

공지가 명시한 **영향받는 프로젝트**: `gen-lang-client-0325676393`(flash-lite·
flash·pro) · `gen-lang-client-0957886559`(flash). 앞엣것이 이 레포가 Vertex
전환 때 쓴 바로 그 프로젝트다(§Gemini 백엔드 — region `us-central1`).

**우리에게 실제로 걸리는 것**: 세 티어 전부를 매일 부르므로(분석 파이프라인·
스크리너·Daily Byte류·번역) 그 프로젝트는 '활성'으로 남을 가능성이 높고,
그러면 **2026-10-20 에는 아무 일도 일어나지 않는다**. 진짜 마감은
**2027-01-28(flash-lite) → 2027-03-31(flash·pro)** 이다. ⚠️ 단 '활성' 판정은
GCP 쪽 집계라 **우리가 잰 것이 아니다** — 콘솔에서 두 프로젝트의 모델별
호출 이력(2026-07-22~)을 확인해야 확정된다. 그리고 `gen-lang-client-
0957886559` 는 이 레포 어디에도 없다 — 사용자의 다른 프로젝트로 보이나
재지 않았다.

⚠️ 공지가 요구한 조치 중 **thought signature 회전**은 이 레포에 구현이
없다(`thought_signature`/`thoughtSignature` grep 0건). Gemini 3.x 로 옮길 때
분석 파이프라인이 langgraph `ToolNode` 로 **다중턴 함수호출**을 하므로 그
경로가 서명 왕복을 요구할 수 있다 — langchain 이 투명하게 처리하는지는
**재고 나서** 말할 것(#12·#79).

⚠️ 공지의 권장 이전 대상은 아래 표(3자 소스)와 다르다: flash-lite →
**3.1 Flash Lite / Gemma 4**, flash → **3.8/3.7/3.5 Flash · 3.5/3.1 Flash
Lite**, pro → **3.8/3.7/3.5 Flash**(권장 목록에 3.x **Pro** 가 없다).
가격은 이 세션에서도 재지 못했다(샌드박스 차단) — 착수 전 공식 표 확인.

### 데드라인 — 3자 소스 추정 (2026-08-09, **위 공식 공지로 대체됨**)
...
### 다음 세션 착수 시
1. **마감은 2027-01-28(flash-lite) → 2027-03-31(flash·pro)** — 위 정정 참조
   (옛 "10/16" 은 3자 소스 추정이었다). 2026-10-20 은 활성 프로젝트엔 무영향
   이므로 그날을 기준으로 급하게 옮길 이유가 없다. 다만 **활성 판정을 콘솔에서
   먼저 확인**할 것(2026-07-22~10-20 모델별 호출 이력) — 비활성으로 잡히면
   그 모델은 10/20 부터 **새로 못 쓴다**. 사전 테스트(구조화출력/thinking_budget
   신모델 호환성 + **thought signature 왕복 필요 여부 실측**, 회귀 6,200+ 전체).
...
215. **주석에 적힌 "원천과 맞는다"가 한 종목만 보고 쓴 것일 수 있다**
    ...
    yfinance `Capital Expenditure` 와도 정의가 같아진다 — ⚠️ **이 뒷문장은
    #396 이 반증했다**(재지 않은 단언 · yfinance 는 무형을 묶는다).
...
354. **측정이 배선을 정했다 — 그리고 한 플래그에 두 사실을 담은 것이 전체
    실행에서만 터졌다**(2026-09-12 테마 정렬 합집합, #353 의 결말): `--probe-sorts`
    VM 실측이 "정렬을 섞으면 천장 너머가 보인다"를 확정했다(기준선 200 →
    합집합 **266**, `fallCnt` +56 · `leadingItem` +9 · `name` +1 · 나머지 11종 +0).
    그래서 수집이 정렬을 바꿔 한 번씩 더 묻고 합집합을 쓴다 — 그 전엔 화면이
    266개 중 200개를 '전체 테마'라고 적고(#45 모집단) `partial=True` 라 캐시도
    못 해 클릭마다 재수집했다. **재고 나서 배선한 것**이 이 라운드의 전부다
    (#12·#79·#151 값을 지어내 배선하면 죽은 경로를 배포한다).
    ⚠️⚠️ **`partial` 과 `lost` 는 다른 사실이다.** 훑기가 천장을 넘게 만들려면
    '쪽 무시'를 부분에서 빼야 했는데, 그 플래그는 **더 큰 크기가 같은 수를 줄 때
    사다리가 일찍 끝나는 경로에서 유일한 부분 증거**이기도 했다 — 플립하자
    100/130 이 완전본으로 구워졌고, 그건 **전체 실행에서만** 빨간불이었다
    (`-k` 필터로 돌린 형제 테스트들은 전부 green, #128·#311). 다섯째 값(`lost`
    = 정렬 훑기로도 못 메우는 손실: 쪽 요청 실패·쪽 상한)을 따로 두고, 훑기는
    `lost` 를 절대 덮지 못한다(#351b 뒷단의 성공이 앞단의 부분 증거를 지운 그
    사고). 한 플래그가 두 뜻을 겸하면 어느 쪽을 고쳐도 다른 쪽이 깨진다(#34).
    ⚠️ 완전본은 **훑기가 실제로 천장을 넘었을 때만** 말한다 — 훑기가 한 종목도
    못 늘렸다면 '총 개수가 그만큼'과 '원천이 정렬을 무시해 매번 같은 목록을
    자른다'가 구분되지 않는다(#165 잰 것만 · #341).
    ⚠️ 새 가드 **넷이 발화 경로 없이** 뮤테이션을 통과했다(#91·#291): 실측
    경로가 분기 A 로 끝나 **마지막 반환점의 배선**이 한 번도 안 걸렸고, `lost`
    는 다른 조건이 늘 먼저 참이라 두 갈래 다 안 걸렸다. 그리고 내가 `partial =
    partial or lost` 와 반환식의 `partial or lost` 를 **둘 다** 써 둬서 뮤테이션이
    **no-op 을 쳤다** — 통과하면 그 자리를 실제로 쳤는지도 볼 것(#267).
    ⚠️ 사유는 경고가 아니다 — `via` 가 페이지 **부제**에 그대로 실리므로 완전본
    수집에 사다리 경위("한도 1000 거절")를 붙이면 정상이 매번 경고로 읽힌다
    (#25·#260). 합산 사실만 화면에, 경위는 로그로(#43 버리지는 않는다).
    ⚠️ 정렬 목록은 열거형이라 원천이 이름을 바꾸면 눈이 먼다(#24) — 거절당한
    정렬은 **이름을 사유에 적어** 드리프트를 보이게 하고, 연속 실패·연속 0종이면
    멈춰 순손실을 막는다(#61·#346 느림의 원인은 양이 아니라 실패 재시도였다).
    ⚠️⚠️ **독립 리뷰가 배포 전 High 2 · Medium 3 · Low 6 을 잡았고 첫째가 이
    변경을 통째로 무효화하고 있었다**: (a) 쪽 요청 실패를 사유와 무관하게 손실로
    세어, 이 가족이 `page` 에 **400 을 주는 실측 거동**(바로 위 주석이 기록하고
    형제 픽스처가 재현한다)에서 266개를 다 모으고도 영원히 부분이라 캐시가 안
    생겼다 — 요청 모양 거절(4xx)은 천장이고 429·5xx·타임아웃이 손실이다(#82).
    **내 픽스처가 `page` 를 무시만 하고 400 을 안 줘서 그 세계를 한 번도 안
    태웠다**(#155 픽스처는 원천이 실제로 보내는 모양대로). (b) '아직도 자라는
    중' 가드가 무가드 (c) 배지를 `partial` 로 좁히자 **일부러 `partial=False` 로
    두고 사유만 남기는** 갈래(하한 미만·읽다 버린 행)가 ✅ 로 바뀌어 "못 읽어
    버렸습니다" 옆에 ✅ 가 놓였다 — 네 번째 값 `warn` 으로 갈랐다(#34·#43)
    (d) 훑기가 형제의 `wrong_resource` 가드·dict 언랩을 건너뛰어 업종 행이 테마
    목록에 섞이고 정상 응답이 '정렬 실패'로 찍혔다(#38 새 경로엔 형제의 가드를
    그대로 옮길 것) (e) 전경 경로인데 예산이 없었다(#116).
    ⚠️ 뮤테이션 둘이 **문법을 깨 무관 테스트를 빨갛게** 만들어 '잡혔다'처럼
    보였다 — 통과·실패 어느 쪽이든 그 자리를 실제로 쳤는지 볼 것(#267·#47).
    ⚠️ 예산 회귀를 `sleep` 으로 쓰자 **단독 green · 전체 실행 red** 였다(#128·
    #130) — **시계를 주입**해 결정적으로 만든다.
    ⚠️ **배포 뒤 사용자가 "클릭하면 들어가는게 여전히 느려"** — 화면 체크포인트
    (266개 · ✅)는 통과했는데 부제가 스스로 잰 값이 `21.7s` 였다. 신선도가
    아니라 **전경에서 수집을 기다리게 한 것**이 원인이다: SWR 창이 10분이라
    마지막 수집 후 10분이 지난 클릭은 저장분이 있는데도 동기 수집을 기다렸다.
    창을 KR 한 세션(6시간)으로 넓혀 **저장분을 즉시 주고 갱신은 뒤에서** 한다
    (화면은 '얼마나 낡았는지'를 이미 말한다, #116·#163). 캐시를 만들어 놓고
    **그 캐시가 사용자의 대기를 실제로 없애는지**는 따로 물을 것 — TTL·SWR·
    방문 간격이 어긋나면 캐시는 있어도 매 클릭이 전경이다.
    ⚠️ 그리고 이 라운드에 stop hook 이 "미커밋 변경을 커밋하라"고 두 번 요구
    했는데, 그때 워킹트리는 **리뷰·뮤테이션이 파일을 바꿔 둔 상태**였다. 트리
    md5 를 백업과 대조해 한 번은 막았고 한 번은 같은 내용임을 확인하고 커밋했다
    — #328 은 규율이 아니라 **커밋 직전 md5 대조**로 지킬 것.

### 실수 #355

355. **"둘 다 대비 확보"라고 적어만 두고 아무도 재지 않았다 — 라이트 모드가 1년 넘게
    AA 미달이었다**(2026-09-12 google-labs-code/design.md 검토에서 파생): `DESIGN.md`
    §4 가 "다크/라이트 둘 다 대비 확보(양쪽 토큰 정의 필수)"라고 **주장**하는데 그걸
    재는 코드가 없었다(#55 설명이 코드와 어긋나면 버그 · #25 '있다'를 묻는 검사에는
    반대 증거를 같이). 실측: 라이트 `--muted` 3.06:1 · `--pos` 3.54 · `--accent` 4.42 ·
    trade `--tone-export` **2.04** — `.si-note`(11px)·표(13~14px)로 실리는 기준시각·
    사유·등락 숫자가 전부 AA(4.5) 미달이었다. #43·#123·#129·#131·#189·#228 이 여섯 번
    싸워 "보이는 줄로 내라"고 올린 바로 그 정보다. 값이 다 '있어서' 어떤 감사도
    안 걸렸다(#96). 다크는 정상이라 **한쪽 테마만 틀린 것**도 눈에 안 띄었다.
    처방: 판정을 순수 모듈(`bot/css_contrast.py`)로 두고 회귀가 `bot/`·`trade/`
    **디렉터리 전수**로 잰다(#24·#176) — 켜자마자 22개 모듈 368쌍 중 **77쌍**
    (19개 모듈)을 잡았고(#87a),
    문서의 주장은 "회귀가 잰다"로 바꿨다. 토큰 값 교체 51건은 색조·채도를 유지한
    **최소 명도 이동**이다(디자인을 바꾸는 게 아니라 읽히게 만드는 것).
    ⚠️ **텍스트·표면 토큰을 이름으로 열거하면 반드시 샌다**(#24) — 손으로 적은 6개
    목록은 `--fg-soft`·`--pending`·`--item`·`--text-sub`·`--tone-*` 를 통째로 놓쳤고,
    `:root` 만 보면 `body.dark{}`(trade/dashboard)가, `--bg`/`--card` 만 표면으로
    보면 `--surface` 가 빠진다. 텍스트는 `color:var(--X)` **용법**에서, 표면은
    `body`/`.card` **선택자**에서 파생시킬 것. ⚠️ 단 그 선택자 파생은 오늘 레포에서
    **한 토큰도 기여하지 않는다**(이름 규약 셋이 전부 덮는다) — 즉 그 루프를 통째로
    지워도 회귀가 전부 green 이었고, 그 상태로 "선택자에서 파생한다"고 적으면 문서가
    자기 자신에 대해 거짓을 말하는 것이다(#286·#291 발화 경로 없는 가드는 가드가
    아니다). 합성 픽스처로 이름 밖 표면(`--page`)을 태워 경로를 살렸다.
    ⚠️⚠️ 그리고 **내 가드가 세 번 눈멀었고 전부 픽스처·뮤테이션이 잡았다**:
    (a) `"--text".endswith("-text")` 라 본문 토큰이 칩 전경으로 분류돼 검사 쌍이
        368→280 으로 줄었다 — 가장 중요한 토큰이 가장 먼저 눈먼다(#47 계수 패턴
        자체가 틀릴 수 있다). base 가 **진짜 토큰 이름인지** 요구해야 한다.
    (b) 선언 파서가 `([^;]+);` 라 **마지막 선언에 `;` 가 없으면** 그 줄을 버려
        팔레트가 '선언 2개'로 보이고 블록째 무검사가 됐다(#155 픽스처는 원천이
        실제로 내는 모양대로 — 합성 픽스처가 그걸 재현해 잡았다).
    (c) 칩 전경을 `-fg`+`-bg` 이름 규약으로만 보다 `--badge-text`↔`--badge` 를
        놓쳐 배지 글자색을 페이지 배경과 대조해 **1.00:1 오탐** 4건을 냈다(#50
        내 가정을 원천의 보장으로 착각). 접미 이름이 아니라 **짝 배경의 존재**로.
    ⚠️ 문턱을 `cc.failures(rows)` 기본값으로 재면 `AA_MIN` 을 1.0 으로 내리는
    뮤테이션이 통과한다 — **리터럴로** 못박을 것(#66 자기 상수로 자기를 검증하는
    tautology). 그리고 첫 M1 뮤테이션(색 되돌리기)이 '통과'로 보였는데 sed 가
    공백 때문에 **아무것도 안 친 것**이었다(#267 통과하면 그 자리를 실제로
    쳤는지부터 볼 것).
    ⚠️ 같이 넣은 둘째 가드 = **문서↔코드 드리프트**(`DESIGN.md` 색상표 ↔
    `bot/dashboard.py` 실값). 오늘 불일치는 0건이었지만 **양쪽이 갈라지는 걸 막는
    것이 아무것도 없었고**, 이번 커밋이 값을 11종 바꾸므로 그 순간 문서가 거짓이
    된다. 판정은 본 테스트와 fires 테스트가 **같은 함수**를 쓴다(#286 인라인
    재구현은 tautology).
    ⚠️ 이 검사가 **못 보는 축**(#274): 인라인 리터럴 색 · 비텍스트 대비(1.4.11 —
    `--border` 는 `--card` 대비 1.22:1 이지만 장식 구분선이라 **재지 않은 것을
    단정하지 않는다**, #165) · 칩 전경×자기 칩 배경 · 큰 글씨 예외(이 레포 본문은
    11~14px 라 전부 small text). 그리고 `--neu` 는 6곳에 선언돼 있는데
    `var(--neu)` 참조가 **0건**이었다 — 문서에 그렇게 적었다(#53 죽은 이름).
    ⚠️⚠️ **그리고 이 fix 자체가 다른 축을 악화시켰다**(독립 리뷰가 배포 전에 잡았다).
    활성 칩은 `background:var(--accent)` + **리터럴 흰 글씨**인데, 접근성을 위해 다크
    `--accent` 를 밝히자 그 흰 글씨가 **3.65 → 3.00** 으로 나빠졌다 — 접근성 커밋이
    접근성을 깎은 것이다. 한 토큰이 '어두운 배경 위 글자'(밝아야 함)와 '그 위에
    놓이는 글자'(어두워야 함)를 겸할 수 없다(#34 한 라벨이 두 계정을 대표하면 한쪽은
    반드시 거짓말) → `--accent-on` 전경 토큰을 두고(잉크는 테마를 짐작하지 않고
    **측정이 정한다**) 리터럴 `#fff` 19곳을 배선했다. 다크 2.53~3.65 → **5.26~7.41**.
    일반화: **대비를 고칠 땐 그 토큰이 배경으로도 쓰이는지 먼저 물을 것** — 한쪽을
    올리면 다른 쪽이 내려간다. 그리고 그 축을 가드로 못박아야 다음에 또 안 난다
    (`on_pairs` — 이름 열거가 아니라 선언된 `--X-on` 전수).
    ⚠️ 같은 리뷰가 셋 더: 드리프트 판정이 팔레트 블록 셋을 `.update()` 로 **합쳐**
    마지막 블록만 검사하고 있었고(#45 — Screener 팔레트가 문서와 갈라져도 통과),
    쌍 하한 250 이 **이 가드가 그물이라고 광고한 바로 그 붕괴**(368→280)를
    통과시켰으며(#25·#260 늘 통과하는 문턱은 안 재는 것과 같다), `_SELECTOR_OK` 가
    버린 팔레트 블록은 **어떤 가드도 못 본다**(0건인 지금 못박았다, #54).
    ⚠️ 리터럴이 나르는 글자는 **가드를 두지 않았다** — 문맥(어느 배경·어느 테마)을
    모르는 기계 검사는 오탐투성이다(실측 238건 중 217건이 '미달'로 나오는데 `#fff`
    은 칩 위 글씨이고 다크 리터럴은 다크 배경 위다, #47). 범위를 **숫자로** 밝히고
    (`DESIGN.md` §4) 등락 숫자(`--up`/`--down` 14개 모듈)와 `--fg-soft` 미정의
    폴백만 토큰으로 옮겼다 — 후자는 그 페이지 팔레트에 **없는 토큰**을 참조해 늘
    리터럴(2.62:1)로 떨어지고 있었다(#53 죽은 이름 · #273).

### 실수 #387

387. **히스토그램만 주는 보고서가 "이걸 붙여넣으면 고친다" 고 말하고 있었다**
    (2026-09-18 수주잔고 격주 리뷰): 사유·관문 계수만 싣고 원문이 없어 **같은
    보고서**가 범위(#105)·관문(#107)·어휘(#109)·창(#275) 넷을 연달아 오진하게
    만들었다. 네 번 다 처방은 "표본 원문을 같이 찍을 것" 이었는데 정작 보고서
    자체는 안 고쳤다 — `backlog_excerpt` 는 미스 시점에 이미 계산되는데
    `_log_miss` 가 그걸 버렸다(#123 계열: 계산해 두고 표시까지 배선 안 함).
    그래서 보고서를 받을 때마다 운영자가 `--ticker` 를 종목 수만큼 돌려야
    했다(§Automation-first: 운영자 반복명령을 요구하는 fix 는 잘못된 fix).
    규칙: **다음 수를 정하라고 내는 진단에는 그 수를 정할 근거가 같이 실려야
    한다** — 계수는 '무엇이 많은가' 까지만 말한다(#93).
    ⚠️ 발췌를 실을 땐 갈래를 **개수로 자르지 말 것**(예산이 남는데 조용히
    버리고 그 수가 '생략' 계수에 안 잡혀 "전부 봤다" 가 거짓이 된다, #45) ·
    신원에 **관측**(상세)을 넣지 말 것(#18) — 파서를 고치면 폐기된 원문이
    '고칠 유일한 근거' 로 남는다.

**증상** 격주 보고서(`bot.dart_backlog.review_text`, 짝수주 금 16:00 KST)가
`막힌 조회 18건 · 종목 10개` + 관문 히스토그램(`헤더에 기초·수주총액 열 없음
9건` 등)만 싣고 맨 끝에 "이 목록을 Claude 에게 그대로 붙여넣으면 파서를
확장합니다" 라고 적었다. 그 목록으로는 `헤더에 기초·수주총액 열 없음 9건` 이
파서가 이미 지원하는 잔고 단일열형(D형)인지 수주와 무관한 표인지 구별할 수
없었다.

**같은 보고서가 만든 네 번의 오진** #105 는 진단이 파서와 다른 범위를 훑어
지원하는 단위를 '미지원' 이라 말했다. #107 은 진단이 '합계행 5값' 이라 적었지만
실제로는 그 앞 헤더 게이트에서 막혀 검산 코드가 한 번도 안 돌았다. #109 는
`기말`·`기초` 가 DART 원문 어디에나 있어 수주와 무관한 표 8건을 '개선 여지' 로
셌다. #275 는 진단이 캡션을 본문 전체에서 역탐색해 파서가 거부하는 창 밖
캡션을 '미지원단위' 로 셌다. 네 항목 모두 원문 발췌를 같이 찍으라고 끝났는데,
그 문장을 진단 CLI 에만 넣고 보고서 자체에는 안 넣었다.

**고친 것** `_log_miss(..., excerpt=)` 가 발췌를 `ex` 로 400자까지 남기게 했고,
`excerpt_samples()` 단일 선택기를 보고서와 CLI 가 같이 쓰게 했다. 보고서는
`<`/`>`/`&` 를 escape 하고 보낼 메시지 전체를 재는 예산(4,000 u16) 안에서만
실었으며 자른 갈래 수를 적었다. 발췌 없는 줄은 사유별로 셌다 — `시계열이상`
은 `quarterly_infographic` 이 원문 없이 남기므로 "다음 조회부터 붙습니다" 가
거짓이었다. 그리고 `--refill` 이 발췌 없는 줄을 다시 조회해 되메우고 값이
나오면 그 줄을 지우게 했다. 원장은 파서 지문이 바뀌어도 안 비워지므로 지우는
쪽이 없으면 건수가 영영 부풀었다.

**독립 리뷰가 배포 전에 뒤집은 둘** 갈래를 `_EX_SAMPLE_N=6` 으로 조용히
잘랐다 — 실측 10갈래 중 6개만 뜨고 생략 문구가 없었으며 메시지는 1294/4096
으로 여유가 많았다. 갈래가 6을 넘는 건 예외가 아니라 기본값이었다(진단 문구에
캡션 원문과 `{gap}자` 가 박힌다). 그리고 예산을 따로 계산해 두면 그 식의
피연산자(머리말)를 빼는 변형이 안 잡혀 실제로 4,106 u16 가 나갔다. 4096 을
넘기면 텔레그램이 거절하고 그 예외는 `_periodic_backlog_review` 의
`log.exception` 에만 남아 보고서가 화면에서 통째로 사라졌다.

### 실수 #390

390. **"다음 갱신에 나옵니다" 는 재지 않은 약속이었고 구조도 수렴하지 않았다**
    (2026-09-19 급등·급락·거래량 상위가 이틀째 `❌ 0행`): 폴백이 렌더 예산을 넘기면
    비우고 그 문장을 적었는데 (a) 참인지 **아무도 안 쟀고**(#380) (b) 참이어도 TTL 이
    지나면 **이미 받아 둔 종가를 버려** 또 비었고 (c) 일일 감사는 **매번 콜드
    프로세스**라 정의상 영원히 ❌ 였다(#260). 만료분은 **나이와 함께** 내고 갱신은
    뒤에서(SWR — 상한은 임의값이 아니라 **신선 경로가 받아들이는 기준일**에서 파생,
    #269) · 시도마다 **관측 기록**을 남겨 화면·감사가 약속 대신 사실을 말하게(#86·#82)
    · '예열 중' 은 **유예이지 면죄가 아니다**(#41). 콜드 비용의 대부분이 **캐시 없는
    헬퍼**였다(하루 한 번 바뀌는 값을 갱신마다 다시 받음 — #61·#119. 반쪽 맵은 굽지
    않는다 #280·#384). ⚠️ 누수를 **시간 단언**으로 막으려다 단독 green · 전체 red —
    #128 이 금지한 그것을 #312 를 인용한 같은 커밋에서 냈다(답은 구조다). ⚠️ 거절과
    '받아들여졌는데 빈 것' 은 다른 사실이고, 뭉치면 **가장 결정적인 증거가
    사라진다**(#82·#45·#292).

    실측 기록(2026-09-19):
    - 사용자가 붙인 일일 감사가 `❌ 급등·급락: 0행 … KRX 벌크 수집이 12초를 넘겨
      이번에는 비웠습니다 — 백그라운드가 받는 중이라 다음 갱신에 나옵니다` 였다.
      그 마지막 문장이 이 항목의 이름이 된 그 약속이었다.
    - 콜드 비용의 정체는 `bollinger_board._kis_master_rows` 였다. 공용 헬퍼라
      캐시가 없었고, 이름표(KOSPI·KOSDAQ 마스터 zip, 각 30초 상한)를 시도마다
      새로 받고 있었다. 갱신 주기가 10분이므로 산술상 하루 144번이다
      (⚠️ 이 경로엔 타이밍 계측이 없다 — 실측이 아니라 주기에서 나온
      계산이고, 그렇게 적는다: 재지 않은 것을 단정하지 않는다, #165).
    - 반쪽 맵 위험은 가상이 아니었다. 코스닥 zip 만 실패한 맵을 완전본으로
      구우면 코스닥 전 종목이 12시간 동안 접미사 없는 맨 6자리가 되어
      `/lookup/123456` 이 미국 심볼로 해석됐다.
    - 시간 단언 사고는 이랬다. 누수를 막겠다고 "마지막 쓰기 2회를 봤다" 를
      `finally` 에서 단언했더니 `-k` 로 좁힌 실행은 green 이고 전체 실행만
      red 였다. `#128` 이 그 실패모드를 적어 둔 항목인데, 그걸 인용한 커밋
      안에서 같은 형태를 다시 썼다.
    - VM 프로브 실측은 이랬다. `현행(대조군)`·`sortType 없음`·`파라미터 없음`·
      `category 없음` 넷이 HTTP 400 이었고 본문이 전부 동일했다
      (`sortType: Invalid input: expected "dividend" · dividendSortType:
      Invalid option: expected one of "rate"|"value"`). `dividend 갈래`
      (`sortType=dividend&dividendSortType=rate`)만 400 이 아니었고 0행이었다.
      옛 판정 문구는 그 다섯을 "전부 거절" 로 뭉쳐 적고 있었다.
    - 같은 실행에서 `integratedPriceInfo` 가 본체+시간외 합산임이 확정됐다
      (005930: 15,042,569 + 6,577,404 = 21,619,973, 세 종목 모두 원 단위 일치).
      거래소를 이름으로 가르는 필드는 그 응답에 없었다.

### 실수 #391

391. **기계 상세를 사용자 화면에 그대로 실었다 — 그리고 첫 fix 가 형제 가드를
    깨뜨렸다**(2026-09-19 급등·급락 부제에 zod 원문, 사용자 "굳이 대시보드에 나올
    필요가 없는 정보야"): 사유 한 줄은 **청중이 둘**이다(#45) — 운영자는 원문까지
    봐야 고칠 자리를 알고(#82·#109) 방문자는 "무엇으로 대체했고 기준일이 언제인가"면
    충분하다. 값은 그대로 두고 **화면만** 줄이되 **갈래 이름은 남긴다**(#43).
    ⚠️ 섞인 문자열은 **되가를 수 없다** — 절 구분자로 잘라내려다 원천 본문이 그
    글자를 담아 실패했고, 표본에서 지우자 그 경계로 키를 귀속시키던 #388 이
    깨졌다(실측). 잘라낼 자리는 surgery 가 아니라 **조립에서** 만든다(기계 상세를
    꼬리로, 회귀가 강제 — #119). ⚠️ 판정이 읽는 **생산부가 둘**이면 한쪽만 읽는
    가드는 실물을 못 본다(감사가 `HTTP 400 (12ms)` 를 '못 닿음' 으로, #38·#292).
    ⚠️ 못 고칠 ❌ 를 막으려던 유예가 **고칠 수 있는 것**을 가렸다(#41·#260 을 거꾸로).
    ⚠️ 회귀가 심는 임시 파일이 커밋에 실려 fresh checkout 이 빨간불 — 규율이 아니라
    `.gitignore` 로 막는다(#119·#328).

    무엇이 잘못됐나(전문):
    - 화면 문구가 `네이버 목록을 못 받아 KRX 벌크로 대체 — 원천이 HTTP 400 —
      원천: sortType: Invalid input: expected "dividend" · dividendSortType:
      Invalid option: expected one of "rate"|"value" (invalid_value,…)` 였다.
      앞 절은 사용자에게 필요한 사실이었고 뒤 절은 운영자용 원문이었다.
    - 첫 시도는 `_sanitize` 에서 표본의 `·` 를 `;` 로 바꾼 것이었다. `make test`
      가 5건을 잡았다. `naver_sector_client.allowed_values`(#388 이 만든 키
      귀속)가 바로 그 `·` 를 필드 경계로 쓰고 있었다. 그대로 배포했으면 #388 이
      고친 버그가 되살아났다.
    - 두 클라이언트가 조립 순서까지 갈려 있었다. 급등·급락은 사람 문장 뒤에,
      거래량 상위는 맨 앞에 기계 상세를 두고 있었다. `naver_diag.compose_reason`
      한 곳으로 모았고 `machine_detail_at` 으로 꼬리 계약을 쟀다.
    - 갈래까지 지우면 #82 를 거스르므로 `screen_gloss` 를 두었다. 400·422 는
      "원천이 우리 요청을 거절했습니다", 403 은 "원천이 우리 접근을 막았습니다",
      429 는 "원천 요청 한도를 넘었습니다" 로 갈래만 남겼다.

    독립 리뷰가 배포 전에 잡은 것(Blocking 1 · High 5 · Medium 8 · Low 6):
    - B1. `tests/_dupscan_probe_tmp.py` 가 커밋에 실려 있었다. 그 파일은
      `TestShadowedTopLevelDefs20260906` 이 실행 중에만 심고 `finally` 에서
      지우던 프로브였고, 그 창에 `git add` 가 겹쳤다. 리뷰가 fresh checkout 으로
      재 보니 첫 `make test` 가 중복 정의로 빨간불이었다. 그 실행이 tracked
      파일을 지워 워킹트리가 더러워졌다. `.gitignore` 에 `tests/_*_tmp.py` 를
      넣어 스테이징 자체를 막았다.
    - H1. 감사의 `endpoint_verdict` 는 `naver_diag.status_from` 으로 갈래를
      갈랐는데 그 정규식이 `원천이 HTTP (\d{3})` 였다. 감사가 받던 detail 은
      `source_health._naver_domestic` 이 만든 `HTTP 400 (12ms)` 라 한 번도 안
      읽혔다. 실물 400 이 도달 실패로 찍혔고 운영자가 갈 곳이 반대가 됐다.
      그리고 ⏸ 갈래는 그
      센티널을 아무도 만들지 않아 도달 불가였다. `_endpoint` 가 `naver_paused()`
      를 먼저 보게 고쳐 발화 경로를 줬고, 일시정지 중 원천을 두드리던 것도 같이
      멈췄다.
    - H2. SWR 경로가 `_kick()` 뒤에 `attempt_record()` 를 읽어 킥이 띄운 스레드의
      `started` 덮어쓰기와 경합했다. 리뷰 실측 20회 중 5회에서 직전 실패 사유가
      "받는 중" 에 가려졌다. 콜드 경로는 이미 제출 전에 읽고 있었다.
    - H3. 만료 경로가 `old["memo"]` 를 버렸다. 신선 경로는 그대로 내보내던
      경고였다. KIS 마스터가 0종목이라 티커가 맨 6자리가 된 배치가 경고 없이
      최대 `max_back` 일 재서빙됐다.
    - H4. 감사의 예열 유예가 자기 실행이 띄운 예열에도 적용됐다. `_WARM_MAX` 가
      정의상 발화할 수 없었다. 두 보드가 영구히 비어도 경고 표시와 rc=0 이라
      `audit_sweep.report_text` 가 빈 문자열을 냈고 일일 결산이 침묵했다.
      `_warming(since)` 로 이 실행 시작 이후에 시작된 것은 면죄에서 뺐다.
    - H5. `_kick` 의 `_bg` 를 어떤 테스트도 실행하지 않았다. 전부 `Thread` 스텁의
      `start()` 가 아무것도 안 했다. 그래서 `_guarded` 를 아예 안 부르는 변형,
      `write=False` 로 부르는 변형, 락을 `_guarded` 전에 푸는 변형이 전부
      통과했다.
    - M1. `stale_note` 의 상대 시각이 페이지 세션 캐시에 그대로 구워졌다. 몇
      시간 뒤에도 `11분 전에 받았습니다` 라고 적었다. 절대 시각으로 바꿨다.
    - M2. 성공 기록의 캐시 쓰기 진단과 시도 행 수가 행이 실린 화면에 붙었다.
    - M3. `_guarded` 의 `try` 가 호출까지 감쌌다. `fn` 안의 지연 import 가
      ImportError 를 내면 `_run_attempt` 를 한 번 더 돌려 바깥 원천을 두 번
      쳤다. 리뷰 실측이 `fetch attempts: 2` 였다. 독스트링은 그걸 막는다고
      적고 있었다.
    - M4. 킥에 백오프가 없어 죽은 KRX/KIS 를 렌더 주기로 계속 두드렸다.
      `retry_wait` 로 1분에서 1시간까지 지수 백오프를 걸었다.
    - M5~M8. 판정 분기 순서, `_volume` 의 warming 배선, `_stale_ok` 네 가드,
      이름표 캐시 읽기 쪽의 빈 맵 거부, 예외 경로의 기록, singleflight 키의
      `max_back` 이 전부 무가드였다.
    - L1·L2. 프로브에 도달 불가 분기가 있었다. 일반 분기가 `served_empty` 를
      버려 거절당하지 않은 모양이 있다는 사실을 빼먹었다.
    - L5·L6. "콜드 비용의 대부분" 과 "하루 144번" 은 계측이 아니라 주기에서 나온
      계산이었다. 예산 테스트에 벽시계 단언 하나가 남아 있었는데 독스트링은
      시간 단언을 구조로 바꿨다고만 적고 있었다.

    남은 미측정(#274) — 이 라운드에서 안 잰 축을 적어 둔다:
    - `screen_gloss` 가 모르는 모양에서는 원문을 그대로 냈다. 폴백이 없는 보드에
      새 갈래가 생기면 그때 화면에 원문이 실릴 수 있었다. 침묵보다 낫다고
      판단했고, 그런 갈래를 실제로 본 적은 없었다.
    - `_kick` 의 `stuck` 판정은 기록이 시작과 미완료를 말할 때만 섰다. 기록 없이
      락만 잡혀 있던 경우는 관측한 사실만 적게 뒀다.

### 실수 #392

392. **대조군이 시험 대상과 같은 값이었다 — "이미 동작을 증명한 호출" 이라 적은
    것이 그날 죽은 바로 그 값이다**(2026-09-19 "신고가 신저가는 네이버에서
    가져오는데 왜 이 두개는 못가져오는거야?"): 대조군이 시험 대상과 같으면
    '못 닿는다' 와 '이 값만 거절당한다' 가 같은 화면이 된다(#143·#91b) — 그래서
    프로브가 "새 주소를 찾아라" 를 냈는데 같은 주소가 52주 값으로는 행을 주고
    있었다(#292·#165).
    ⚠️ 두 설명이 같은 관측을 내면 단정 말고 **가르는 관측**을 만들 것(#255·#12).
    ⚠️ 반증이 **레포 안에 이미 있었다** — 있는 반증을 한 화면에 안 놓은
    것이다(#51·#150).
    ⚠️ 대조군 값은 **제품이 실제로 보내는 것**에서 파생시킬 것 — 리터럴을 두 군데
    적으면 갈린다(#38). 회귀가 수집기를 태워 URL 로 잰다(#20·#66).
    ⚠️ "행이 온다" 와 "이 두 보드가 **쓸 수 있다**" 는 다른 사실이다(#34). 대상이
    거절당한 실행에 rc=0 을 내면 "이상 없음" 으로 읽힌다(#54·#41).
    ⚠️ 같은 병이 **무인 경로**에 남아 있었다 — 일일 감사가 부르는 도달성 점검
    (`source_health._naver_domestic`)이 여전히 죽은 값을 찔렀다(#260). 그리고
    대상 사유를 `bool` 로 접으면 타임아웃·429·일시정지가 "이 값만 거절당한다"
    로 찍힌다 — 3-상태로 갈랐다(#82·#279).

    무엇이 잘못됐나(전문):
    - 세 보드가 같은 주소(`front-api/domestic/stock/list`)·같은 함수를 쓰고
      `sortType` 만 달랐다. /kr52 는 `high52week`, 급등·급락은 `up`, 거래량
      상위는 배운 키였다. 프로브 ① 의 대조군은 `up` 이었고, 그건 2026-09-18 에
      죽은 바로 그 값이었다.
    - 그래서 ① 이 400 을 받으면 "아래 0건은 판정 불가" 로 끝났고, ①-b 는
      "이 주소의 `sortType` 이 배당 한 갈래로 좁혀졌다 — 다음 측정은 새 주소를
      읽는 것" 을 냈다. 같은 주소가 52주 값으로 행을 주고 있다는 사실이 출력
      어디에도 없었다.
    - #389 의 추론("값마다 다른 사유가 안 오면 거절 대상은 그 값이 아니다")이
      그 대조군 위에 서 있었다. `up` 과 미끼가 같은 400 을 받는 것은 둘 다 이제
      무효값인 경우에도 똑같이 일어날 수 있는 일이었다. 두 설명이 같은 관측을
      냈는데 가르는 관측이 없었다.
    - 반증은 레포 안에 이미 있었다. `kr_volume_client.learn_fail_reason` 이
      "같은 주소가 다른 `sortType` 값으로는 행을 주므로 이 응답은 그 키의 스키마
      전체가 아니다" 라고 적어 두었다. 한 화면에 나란히 놓인 적이 없었다.
    - 고친 것은 넷이었다. ① 대조군을 `high52week`(= /kr52 가 보내는 값)로 바꾸고
      시험 대상 `up` 을 그 옆에 나란히 찍었다. `control_verdict` 순수 함수가
      갈래 넷(둘 다 행 / 대조군만 행 / 대상만 행 / 둘 다 0행)을 이름으로 말하게
      했다.
      ①-b `_SHAPES` 에 대조군 갈래를 넣어 원문이 같이 찍히게 했다. 시험 대상
      라벨 `현행(대조군)` 은 거짓이라 `현행(급등·급락)` 으로 바꿨다.
    - `shape_verdict` 는 "행이 온다" 를 "배선하면 된다" 로 옮기고 있었다. 52주·
      배당 갈래는 행을 줘도 급등·급락을 못 만드는데 둘 다 `ok` 에 담겼다.
      쓸 수 있는 갈래만 남기고, 대조군이 살아 있으면 "주소는 살아 있고 값 집합만
      좁혀졌다" 를 내게 했다.
    - 배선도 갈려 있었다. 대조군이 살아 있고 시험 대상만 거절당한 실행에서
      ①-b 가 건너뛰어졌고 rc 는 0 이었다. `_SUBJECT_OK` 를 ① 에서 재서 두 자리에
      실었고, 실행마다 초기화하게 했다.
    - 대조군 값이 제품과 갈리지 않게 회귀가 `fetch_kr_highlow` 를 태워 URL 에
      `sortType=high52week` 가 실리는지 보게 했다. 뮤테이션 9종이 전부 잡혔다.

    독립 리뷰가 배포 전에 잡은 것(5건 · 전부 실행으로 재현):
    - F1. `_section_control` 이 시험 대상의 사유(`sw`)를 재 놓고 버렸다.
      `control_verdict` 가 `bool` 만 받아서 타임아웃·429·일시정지가 "급등·급락이
      쓰는 값만 거절당합니다" 로 찍혔고 `main()` 이 rc=1 을 냈다. 독스트링이
      말한 `_SUBJECT_OK = None`("못 쟀다") 상태는 도달 불가였다. 형제
      `kr_boards_audit.endpoint_verdict` 는 이미 `REQUEST_SHAPE_4XX`·`PAUSED`
      로 갈래를 갈랐는데 프로브만 안 그랬다. `subject_state` 로 3-상태를
      만들었고, 일시정지면 ①-b 를 건너뛰고 rc 도 0 으로 뒀다.
    - F2. 무인 일일 감사가 부르는 `source_health._naver_domestic` 이 여전히
      `?sortType=up` 을 찔렀다. 프로브의 옛 대조군과 같은 결함이 사람이 안 보는
      경로에 남아 있었고, 매일 "주소가 거절한다" 를 내면서 같은 주소가 52주
      값으로 행을 주고 있었다. 정렬 키를 `naver_ranking_client` 단일 출처
      (`KR_HIGHLOW_SORT` 등)로 올리고 점검이 그걸 쓰게 했다. detail 에 어느
      값으로 쟀는지도 적었다.
    - F3. `usable` 이 2항목 denylist 라 `sortType 없음`·`파라미터 없음` 이
      "그 모양으로 배선하면 됩니다" 로 샜다. 정렬을 안 지정한 응답의 순서는
      우리가 잰 적이 없었다. `usable_labels()` 가 `_SHAPES` 에서
      파생하게 했고, 순서 미측정 갈래는 그렇게 말하게 했다.
    - F4. 대조군 생존 갈래가 사유도 행 수도 안 읽고 "나머지는 거절당했습니다"
      를 적었다. 타임아웃난 대상과 5행을 준 `dividend 갈래` 가 있으면 두 절이
      모두 거짓이었다. 거절·행·그 밖을 갈래로 나눠 사실만 적게 했다.
    - F5. 내 테스트가 `v.split("거절:")[1].split(")")[0]` 로 잘라 `52주
      갈래(대조군)` 안의 `)` 에서 끊겼다. 그래서 #292 가드가 `' 52주 갈래(대조군'`
      만 보고 있었고 `rejected=[전 라벨]` 뮤테이션도 통과했다. 거절 목록 전체를
      값으로 못박았다. 뮤테이션 11종이 전부 잡혔다.


### 실수 #394

**갱신자가 없는 곳의 TTL 은 신선도 가드가 아니라 삭제 타이머다**(2026-09-19
장전·장후 보드가 토요일에 빈칸): 저장분을 `ttl=86400` 으로 읽는데 이 보드는
**창 밖에서 재스캔을 아예 안 한다** — mtime 이 마지막 창에 얼어붙어 24h 뒤
죽고 **주말·연휴가 정의상 빈칸**이 된다(실측: 금 19:52 집계 → 토 19:52 폐기).
형제(movers·jp_stop)는 창 밖에서도 kick 해 같은 TTL 인데 구멍이 없다 —
범인은 **"재스캔 안 함 + TTL" 조합**이다. TTL 을 붙일 땐 "이 캐시를 **누가**
다시 쓰나 · 그가 안 도는 구간이 있나"를 먼저 물을 것(#171·#21b·#348·#384).
낡은 값이 빈 값보다 낫고 낡았다는 사실은 **보이는 줄**이 말한다(#41·#43).
⚠️ 라벨이 없던 이유는 **판정 함수 이름**이다 — `_prepost_fresh` 가 창 밖을
True 로 돌려줬는데 그 뜻은 "재스캔 불필요"지 "최신"이 아니다(#34).
⚠️ 나이를 **먼저 재고** 내용을 읽을 것 — 반대면 낡은 내용에 새 나이가 붙는다(#160).
⚠️ 문구는 두 화면이 단일 출처(#38·#147)이고 **우리 스냅샷에 대한 주장**이다(#375).
⚠️ 저장분이 **아예 없는 것**과 낡은 것은 다른 사실이다(#82).
⚠️ 같은 파일의 **상태 캐시**도 같은 조합이었다 — 첫 fix 가 그걸 놓쳐 실패
사실이 24h 뒤 사라지고 '다음 창에서 자동 갱신'이 지킬 수 없는 약속이 됐다(#380).


사용자는 토요일 22:5x KST 에 `/krprepost` 를 열고 "장전장후 시간대가 아니라면
가장 최종데이터를 그대로 남겨줘. 무슨 말인지 이해하지? 한국장전장후야. 지금
한국시간으로 토요일이니 금요일 장후데이터가 되겠지 현재기준으로는." 이라고
적었다. 화면에는 표 대신 `데이터가 없습니다` 안내와 창 시각만 떠 있었다.

원인을 코드로 읽어 단정하지 않고 먼저 재현했다. 금요일 19:52 집계 스냅샷을
27시간 전 mtime 으로 놓고 창 밖에서 `fetch_kr_prepost_movers()` 를 부르니
`up=0 down=0 ts=''` 였다. `_cached(_KR_PREPOST_CACHE, ttl=86400)` 이 금
19:52 + 24h = 토 19:52 에 그 파일을 만료시켰고, 사용자는 그 3시간 뒤에 본
것이었다.

`ttl=86400` 은 이 레포에 20곳 넘게 있는데 처음엔 그중 **둘**(저장분 리더)만
이렇게 됐다고 적었다. 배포 전 독립 리뷰가 **같은 파일의 상태 리더 둘**
(`prepost_status`·`kr_prepost_status`)도 같은 조합임을 짚었다 — 상태 도장은
`_compute_*` 안에서만 쓰였고 그건 창 안에서만 돌았다. 그래서 24h 뒤엔 실패
도장까지 사라져 파이프라인이 깨진 채 보드가 건강해 보였고, 이 커밋이 새로
붙인 "다음 창에서 자동 갱신됩니다"가 **지킬 수 없는 약속**이 됐다(#380).
넷 다 `_STORED_FOREVER` 로 고쳤다. 대신 `running` 도장은 나이로 만료시키게
했다(`scan_state`, 문턱은 재발동 백오프와 같은 값) — 스캔이 중간에 죽으면 그
도장이 영원히 '진행 중'이라고 말하는 문제를 리뷰가 같이 짚었다(#25). 형제
`finviz_client.fetch_us_movers` 와 `jp_stop.fetch_jp_stop` 은 창 밖에서도
`_kick*` 을 걸어 mtime 이 계속 갱신됐고, prepost 둘만 `if not in_win: pass`
로 재스캔을 아예 멈춰 mtime 이 마지막 창에 얼어붙었다. 그래서 같은 TTL 값이
한쪽에서만 삭제 타이머가 됐다. `ttl=86400` 전수를 손대는 것은 요청 범위 밖이라
건드리지 않았다.

고친 것을 순서대로 적었다.

- `_STORED_FOREVER = float("inf")` 를 두고 두 fetcher 가 저장분을 그 값으로
  읽게 했다. 회귀가 `_cached(ttl=)` 이 그 상수인지를 AST 로 보고, 연휴를
  흉내낸 7일 된 스냅샷도 서빙되는지를 값으로 쟀다.
- `_prepost_fresh`·`_kr_prepost_fresh` 를 `_prepost_live`·`_kr_prepost_live`
  로 갈랐다. 옛 함수는 창 밖을 True 로 돌려줬는데 그 True 의 뜻은 "재스캔할
  필요 없음" 이었지 "최신" 이 아니었고, 이름이 '신선' 이라 화면에서 최신으로
  읽혀 저장분에 아무 라벨도 안 붙었다. 새 함수는 창 안이면서 TTL 안일 때만
  True 였고, 재스캔 여부는 호출부가 `in_win` 으로 따로 봤다. 나이를 못
  재면(`age_sec is None`) 라이브라고 주장하지 않게 했다.
- payload 에 `stale`·`stale_min`·`in_window` 를 실었다. 키 이름은 형제
  위젯(`marketcap_client`·`naver_sector_client`·대시보드 TW 업종)이 쓰던 것을
  그대로 가져왔다.
- `prepost_client.stored_note(data, window)` 를 두어 US·KR 두 화면이 같은
  문구를 쓰게 했다. 나이 라벨은 `naver_diag.stale_label` 을 그대로 썼고,
  `in_window` 로 사유를 갈랐으며, 창 안일 때는 '다음 창' 꼬리를 안 붙였다.
  ⚠️ 대체됨 2026-09-20 — 사용자 요청으로 평상시 줄이 사라지고 실패 갈래만
  남았고 '왜 저장분인가' 사유 문구도 같이 사라졌다. 현행 동작은 아래
  §실수 #395 에 적었다.
- '데이터가 없습니다' 화면에 저장분조차 없다는 사실을 적었다.

순서 함정이 하나 있었다. `cache_age_sec` 를 `_cached` 뒤에 부르면 읽는 사이
백그라운드 재집계가 끼었을 때 낡은 내용에 새 나이가 붙어 라이브로 기울 수
있었고, 그건 고치려던 그 무라벨 상태였다. 나이를 먼저 재도록 순서를 바꾸자
반대로 저장분 쪽으로 안전하게 틀렸다.

화면에는 `💾 저장분 — 2026-09-18 19:52 집계 · 27시간 전 · 지금은 장전·장후
창 밖이라 마지막 집계를 그대로 보여줍니다` 와 다음 창 시각을 적은 꼬리가
한 줄로 실렸다. ⚠️ 대체됨 2026-09-20 — 그 줄이 부제와 같은 말을 한다는 사용자
지적으로 평상시엔 안 나가게 좁혀졌고, 현행 출력은 아래 §실수 #395 에 적었다.

검증은 재현 테스트를 먼저 쓰고 고치는 순서로 했다. 새 회귀 14건이 green
이었고 옛 코드에서 13건이 빨간불이었다. 뮤테이션 10종(만료 24h 되살리기 ·
stale 상수화 · stale_min 을 늘 None · 두 렌더의 저장분 줄 제거 · '다음 창'
꼬리 무조건 · 나이 미측정을 라이브로 · 꼬리 반환에서 라벨 떼기 · '저장분도
없습니다' 제거 · 창밖 사유 제거)이 전부 잡혔고, 베이스라인 green 확인과 치환
1건 단언과 md5 복원 확인을 같이 했다.

화면 단언에서 한 번 헛짚었다. 페이지 전체에서 `저장분` 을 찾으면 shell 의
`live_refresh` JS 주석(#360a 가 남긴 것)이 대신 만족시켰다. 단언을
`<div class="sm-note">` 칸 하나만 잘라서 보도록 고쳤다.

못 보는 축을 두 가지 적었다. 스냅샷이 몇 세션 뒤처졌는지는 재지 않았는데,
달력 기반 세션 격차 판정이 장전 창에서 상시 경고가 될 위험이 있어 넣지
않았다. 그리고 `ttl=86400` 을 쓰는 다른 20여 곳은 창 밖에서도 kick 하므로
같은 구멍이 없다고 보고 그대로 두었다.

### 실수 #395

**부제가 이미 말하는 사실을 본문에 또 적으면 정보가 아니라 소음이다**
(2026-09-20 장전·장후 저장분 줄, 사용자 "이 코멘트는 대시보드에 없어도 될것
같아. 당연한거니까"): #394 를 고치며 부제에 `💾 저장분(28시간 전) · 실시간
아님 · <창> · <기준시각>` 을 넣고 **본문에 같은 사실을 한 줄 더** 적었다 —
'왜 저장분인가'(창 밖이라)도 부제의 창 표기가 답한다. 줄을 더할 땐 **어느
칸이 이 사실을 이미 말하나**를 먼저 셀 것(#45 같은 모집단 · #25).
⚠️ 그렇다고 지우면 **같이 사라지는 갈래**가 있었다 — 실패 사실은 부제의
어느 칸도 말하지 않고, US 는 실패 배너가 `building` 분기 안이라 표가
있으면 도달 불가다. 지우지 말고 **좁힐 것**(#222).
⚠️ 음수 나이 가드는 **동어반복**이었다 — 원인은 빈 문자열이 아니라
`split("집계")[-1]` 이 꼬리의 '마지막 **집계** 시도' 를 집은 것이라 **base
에서도** 통과했다. 잰 자리가 틀린 가드는 줄을 되살려도 안 고쳐진다 —
**절을 잘라서** 볼 것(#55·#75·#91b·#291).
⚠️ 같은 화면 두 줄 위 배너가 `자동 재집계됩니다` 였다 — 회귀가 `자동 갱신`
을 금지해 놓고 **형제 표면의 동의어**는 아무도 안 봤다(#38·#147).
⚠️ 재작성이 축을 떨어뜨렸다 — 부정 단언으로 바꾸며 KR 의 창 인자·렌더 배선
둘이 무가드가 됐다(base 에선 잡혔다). #222 는 '다시 쓴다' 이지 '줄인다' 가
아니다(#378·#20·#313).


사용자가 `/krprepost` 캡처와 함께 그 한 줄을 그대로 붙여 놓고 "이 코멘트는
대시보드에 없어도 될것 같아. 당연한거니까" 라고 적었다. 그 줄은 전날 #394 가
붙인 것이었고, 같은 화면 부제가 이미 `💾 저장분(28시간 전) · 실시간 아님 ·
<창> · 2026-09-18 19:54 기준` 을 말하고 있었다.

통째로 지우지 않고 좁혔다. `stored_note` 는 `scan_state(status) != "failed"`
면 빈 문자열을 돌려주고, 실패 갈래에서만 한 줄을 냈다. 그 갈래를 남긴 근거는
둘이었다. 실패 사실은 부제의 어느 칸도 말하지 않았고, US 는 실패 배너가
`building` 분기 안이라 표가 있으면 도달할 수 없었다. 독립 리뷰가 그 두 번째
근거를 코드 추적과 렌더 둘로 확인해 줬다 — 행이 있으면 `최근 산출 실패` 가
html 에 없었고 행이 없을 때만 떴다.

꼬리 문구도 같이 바꿨다. `다음 창(…)에서 자동 갱신됩니다` 는 실패한 마당에
지킬 수 없는 약속이라(#380) `다시 시도합니다` 로 적었고, 시점은 창 판정이
있을 때만 적었다. 창 안이면 '이번 창', 창 밖이면 '다음 창(<창>)', 판정을 못
받았으면 아무 시점도 적지 않았다.

배포 전 독립 리뷰가 Blocking 0 · High 2 · Medium 4 · Low 4 를 냈고 전부
반영했다.

음수 나이 가드가 첫 High 였다. 내가 CLAUDE.md 와 docs 에 "빈 문자열이라
동어반복이 됐고 실패 갈래로 태워 되살렸다" 고 적었는데, 리뷰가 `-120분 전`
을 실제로 넣어 보니 고친 뒤에도 통과했다. 원인은 빈 문자열이 아니라
`note.split("집계")[-1]` 이었다 — 새 꼬리 `마지막 **집계** 시도가` 가 "집계"
를 품어 `[-1]` 이 실패 문장이 되고 나이는 `[1]` 에 살았다. 옛 사유 문구
`마지막 **집계**를 그대로` 도 같은 글자를 품었으므로 base 에서도 통과했고,
줄을 비우지 않게 만든 것으로는 고쳐지지 않았다. 나이 절 하나를
(`split(" 집계 · ")[1].split(".")[0]`) 잘라 보도록 고쳤다.

둘째 High 는 재작성이 떨어뜨린 축이었다. 옛 KR 페이지 테스트의
`assert "다음 창(" in hit[0]` 은 주석이 적은 그대로 창 인자 배선을 화면에서
재고 있었는데, 본문 줄이 없다는 부정 단언으로 다시 쓰면서 그 축이 사라졌다.
리뷰가 뮤테이션 둘을 돌려 확인했다 — 호출부에서 `win` 인자를 떼도, 호출은
남기고 `if False:` 로 결과를 버려도 46건이 전부 green 이었고 base 에서는 둘
다 잡혔다. KR 실패+행 경로를 렌더하는 E2E 를 새로 두어 두 축을 한꺼번에
복구했고, 형제 US 테스트에도 같은 단언을 더했다.

Medium 에서 가장 아팠던 것은 배너였다. KR 은 실패 배너가 `not up and not
down` 밖이라 행이 있어도 떴고, 그 배너가 `{창} 창에서 자동 재집계됩니다` 라고
적고 있었다. 새 테스트가 저장분 줄에서는 `자동 갱신` 을 두 곳에서 막아
놓고, 같은 화면 두 줄 위의 동의어는 아무도 안 봤다. 두 줄이 같은
함수에서 나오게 `retry_clause(in_window, window)` 를 단일 출처로 두었다.

낮은 등급에서는 낡은 독스트링 한 줄과 `window` 가 빈 경우의 빈 괄호, 그리고
US 부제의 기준시각 무가드를 고쳤다. `docs/tests.md` 의 '못 보는 축' 문단은
보상 신호를 "부제 나이 라벨과 `failed` 배너 둘" 이라 적고 있었는데 US 에서는
배너가 도달 불가라 절반이 거짓이었고, 같은 파일 위쪽 문단과 서로 다른 말을
하고 있었다. 그것도 같이 고쳤다.

고치지 않고 적어 둔 축이 하나 있다. 창 안이면서 저장분이고 스캔이 정상인
상태는 이제 아무 줄도 내지 않았다. KR 은 킥이 `running` 도장을 찍으면 다음
로드에서 파란 배너가 뜨지만 US 는 그 배너도 `building` 안이라 행이 있으면
뜨지 않았다. 사용자가 없애 달라고 한 것이 정확히 그 '당연한' 줄이었으므로
줄을 되살리지 않았고, 대신 그 사각을 `docs/tests.md` 에 적었다.

뮤테이션은 11종을 돌려 전부 잡혔다. 베이스라인 green 확인과 치환 건수 단언과
md5 복원 확인을 같이 했고, 치환이 1건이 아닌 앵커 둘은 건너뛴 사실을 출력이
스스로 말하게 한 뒤 앵커를 고쳐 다시 돌렸다.

### 실수 #396

**감사 ② 가 '정의 차이'를 매일 ❌ 로 찍고 있었다 — 8건이 전부 한 종목**
(2026-09-21 일일 감사 `739de2ec2a`, 181710.KS): 재료를 맞추니 8기간(분기
5·연간 3) 전부 `|yf CAPEX| = DART 유형 + 무형` 이고 `DART FCF − yf FCF =
무형` 이었다 — 값이 아니라 **CAPEX 정의**가 다르다. 우리가 고칠 것이
없고(산식은 사용자가 정한 FnGuide 기준 #215 · 원천은 무형을 따로 안 준다)
화면은 이미 두 탭이 서로를 가리키며 밝히는데(#102·#186), 못 고칠 ❌ 가
매일 오면 진짜 ❌ 를 가린다(#260) → **항등식이 설 때만** 설명하고 ✅,
잔차만 ❌(#106·#182). 차이의 크기는 그대로 찍는다(#41).
⚠️ 그 실측이 #215 가 덧붙인 "yfinance 와도 정의가 같아 시장 간 기준이
하나로 맞는다"를 **반증**했다 — 재지 않은 단언이었다(#165·#55). 같은
커밋에서 두 주석을 고쳤다. 가드는 문구 denylist 가 아니라 **측정**이다
(표현만 바꾸면 통과한다, #373·#274).
⚠️ 이 진단은 #356 이 ❌ 줄에 심은 **재료**가 없었으면 또 추측이었다.
⚠️ ❌ 를 내리는 근거가 "화면이 이미 말한다" 였는데 화면은 '다를 수 있다'
까지만 말했다 — 같은 커밋에서 두 탭이 사유를 대게 했다(#202·#209·#38).
⚠️ 분기·연간은 잘라서 잴 것 — 분기 재료를 지운 뮤테이션을 **연간 줄이
대신 만족**시켜(#75·#55·#91b) 통과했다.
→ REFERENCE §실수 #396


사용자가 2026-09-21 09:36 KST 일일 감사를 그대로 붙여 놓았다. 결함 8건에
경고 24건이었고 여덟 줄이 전부 `💵 FCF 정확도(3화면 교차) [② 교차출처(DART
↔ yfinance)] [181710.KS]` 였다. 차이는 25.2Q 8.09% · 25.3Q 2.93% · 25.4Q
4.07% · 26.1Q 2.03% · 26.2Q 8.98% · FY2025 8.88% · FY2024 94.07% · FY2023
17.92% 였다.

먼저 지문을 쟀다. 현재 트리의 `audit_fingerprint()` 가 `739de2ec2a` 로
헤더와 같았으므로 그 감사는 배포된 코드가 낸 것이었다. 직전 건
`613fd31136` 처럼 옛 코드가 아니었다. 같은 목록에 KR 네이버 보드 줄이 한
줄도 없었던 것도 그 실행이 확인해 준 사실이었다.

여덟 줄이 달고 있던 재료를 산수로 맞췄다. `DART FCF − yfinance FCF` 가
여덟 기간 모두 무형자산취득과 같았고 `|yfinance CAPEX|` 가 여덟 기간 모두
`DART 유형 + 무형` 과 같았다. 표시 반올림 0.1억 안쪽이었다. FY2024 의
94.07% 는 분모가 작아서 생긴 것이었다.

그 재료는 #356 이 심은 것이었다. 2026-09-14 에 `audit_sweep._findings` 가
결함 한 줄만 결산에 올린다는 이유로 재료를 같은 줄로 옮겨 놓지 않았다면
이번에도 종목별 프로브를 돌려야 했다.

원인이 정의 차이로 확정되자 갈래가 셋 나왔다. 제품 산식은 그대로 두었다.
`CAPEX = 유형자산의증가` 는 사용자가 신뢰 기준으로 제시한 FnGuide 산식이었고
#215 가 LG이노텍 세 해 실측으로 확정해 두었다. yfinance 쪽도 바꿀 수 없었다.
원천이 무형을 따로 주지 않으므로 묶인 CAPEX 를 풀 방법이 없었고, DART 무형을
빼서 맞추면 한 차트가 두 원천을 섞는 것이 됐다.

화면은 이미 정직했다. 밸류에이션 탭 각주가 `FCF = 영업활동현금흐름 −
CAPEX(유형자산 취득) · 출처 DART 현금흐름표 · FnGuide 산식과 같은 기준 —
무형자산 취득과 처분액은 넣지 않습니다` 를 적고 두 탭이 서로를 가리키고
있었다.

남은 것은 감사였다. `_mark` 에 설명 축을 더해 판정 글자를 한 곳에서 만들게
했고, `bundled_intangible` 과 `basis_note` 를 순수 함수로 두어 분기·연간 두
호출부가 같은 판정을 쓰게 했다. 설명은 조건부로 두었다. `|yf CAPEX|` 가
`유형 + 무형` 과 1% 안에서 같을 때만 그 무형 크기를 돌려주게 했고,
`DART FCF − 무형` 이 yfinance FCF 와 1% 안에서 같을 때만 판정이 뒤집히게
했다. 두 조건 중 하나라도 깨지면 종전대로 결함으로 남게 했다.

이 대조가 tautology 가 아닌 근거는 입력이 갈린다는 점이었다. 좌변은 DART
OCF·유형·무형이었고 우변은 yfinance OCF·CAPEX였다. 잔차가 0 이 되려면 두
OCF 가 같고 동시에 yfinance CAPEX 의 구성이 유형+무형이어야 했으므로, 옛
판보다 오히려 CAPEX 구성까지 더 재게 됐다.

통과 줄은 크기를 지운 자리가 아니었다. `차이 17.92% — CAPEX 정의차(yf CAPEX
3,512.4억 = 유형 3,179.2 + 무형 333.2) · 보정 후 0.00%` 처럼 원차와 구성과
잔차를 같이 적어 눈으로 검산되게 했다.

그 실측이 주석 둘을 반증했다. `bot/fcf.py` 와 `bot/dart_quarterly.py` 가
#215 에서 "이 정의는 yfinance `Capital Expenditure` 와도 같아 시장 간 정의가
하나로 맞는다" 고 덧붙여 놓았는데 재지 않은 단언이었다. 두 곳을 같은
커밋에서 고쳤다. 그 주장이 다시 적히는 것을 문구로 막지는 않았다. denylist
는 표현만 바꾸면 통과하므로 가드를 측정 쪽에 두었다. 새 E2E 가 두 정의를
실제로 가를 때만 통과하게 했다.

뮤테이션은 두 라운드로 16종을 돌렸다. 첫 라운드 10종에서 아홉이 바로 잡혔고 하나가 생존했다. 분기 호출부의
재료를 지우는 변형이었는데, 단언이 페이지 전체에서 `↳ DART OCF` 를 찾아
연간 줄이 대신 만족시켰다. 분기와 연간을 잘라서 각각 보도록 고치자 잡혔다.
픽스처도 두 번 손봤다. DART 쪽 `FCF` 를 손으로 넣었더니 `_attach_fcf` 가
산식을 정하는 실제 경로를 안 탔다. 무형 0 케이스는 CAPEX 를 유형+무형으로
둔 탓에 항등식 검사가 먼저 걸러 `not intan` 가드를 지워도 통과했다.

배포 전 독립 리뷰가 11건을 냈고 전부 반영했다.

가장 아픈 것은 사유 문구였다. 왼쪽 숫자를 `유형 + 무형` 으로 적고 라벨만
`yf CAPEX` 라 붙여 놓아, 등식이 구조상 항상 맞았고 원천 값과 다른 수가
화면에 실렸다. 리뷰가 504.9억을 500.0억으로 적는 경우를 제품 코드로
재현했다. 원천이 준 CAPEX 를 그대로 찍게 고쳤고, 항등식 허용치 안에서
일부러 어긋난 픽스처로 그 차이가 보이게 했다.

둘째는 근거였다. 결함 판정을 통과로 내리는 근거로 "화면이 이미 말한다" 를
들었는데, 두 각주는 '값이 다를 수 있습니다' 까지만 적고 원인도 크기도
말하지 않았다. 측정한 사실이 전체 로그에만 남게 되므로 같은 커밋에서
`bot.fcf.CAPEX_BASIS_NOTE` 단일 출처를 두어 두 탭이 사유를 적게 했다.

셋째는 반증된 문장이 남은 자리였다. `tests/test_regression.py` 의 계약
독스트링과 `bot/dart_client.py` 의 canonical 맵 주석과 CLAUDE.md·REFERENCE
의 #215 본문이 그대로였다. 네 곳을 같은 커밋에서 고쳤고 #215 에는 대체
표시를 달았다.

나머지는 픽스처와 죽은 코드였다. 잔차 문턱은 19.6% 짜리 픽스처 하나뿐이라
1%를 5%로 풀어도 전 슈트가 green 이었고, 2.27% 짜리를 더해 그 사이를
메웠다. 원천이 `Free Cash Flow` 를 같이 준다는 사실이 픽스처에 없어 제품이
실제로 가는 길을 안 탔다. `not both` 가드와 `or 0.0` 폴백은 도달 불가라
지웠다. 무형 계정명만 리터럴이라 `bot.fcf._DART_INTANGIBLE_KEY` 로 올렸다.
...
**경계를 어구 열거로 되짚어 절반만 잘랐다 — 화면에 값 없는 라벨이 떴다**
(2026-09-21 거래량 상위 배너, "이것도 코멘트 확인해줘"): `public_reason` 이
마커 3개 열거로 경계를 **다시 추론**해 목록 밖 상세는 머리에 남고 목록 안
어구가 `원문: {…}` **안**에서 걸려 `원문: ·` 만 남았다(잘린 건 **63자뿐**).
`compose_reason` 은 경계를 **알면서 버리고** 있었다(#123 계열·#24).
**아는 쪽이 표식을 찍고 자르는 쪽은 읽기만 할 것**(#86).
⚠️ **무조건 찍으면 줄이는 것이 지우는 것이 된다** — 그 슬롯엔 덤프 없는 사람
문장도 온다(#395·#43). ⚠️ 상세가 **사람 문장 안**에 있는 생산부도 있다 — 이어
붙이면 사이의 사람 문장이 먹힌다. **이어 붙이지 말고 가를 것**(경계는 언제나
하나 · 잇는 자리가 여럿이면 **결합기 하나**로, #38).
⚠️⚠️ **한 갈래만 막고 끝냈다** — 같은 채널에 쓰는 생산부를 세지 않아 '행을
찾은' 경로의 메모·예외가 그대로 샜고 그 예외엔 **URL 속 토큰**이 있었다
(§Secrets 는 사유 채널에도). **한 채널을 고칠 땐 거기 쓰는 생산부를 전부 세고
시작할 것**. ⚠️ 머리 없는 조각도 **경계를 긋는다**(머리는 호출부가 댄다). ⚠️ **공백에 기대는 탐색은
진다** — `.strip()` 한 번에 표식이 안 잡혔다. 찾을 땐 공백 없는 **핵**으로(#65).
⚠️ 표본을 자르면 **자른 사실도**(#45).
→ REFERENCE §실수 #398


사용자가 거래량 상위 보드 캡처를 붙이고 "이것도 코멘트 확인해줘" 라고
했다. 배너 한 줄에 pykrx 함수명과 예외 클래스, zod 리터럴, 우리 내부
추론("판별 유니온의 한 갈래로 보이지만 그건 재지 않았습니다"), 그리고
값이 잘려 나간 `원문: ·` 라벨이 함께 떠 있었다.

연역 대신 재현했다. 제품 생산부로 그 사유를 다시 만들어 보니 541자였고
`machine_detail_at` 이 478 을 돌려줬다. 잘린 것은 63자였다. 남은 63자가
자르기의 전부였다. 그 541 은 첫 재구성의 총 길이였고 메모를 몇 줄 묶느냐에
따라 달라졌다 — 그래서 커밋된 회귀 픽스처로 다시 쟀고 옛 판 719자에서
경계가 656, 잘린 것은 똑같이 63자였다. 재현 못 하는 총 길이를 그대로
남겼으면 다음 사람이 그걸 기준으로 삼았다.

원인은 두 함수가 같은 경계를 서로 다르게 알고 있던 것이었다.
`compose_reason` 은 사람 문장과 기계 상세를 이어 붙이는 자리라 경계를
정확히 알았지만 ` · ` 로 이어 붙이고 그 지식을 버렸다. `public_reason`
은 `_MACHINE_MARKS` 세 어구로 그 경계를 되짚었다. 그 목록에 없는
`learn_fail_reason` 의 zod 해설은 머리에 남았고, 목록에 있는
`원천이 HTTP ` 는 하필 그 해설이 꼬리에 붙인 `원문: {…}` 안에서 걸렸다.
그래서 라벨만 남고 값이 잘렸다.

두 번째 덤프는 더 깊은 자리에 있었다. `_kr_bulk_rows_uncached` 가
`KRX 벌크가 6거래일에서 행을 못 냈습니다 — 20260917: get_market_price_
change_by_ticker 실패(IndexError) …` 를 만들어 상태 기록에 저장했고,
그 문자열이 `attempt_note` 와 `stale_note` 를 타고 보드 사유의 **사람
문장 안**으로 들어왔다. 꼬리만 자르는 어떤 방식으로도 닿지 않는
위치였다.

조립부가 경계를 찍게 했다. `_DETAIL_MARK` 를 `compose_reason` 이
삽입하고 `machine_detail_at` 이 그 자리를 돌려주게 했다. 표식이 여럿이면
가장 이른 것을 택하게 했더니 사람 문장 안에 끼워 넣은 벌크 상세가
조립부의 꼬리보다 먼저 잘렸다. 표식이 없는 사유는 옛 열거로 되짚게
남겨 뒀다. `http_reason` 하나만으로 만든 사유가 그 경로였다.

표식을 경계 **자리**에 두고 뒤쪽에 남긴 것은 첫 판과 달랐다. 첫 판은
표식 뒤를 시작점으로 돌려주고 자르는 쪽이 `endswith` 로 표식을
벗겼다. 그 한 줄을 지우면 `⟪상세⟫` 가 화면에 샜다. 경계 자리를
돌려주면 표식이 정의상 뒤쪽에 남아 그 분기 자체가 없어졌다.

`_bulk_fail_reason` 을 만들어 벌크 실패 사유를 사람 문장과 덤프로
갈랐다. 표본을 3건으로 자르는 옛 동작은 그대로 뒀고 자른 건수를
덧붙였다. 머리가 6거래일이라 적는데 몸통이 3건이면 나머지를 안 본
것으로 읽힐 수 있었다.

결과는 같은 회귀 픽스처에서 722자 중 175자였다. 화면에는 무엇으로 대체했는지, 기준일이
언제인지, 갱신을 누가 받는지, 지난 시도가 언제 어떻게 실패했는지,
그리고 원천이 우리 요청을 거절했다는 갈래만 남았다. 감사와 로그와
`--why` 가 읽는 전문은 그대로 두었다.

첫 판은 표식을 무조건 찍었다. 그 상태로 세 클래스만 골라 돌리자 전부
통과했다. 전체 게이트가 급등·급락 두 건을 빨간불로 냈다. `det` 슬롯에는
`http_reason` 의 덤프만 오는 것이 아니었다. `shape_reason` 과 `parse_reason`
이 만드는 덤프 없는 짧은 사람 문장도 같은 자리로 왔고, 그것이 통째로
잘려 화면이 다시 "(잠시 후 다시 시도)" 만 적었다. `naver_diag` 의 주석이
그 사실을 이미 적어 두고 있었는데 내가 읽지 않았다.

`carries_dump` 를 두어 덤프를 품은 것에만 경계를 긋게 했다. 판정은 표식이
이미 있거나 옛 열거 어구가 들어 있는 경우였다. 덤프인 줄 아는 생산부는
`mark_detail` 로 스스로 찍게 했다. `_bulk_fail_reason` 이 그 경우였다. 그
덤프에는 열거 어구가 없어 `carries_dump` 가 못 알아봤다.

전체 게이트가 같은 두 건을 또 빨간불로 냈다. 이번엔 `-k` 로는 재현되지
않았다. 프로브를 붙여 그 자리에서 상태를 찍자 원인이 내 표식이었다. 세션
앞쪽 테스트가 `KRX_ID` 를 넣어 두어 pykrx 가 소켓 가드까지 갔고
`ConnectionError` 로 끝나 `_bulk_fail_reason` 이 표식을 품은 memo 를 만들었다.
그 memo 는 `attempt_note` 를 타고 **사람 슬롯**으로 들어왔다. 조립부가 기계
상세를 그 뒤에 이어 붙이자 사이의 사람 문장이 가장 이른 표식 뒤로 밀렸고,
급등·급락의 `엔드포인트 계약 변경 의심` 이 화면에서 사라졌다. 혼자 돌리면
자격증명이 없어 네트워크까지 가지 않았고 표식 없는 memo 가 나왔다.

조립부를 이어 붙이기에서 **가르기**로 바꿨다. 각 조각을 표식에서 잘라 머리는
머리끼리 상세는 상세끼리 모으고 경계를 하나만 긋게 했다. 앞은 전부 사람이고
뒤는 전부 상세가 됐다. 세션 순서에 기대지 않도록 그 상태를 값으로 재현하는
회귀를 같이 뒀다.

뮤테이션을 다시 돌리자 둘이 살아남았다. 조립부의 구분자를 되돌리는 것과
머리 없이 표식을 찍는 것이었다. 첫째는 배너 픽스처에 벌크 표식이 더 앞에
있어 거기서 잘렸고, `http_reason` 모양은 옛 열거가 대신 잡았다. 열거가
눈머는 자리는 그 어구가 값 한가운데 있을 때뿐이었다. 벌크 층이 없는
픽스처를 따로 두자 발화했다. 둘째는 `mark_detail` 의 독스트링이 적어 둔
규약이었고 가드가 없었다. 한 줄로 못박았다. 열두 종이 전부 잡혔다.

뮤테이션 여덟 종이 전부 잡혔다. 구분자를 옛 ` · ` 로 되돌리는 것,
표식을 무시하는 것, 가장 늦은 표식을 택하는 것, 벌크 덤프를 사람
채널에 그대로 두는 것, 자른 건수를 빼는 것, 렌더가 `public_reason`
을 안 부르는 것, 갈래 이름을 안 붙이는 것, 표식 없는 사유를 통째로
비우는 것이었다. 렌더를 태우는 테스트가 없었으면 여섯째는 살아남았다.
그 여덟은 좁히기 전의 1차 배터리였다.

같은 실행에서 잰 것이 하나 더 있다. 그 배너가 말하는 pykrx 벌크 갱신은
6거래일에서 행을 못 냈다. 보드는 19시 56분 저장분을 계속 서빙하고
있었으므로 실패한 것은 갱신이었다. 원인은 재지 못했다. 샌드박스에서는
KRX 에 닿을 수 없었다.

배포 전 독립 리뷰가 셋을 더 잡았고 둘은 이 델타가 고치려던 그 누출이
다른 갈래에 그대로 남은 것이었다. 첫째는 행을 **찾은** 경로였다. 경계를
`_bulk_fail_reason`(전 거래일 실패)에만 그어 두었는데, 더 자주 도는 것은
행을 찾고 경고만 남은 경로였고 그 메모가 `stale_note` 를 타고 배너로
나갔다. 재현하니 `get_market_cap_by_ticker 실패(TypeError: …) · 종목명
미확보 12종목` 이 화면에 그대로 떴다. 둘째는 `_run_attempt` 의 예외
사유였다. 표식도 없고 `mask_secrets` 도 안 거쳐서, 재현한 배너에
`HTTPSConnectionPool(host='data.krx.co.kr' … /getJsonData.cmd?apikey=
SECRETTOKEN…)` 이 통째로 실렸다. 형제 사유 채널은 전부 마스킹을 거치는데
이 한 곳만 빠져 있었다. 셋째는 `compose_reason` 이 사람 조각만 가르고
기계 슬롯은 통째로 이어 붙이던 것이었다.

고치면서 두 가지가 더 드러났다. `mark_detail` 은 머리가 없으면 표식을
찍지 않았는데, 머리를 호출부가 대는 조각(`_kis_names` 의 why)이 그래서
그대로 샜다. 경계를 긋는 쪽을 구조로 바꾸고 화면을 비우지 않을 책임은
`public_reason` 으로 옮겼다. 그리고 표식을 앞뒤 공백까지 포함해 찾고
있었는데 `public_reason` 이 맨 앞에서 `.strip()` 을 하는 바람에 머리 없는
조각에서 경계가 통째로 안 잡혔다. 공백 없는 핵으로 찾게 바꾸자 잡혔다.

세 갈래를 각각 회귀로 못박았다. 메모 경로는 stub `stock` 으로 `_rows_on`
을 태워 화면까지 갔고, 예외 경로는 `_run_attempt` 를 실제로 태워 기록된
사유에 비밀값이 없는 것과 화면에 덤프가 없는 것을 같이 쟀다. 결합기는
표식을 품은 메모 뒤의 `지난 시도가 …` 가 살아남는 것으로 쟀다.


### 실수 #399

399. **딥링크는 '앵커가 붙었나'가 아니라 '그 주소가 목적지에서 해석되나'까지
    재야 한다**(2026-09-22 수출입 종목 딥링크): NOAH `resolve_name_to_ticker` 가
    무접미 6자리를 US 심볼로 떨어뜨려 `/lookup/005930` 이 미국 조회였다 — href
    만 보는 렌더 테스트는 초록이었다. 목적지 리졸버를 같은 커밋에서 고치고
    회귀가 그 해석을 **값으로** 잰다(못 푼 이름은 평문, #144·#43).
    ⚠️ 맨 숫자 티커는 **보드가 시장을 선언**할 때만 질의가 된다 — CN·TW 보드
    정규식이 A주 코드 모양을 받아 `600519`→`600519.KS`(남의 회사 화면)가 된다.
    위 리졸버 fix 가 그 자리를 죽은 링크에서 **틀린 링크**로 바꿨다(#34).
    ⚠️ 접두는 상대 `../lookup/` — 절대로 적으면 토큰이 떨어져 404 다(형제
    `../media/` 와 같은 자리, #38·#359).
    ⚠️ 테스트 스텁은 **리졸버 경계**에 건다 — 네 단 사다리 중 한 단만 갈면
    운영자 `~/.trade`·`_DIRECT_CODES` 한 줄이 단언을 뒤집는다(#30·#312·#344).
    ⚠️ `"sl-link" in html` 은 **스타일시트**가 대신 만족시킨다(#300·#75) ·
    헬퍼만 재면 배선을 떼는 변형이 통과한다(#20).
    ⚠️ 내 뮤테이션 8종 '전부 잡힘' 도 **집합이 좁으면 거짓 안심**이다 — 독립
    리뷰가 52종에서 9종 생존을 실측했고 전부 이번에 만든 축이었다(#291).
    → REFERENCE §실수 #399


수출입 대시보드의 나라별 종목 보드(말레이시아·일본·중국 수출/수입·대만
월매출·한국 수출/수입)와 회사별 탭, 모달 관련종목 칩, 레퍼런스북 관련상장사
— 종목 이름이 실리는 자리를 전부 NOAH `/lookup/` 으로 걸었다. 사용자 요청은
"종목제목을 클릭하면 캡쳐된 종목화면으로 … 종목이 있는 대시보드에 종목들은
모두 적용 … 회사별도 똑같이" 였다.

질의를 만드는 함수(`trade/stock_link.lookup_query`)는 지어내지 않고 커밋된 픽스처의
티커 모양을 세어 정했다. 보드 대부분이 미국 영문 심볼(TTMI·DELL·ASX·TXN·
LITE·COHU·PENG)이었고 맨 숫자는 KR 보드의 6자리(403870)와 JP 보드의
로컬코드(6857·285A)뿐이었다. 그래서 순서를 ① 접미사가 이미 있으면 그대로
② 영문만이면 그대로(미국) ③ 6자리면 그대로(KR) ④ 영문 별칭표에 이름이
있으면 이름(`Advantest → 6857.T` 실측) ⑤ JP 보드에 한해 로컬코드 + `.T`
⑥ 아니면 링크 없음, 으로 잡았다. 국내 회사명은
`price_provider.resolve_codes(fetch=False)` 로 페이지당 한 번 코드로
바꾸게 했고 외부 호출은 0 이었다.

그 과정에서 NOAH 쪽 잠복 결함이 드러났다. `/lookup/005930` 은 `_TICKER_RE`
를 통과해 `detect_market` 으로 갔는데 접미사가 없으면 US 가 기본이라 국내
6자리가 미국 종목으로 조회되고 있었다. 링크를 붙이기만 했으면 국내 전
종목이 죽은 링크였고, href 만 보는 렌더 테스트는 그대로 초록이었다. 목적지
리졸버를 같은 커밋에서 고쳐(6자리 → `normalize_kr_ticker_suffix` 가 KRX
목록으로 KS/KQ 판정) 회귀가 `403870 → 403870.KQ` 를 값으로 쟀다.

접두는 형제(`../media/`)가 하던 대로 따라갔다. trade HTML 은 미디어를 `../media/` 로
참조하고 NOAH 프록시(`_rewrite_trade_html`)가 토큰 포함 절대로 바꾸는데,
`../lookup/` 도 같은 자리에서 같이 바꾸게 해 토큰 경로·아카이브 깊이와
무관해졌다. trade 백엔드(:8765)에 직접 붙으면 `/lookup` 이 없다는 점은 못
보는 축으로 적어 뒀다(§Stance 상 NOAH 프록시가 정식 접근 경로였다).

뮤테이션에서 배운 것이 둘이었다. 회사별 뷰의 배선을 떼는 변형
(`stockHref(name)` → `''`)이 처음엔 통과했는데, node 하네스가
`renderSection` 을 직접 불렀기 때문이었다. `buildCompaniesView` 를 통째로
태우자 잡혔다. 그리고 레퍼런스북 테스트의 `assertNotIn("sl-link", html)` 은
스타일시트(`.sl-link{…}`)가 대신 만족시켜 링크가 하나도 없어도 빨간불이었다
— #300 에서 `df-badge-sig` 로 겪은 그것을 내 테스트에서 다시 냈다. 앵커
정규식으로 바꿨다.

전체 게이트가 옛 계약 둘을 잡았다. 뮤테이션은 `-k` 로 좁혀 돌려 그 밖을
못 봤다. `_stock_quotes_for` 의 반환 dict 가 `{p, c}` 뿐이라는 단언과
`"esc(s)+stockPx(s)" in _JS` 라는 소스 문자열 단언이었다. 둘 다 지우지 않고
다시 썼다 — 전자는 남는 보장(값 · 합성키를 코드로 싣지 않는다)을 유지했고, 후자는
`renderModalCard` 본문을 잘라 "칩이 이름+시세를 낸다" 는 계약으로 재작성했다.

레퍼런스북 미매칭·DART 보강 후보 두 패널은 링크를 붙이지 않았다. 앞은
아직 그 품목의 상장사로 확정되지 않은 후보이고 칩의 주 동작이 '반영'
버튼이었으며, 뒤는 셀이 회사명을 콤마로 이은 한 문자열이라 칩으로 쪼개면
그 셀의 textContent 를 읽는 CSV 경로가 바뀌기 때문이었다.

배포 전 독립 리뷰가 뮤테이션 52종을 돌려 9종 생존을 실측했다. 내가 돌린 8종은
전부 잡혔지만 그 집합이 좁았다. 생존분은 `local=` 배선 넷, 캡션 이름과 href 의
이스케이프 둘, payload 여분 키, 도쿄 코드 폭, 그리고 재키잉이었다. 리뷰는
`esc(text)` 를 벗긴 상태에서 칩에 원시 마크업이 실리는 것을 재현해 보였다.

가장 무거운 것은 테스트가 운영자 홈 상태를 읽는 것이었다. 회사별 보드와
레퍼런스북 테스트가 `price_provider` 의 네 단 사다리 중 `_load_krx_master`
한 단만 갈아끼우고 실제 `resolve_codes` 를 불렀다. 그래서 운영자의
`~/.trade/stock_overrides.json`, durable 캐시와 `TRADE_DATA_GO_KR_KEY`,
`_DIRECT_CODES` 한 줄 추가가 각각 단언을 뒤집었다. 세 트리거를 임시 홈으로
재현해 고치기 전 세 건이 빨간불이고 고친 뒤 전부 초록인 것을 확인했다. 스텁을
리졸버 경계로 올리자 `kr_codes` 자신의 계약은 그대로 탔다.

맨 숫자 티커의 시장 축도 리뷰가 짚었다. 중국·대만 보드의 티커 정규식이
A주 코드 모양을 받는데, 맨 6자리를 무조건 KR 로 본 탓에 `600519` 가
`600519.KS` 로 갔다. 같은 커밋이 NOAH 리졸버를 고친 탓에 그 자리는 죽은
링크에서 남의 회사 화면으로 바뀌어 있었다. `jp_local` 불리언을 `local`
문자열 한 축으로 합쳐 보드가 시장을 선언하게 했고, 선언이 없는 보드의
맨 숫자는 평문이 됐다.

이스케이프 두 자리는 제품 경로에서 no-op 이었다. `lookup_href` 와 `stockHref`
가 퍼센트 인코딩만 내보내므로 `esc(href)` 를 벗겨도 아무것도 안 깨졌다.
그래서 `linked_name` 과 `slLink` 를 직접 태워 재웠다 — 둘 다 아무 href 나 받는
공개 헬퍼라 속성값 이스케이프가 그 시그니처에 실린 약속이었다. 캡션에서 온
이름 쪽은 모달 칩 픽스처에 `<b>회사</b>&` 를 넣어 링크가 붙는 가지와 안 붙는
가지를 같이 쟀다.

작은 것 셋을 같이 손봤다. `bot/market.py` 머리말이 이 레포가 맨 6자리를 KR 로
안 본다고 적고 있어 `/lookup` 계층이 달라졌다는 문단을 덧붙였다. `kr_codes` 가
`resolve_codes` 의 strip 된 키를 그대로 내보내 앞뒤 공백이 붙은 이름을 넘긴
렌더러가 조용히 못 찾던 자리를 호출부 키로 되돌렸다. 회사별 섹션 부제
(`stocksSubtitle`)가 링크 없는 세 번째 표면인데 문서가 둘이라고 적고 있어
셋으로 고쳤다.

### 실수 #400

400. **'보드가 선언한다' 축을 만들 땐 선언할 수 없는 보드가 있는지 먼저 물을 것**
    (2026-09-22 혼합시장 딥링크 — #399 의 다음 층): 보드의 나라는 **교역
    상대국**이지 상장 시장이 아니다 — 말레이시아 보드에 `삼성SDI (006400)`,
    중국 보드에 `Taiyo Yuden (6976)` 이 실렸고 `local=` 만으로는 그 카드가
    영원히 평문이었다(#171). 실측 57장 중 3장 — **재기 전엔 '모두 적용'이
    참인지 몰랐다**(#12). 답은 추측이 아니라 **대조**다 — **거래소 목록**에 물어
    **이름·코드가 둘 다** 맞을 때만 붙일 것(#86·#25).
    ⚠️ **대조 오라클이 '값을 보여주는 경로'면 안 된다** — 시세 사다리는
    `_DIRECT_CODES` 로 상장사를 모회사로 바꾸므로(`코오롱플라스틱 → 120110`)
    신원에 쓰면 진짜 코드가 평문이 되고 대용 코드가 **남의 회사 화면**을 연다.
    한 표가 두 질문에 답하게 두지 말 것(#34·#55).
    ⚠️ **대조 수단이 없는 시장은 그대로 비울 것** — 별칭표에 이름을 하나씩
    더하는 건 종목별 패치다(§UNIVERSAL·#165).
    ⚠️ **새 의존을 넓히면 그 축의 테스트 격리도 같이 옮길 것**(#30·#312·#344·
    #384·#119). 배선을 넓히면 **전수로** 태울 것(#20·#291).
    ⚠️ 조회 **범위**는 결과가 아니라 인자로 잴 것(#91b) — 결과가 같으면 눈이 먼다.

배포 확인 중에 나왔다. 여섯 신호(양쪽 checkout sha · 프로세스·타이머 active ·
보드 HTML mtime · 앵커 수)가 전부 초록인데 카드 수와 링크 수를 세어 보니
`cn_stock 7/6` · `my_stock 8/6` 이었다. 사용자의 원 요청이 "종목이 있는
대시보드에 종목들은 모두 적용" 이었으므로 5% 를 조용히 넘기지 않고 재현
테스트부터 썼고(§Pre-commit 9), `삼성SDI (006400)` 을 말레이시아 보드에
`Taiyo Yuden (6976)` 을 중국 보드에 태우니 대조군 `PENG` 만 링크가 붙었다.

`lookup_query` 에 `kr_master` 축을 더했다. 세 번째 판정이 "보드가 `local="KR"`
로 선언했거나 KRX 목록이 이 이름을 이 코드로 푼다" 두 갈래가 됐고
`kr_confirms` 가 이름·코드 양쪽 일치를 요구했다. 마스터는 이미 mtime 키로
메모이즈돼 있어 보드 넷이 더 불러도 파일 읽기는 프로세스당 한 번이었고 렌더
경로는 `fetch=False` 라 외부 호출이 0 이었다. 맨 6자리 행의 이름만 묻도록
걸러 리졸버가 헛돌지 않게 했다.

뮤테이션 10종을 돌려 처음엔 M6(jp 배선)·M7(tw 배선) 둘이 생존했다. 네 보드에
같은 배선을 넣고 테스트는 둘에만 썼기 때문이었다. 두 보드의 캡션이 실제로 맨
6자리 티커를 뽑는지 먼저 쟀고 둘 다 뽑았으며, 케이스를 전수로 넓히자 열 개가
전부 잡혔다. 복원은 md5 로 대조했다.

일본 상장사 `Taiyo Yuden (6976)` 은 여전히 평문으로 남았다. 대조할 목록이 없고
별칭표에 이름을 하나씩 더하는 건 종목별 패치라 하지 않았다. 이 보드들에 한국
상장사가 얼마나 자주 오는지도 재지 않았고 오늘 실측이 3장이었다.

독립 리뷰가 배포 전에 오라클을 뒤집었다. 나는 "이미 로딩된 KRX 목록에 물어"
라고 적어 놓고 실제로는 `price_provider.resolve_codes` 를 배선했는데, 그 사다리
1단 `_DIRECT_CODES` 가 `코오롱플라스틱 → 120110`(코오롱인더스트리) 처럼 상장사를
모회사로 바꿔 놓았다. 소스 주석이 "138490 시세 미표시 — 운영자 확인" 이라고 그 의도를
밝히고 있었다. 마스터를 스텁해 재현하니 캡션이 진짜 코드 138490 을 들고 오면
대조가 어긋나 평문이 됐고, 대용 코드 120110 이면 확인을 통과해 남의 회사 화면이
열렸다. 오라클을 `_load_krx_master` + `_NAME_ALIASES` 로 바꾸자 양쪽이 고쳐졌다.
같은 리뷰가 잡은 나머지 넷은 죽은 분기, 세 보드의 무가드 필터, 네 보드에 그대로
붙여 넣어 말레이시아 보드에서 거짓이 된 주석, 계열이 아닌 색인 등재였다.
뮤테이션 8종을 다시 돌려 전부 잡았고 복원은 md5 로 대조했다.

### 실수 #401

401. **테스트의 자식 프로세스는 부모의 소켓 그물을 빠져나간다 — 그 그물이 "어느
    라이브러리든 걸린다"고 적고 있었다**(2026-09-22 일본 마스터 프로브, 리뷰 실측):
    자식으로 yfinance 를 부르자 `make test` 가 Yahoo ~6 · JPX 9 요청을 내보냈는데
    #336 의 루트 conftest 는 한 글자도 못 봤다(소켓 패치는 **이 프로세스**만
    덮는다). 테스트별 스텁은 목록형이라 다음을 못 잡으므로(#24) `PYTHONPATH` 맨
    앞에 같은 가드를 담은 `sitecustomize` 를 넣어 **어느 파이썬 자식이든** 걸리게
    했다(#119). 기존 것은 이어 실행한다. **못 보는 축**: 파이썬 아닌 자식과
    `-I`/`-E`/`-S`(#274).
    ⚠️ 그 체이닝 가드가 처음엔 눈이 멀었다 — `'sitecustomize' in sys.modules` 는
    **우리 것**이 들어와도 참이다(#91b). 차단 확인은 **리터럴 IP** 로(이름은 DNS
    실패를 '차단됨' 으로 읽는다).
    ⚠️ 짝 둘: 진단이 제품보다 **좁은 술어**를 복제하면 화면이 거는 것을 못 센다 —
    술어는 import 할 것(#35·#38) · 인코딩 사다리의 `errors="ignore"` 는 첫 후보를
    늘 '성공' 시켜 나머지를 죽인다(#136). 사다리 조건은 '실패했나' 가 아니라
    '요구를 충족했나' 이다.

사용자가 "일본 마스터, 첫번째 해줘" 라고 해 시작했다. 샌드박스는 yahoo·jpx 를
프록시가 둘 다 막아 외부를 한 줄도 못 쟀고, 그래서 배선 전에 재기만 하는 프로브를
먼저 심었다. 그 프로브가 자식 인터프리터로 yfinance 를 불렀다. 독립 리뷰가 배포
전에 그 자식이 `make test` 에서 실제로 Yahoo 약 6건 · JPX 9건을 낸다는 것을
실측해 잡았다. 루트 conftest 독스트링이 "소켓에서 막으면 어느 라이브러리든
걸린다" 고 단언하고 있었으므로 그 문장부터 거짓이었고 같은 커밋에서 고쳤다.
막는 방식은 `PYTHONPATH` 맨 앞에 같은 소켓 가드를 담은 `sitecustomize` 를 넣는
것이었고, 기존 `sitecustomize` 는 가리지 않고 이어 실행하게 했다.

첫 차단 확인은 호스트 이름을 써서 `Errno 97` 을 '차단됨' 으로 읽을 뻔했다.
리터럴 IP 로 바꾸자 비로소 가드 문구가 나왔다. 체이닝 가드는 뮤테이션이 생존해
두 번째 판에서야 실행한 경로를 값으로 남기게 고쳤다. 프로브 쪽 뮤테이션 19종과
conftest 쪽 4종은 전부 잡혔고 복원은 md5 로 대조했다.

리뷰가 같이 잡은 나머지 여덟은 제품보다 좁은 코드 모양, 삼켜진 DB 읽기 실패,
전부 실패한 단계를 '쟀다' 로 센 계수, 멀티바이트 CSV 절단, 상한 없는 일괄 조회,
상태코드를 안 본 표본 단계, 링크 정규식의 사각, 그리고 페이지 라벨로 표본을
중복 제거한 것이었다. 인코딩 사다리를 `errors="ignore"` 로 고친 첫 판은 내가
쓴 회귀가 다시 잡았다.

### 실수 #403

403. **사용자가 이미 본 캡션을 '원천 미게시'로 판정했다 — 반증이 레포에 있었다**
    (2026-09-24 kri): `kr_stock_imports` 독스트링이 사용자의 09-16 텔레칩스
    스크린샷을 적고 있는데, 09-22 dry-run 에 내 grep 0건으로 '원천 미게시 확정'
    을 보고했다. 도구는 **파서는 받는데 아직 안 받아 온 유닛**을 계수로만 셌고,
    그 줄의 라벨 `한국 수입(회사별)` 을 내 `grep '수입 한국'` 이 **어순**으로
    걸렀다(#83). 양성 대조 없는 0건은 판정이 아니다(#54·#91b) — 단정 전에 레포에
    있는 반증부터 볼 것(#392). 유력한 뿌리는 "파서 배포 = 백필 수동 회수"(#261)를
    사람 손에 둔 것이라 구조로 옮겼다(#119): 필터 코드 지문이 바뀌면 동기화가
    40일을 한 번 훑는다. 원인 갈래는 추측하지 않고 `--find`(어디로)·
    `diagnose_badonion`(왜)이 가른다. 새 진단을 만들기 전에 같은 일을 하는 형제
    도구부터 볼 것(#38) — `--find` 는 형제가 하던 일을 겹쳐 만들었고 둘의 세션
    복사 규약이 갈렸다. 재시도 상한은 **다시 물어서 답이 바뀌나**로 셀 것(#279) —
    FloodWait 을 세면 기다리면 풀릴 것을 포기하고, 진전을 한 실행의 포워드 수로
    재면 inbox 에 안 닿는 사본이 매 틱 진전처럼 보여 영영 안 센다. 자동 회수를
    연속 실패로 끊으면 뒤의 유닛은 한 번도 시도되지 않은 채 포기된다.

사용자가 "두번째도 해줘" 라고 해 다시 봤다. 09-22 에 나는 kri(한국 수입 회사별)
보드가 비어 있는 이유를 '원천 미게시' 로 확정했는데, 근거는 사용자가 돌린
`cd ~/stock-trade && .venv/bin/python -m trade.scripts.backfill_badonion --dry-run
--show-irrelevant --lookback-days 14 2>&1 | grep -E '▶️|수입 한국' | head` 가
0줄이었다는 것뿐이었다. 그런데 `trade/kr_stock_imports.py` 독스트링은 사용자가
2026-09-16 에 "현재는 8월부터 텔레칩스 하나야" 라며 첨부한 캡션 스크린샷을 적고
있었다 — 원천은 이미 냈다.

그 측정이 눈먼 이유는 둘이었다. 첫째, `--show-irrelevant` 는 드랍된 유닛과 이미
inbox 에 있는 유닛만 머리를 찍고, 파서는 받는데 아직 포워드되지 않은 유닛은
`relevance breakdown` 계수 줄로만 셌다. 둘째, 그 계수 줄의 라벨은 `한국 수입(회사별)`
인데 내가 준 grep 은 `수입 한국` 이라 어순이 반대였다. 같은 grep 에 수출 쪽 `▶️`
캡션도 한 줄 안 잡혔다는 것 자체가 필터가 눈멀었다는 신호였는데, 그걸 양성 대조로
쓰지 않았다.

유력한 뿌리는 따로 있었다. 관련성 필터가 곧 파서라 파서가 생기기 전 캡션은
리스너가 버렸다. 되찾는 길은 백필이었는데 6시간 동기화 타이머는 최근 3일만 봤다.
#261·#330 은 "파서 배포는 백필 수동 회수까지가 한 세트" 라고 적어 사람 손에
맡겼고, #370 배포 뒤 그 수동 단계는 telethon 미설치와 지어낸 `--days` 플래그로 두 번
실패했다(#371). 09-17 에도 백필이 돌았지만(`9/402`, #374) 그때는 소스별 계수가 없어
kri 캡션이 그 9건에 있었는지 알 수 없었다. 그래서 원인은 확정할 수 없었다 —
캡션이 파싱은 되는데 포워드된 적이 없었을 수도(이 변경이 회수한다), 실물 캡션을
kri 파서가 거부했을 수도(이 변경으로는 안 채워진다) 있었고, 09-22 의 grep 이 다른
이유로 비었을 수도 있었다(캡션이 14일 창 밖이었거나, 명령 오류가 `2>&1 | grep` 에
가려졌거나). 첫 판의 이 문단은 앞의 둘만 적어 '갈래는 끝까지 둘' 이라고 했는데,
2차 리뷰가 셋째 가능성을 짚었다. 그 빈 출력은 어느 쪽과도 모순되지 않았다.

고친 내용: `badonion_sources.relevance_fingerprint` 가 레지스트리에서 파생한 뿌리
(레지스트리 + 각 파서 모듈)에서 패키지 안 import 를 따라간 전이 폐포(실측 27개
모듈, 약 0.2초)를 독스트링을 걷어낸 AST 로 재게 했다. `sync_plan` 은 그 지문이
`~/.trade/badonion_sync_state.json` 의 기록과 다르면(또는 기록이 없으면) 40일 창을,
같으면 3일을 고르게 했다. 백필은 성공한 비 dry-run 실행에서만 기록하게 했고,
명시 창(`--since`·`--lookback-days`·`--to`)은 지금까지 40일을 덮을 때만 기록하게
했다. dry-run 이든 아니든 포워드할 유닛의 머리와 소스 이름도 찍게 했다.

독립 리뷰가 배포 전에 결함 여럿을 더 잡았고 전부 고쳤다. 포워드가 일부 실패한
회수도 기록하던 것을 막았다 — 일시 장애가 같은 경로로 와 캡션을 영영 잃을 수
있었다. 대신 같은 지문으로 3회째 실행에도 실패가 남으면 더 재시도하지 않고
기록하게 해 무한 재시도를 막았다. `--since` 와 `--lookback-days` 를 같이 주면 판정이
`--lookback-days` 를 먼저 봐 4일만 훑고도 기록하던 것을 백필과 같은 우선순위로
맞췄다. `--since ""` 가 TypeError 로 죽던 것, 명시 실행의 사유가 로그에 안 찍히던
것, 기록이 다른 모듈의 원자 쓰기를 빌려 쓰던 것도 고쳤다. 자동 회수는 한 번에
100유닛까지만 포워드하게 했다 — 새 파서가 너무 넓게 잡으면 40일치 무관 글이
비공개 채널에 쏟아질 수 있었다. 그리고 원인 갈래를 사람이 grep 으로 가르지 않게
`--find TEXT` 를 심었다: 캡션 전문에서 찾아 to-forward·irrelevant·already-in-inbox·
ignored·없음 중 어디인지 말하게 했다.

보류한 가설: kri 파서는 스크린샷 재구성이라 `▶️` 의 변형 선택자(U+FE0F)를
스크린샷으로는 확인할 수 없었다. 같은 채널의 품목판 파서 9개(cn·jp2·mx·my·ph·th·
tw·us_imports·us_ppi)가 같은 바이트로 실물 캡션을 운영 중 파싱하고 있었지만, 금액판은
대시 구분자와 한 줄 마커를 더 요구하는 다른 템플릿이라 그 증거는 간접적이었다.
그래서 파서를 바꾸지 않았고 판정은 `--find` 에 맡겼다.

두 번째 독립 리뷰(da4e430..abf8fa6)가 동작 결함 넷(P1·P2·P6·P11)과 사실과 어긋난
문구 셋(P12 · '영구 실패' 단정 · 후보 상한 알림의 '멈춘다'), 테스트가 못 잡는 뮤테이션
16종을 더 잡았다. `--find ""` 는 `bool("")` 이 거짓이라 dry-run 이 강제되지 않아
실제 동기화(포워드·기록)를 돌렸다 — 빈 값은 argparse 단계에서 거부하게 했다. 지문이
이미 기록된 뒤 명시 회수가 일부 실패하면 재시도 표식은 남았지만 다음 자동 동기화는
기록된 지문만 보고 3일로 돌아가, "다음 동기화가 다시 시도한다" 는 로그가 거짓이었다
— `sync_plan` 이 이 지문의 표식을 보면 지문이 같아도 40일을 다시 훑게 했고, 로그는
무엇을 다시 훑는지(최근 40일) 적게 했다. 상태 파일의 재시도 횟수가 숫자가 아니면
포워드를 끝낸 동기화가 `ValueError` 트레이스백으로 끝났다 — 횟수를 방어적으로 읽게
했고, 못 읽어도 '재시도가 남았다' 는 사실까지 버리지는 않게 했다. dry-run·`--find`
는 후보 상한에서 멈춰 찾은 결과를 한 줄도 못 찍고 '동기화 중단' 알림까지 보냈고,
dry-run 인데도 로그가 '성공하면 기록한다' 고 적었다 — 둘 다 고쳤다. 같은 원리로
dry-run 의 시작 실패가 폰에 '동기화 — 시작 실패' 를 보내던 것도(타이머 장애로
읽혔다) 배포전 셀프리뷰에서 찾아 막았다. 재시도 횟수는
유닛별이 아니라 실행별이라 3회째에 처음 실패한 유닛도 거기서 멈추는데, 로그가
그걸 '영구 실패' 로 단정했다 — '재시도 상한' 으로 바꿨다. 후보 상한 알림의 "한 번
돌리면 이 알림이 멈춘다" 도 그 명시 실행이 일부 실패하면 거짓이 되어 그 단서를
붙였다.

리뷰는 `--find` 가 형제 `diagnose_badonion` 과 겹친다는 것도 짚었다. 그 형제는 읽기
전용이고 세션 복사본으로 접속하며 원문 전체를 repr 로 찍었는데, `--find` 는 6시간
타이머가 쓰는 라이브 세션을 열었고, 대소문자를 가렸고, 파서 입력 `text` 만 봤고,
머리 160자만 찍었다. 그래서 `--find` 는 캡션이 어디로 갔는지까지만 말할 수 있었고
왜 버려졌는지는 말하지 못했다 — "`--find` 한 번이면 원인이 갈린다" 는 앞 라운드의
서술은 과대 주장이었다. dry-run 류는 형제처럼 세션 복사본으로 접속하게 했고, 찾기는
원문·마크다운 둘 다 대소문자 무시로 보게 했으며, irrelevant 에서 찾으면 형제 진단의
명령을 찍게 했다. 그 명령은 창 시작일이 아니라 첫 irrelevant 유닛의 날짜에서
시작하게 했다 — 형제는 `--since` 부터 `--limit`(기본 1000)개만 훑어, 넓은 창의
시작일을 주면 그 캡션에 닿기 전에 멈출 수 있었다. 찍는 플래그가 형제의 argparse 에
실제로 있는지는 회귀가 AST 로 쟀다(#371 에서 플래그를 지어내 사용자가 한 번 더
헛돌았다).

검증: 형제 `test_backfill_beon` 은 telethon 이 없으면 통째로 skip 이라 게이트에서
한 번도 안 돌았다. 그래서 가짜 telethon 을 monkeypatch 로만 꽂고 스크립트를
sys.modules 에 등록하지 않는 사설 모듈로 읽어 `main()` 을 태웠다. 리뷰에서 생존한
뮤테이션마다 E2E 를 더했고 #397 누수 가드는 조용했다. 내가 돌린 뮤테이션 45종(리뷰
반영분 20종 포함)은 전부 잡혔고, 리팩터 뒤 앞 라운드 18종을 다시 돌려 가드가 약해지지
않았음도 확인했다. 리뷰가 보고한 생존 9종 중 8종은 재현해 이제 잡히는 것을
확인했고, 남은 하나(해시 전 정렬 제거)는 순회 순서가 이미 결정적이라 동작 차이가
없었다. 2차 리뷰의 생존 16종에는 각각 그걸 잡는 픽스처를 더했다(갈래 사이에서
글이 뒤바뀌는 것, 머리 160자만 찾는 것, 앨범 첫 멤버만 보는 것, 원자 쓰기를 제자리
쓰기로 바꾸는 것 등). 새 동작까지 합친 뮤테이션 45종을 레포 복사본에서 다시
돌렸는데, 베이스라인이 green 인 것을 먼저 확인했고 치환이 정확히 1건인 것만
셌으며, 45종이 전부 잡혔다. 그 라운드에 `main()` 의 dry-run 계산을 중간 변수(`diag`)로
가르는 리팩터를 넣었는데, 형제 파일 `test_tw_monthly_revenue` 의 AST 계약("이 대입이
`show_irrelevant` 를 직접 본다")이 깨졌다 — #403 파일만 좁혀 돌려 못 봤고 전체 게이트가
잡았다(#363·#366). 동작은 같았지만 변수가 애초에 필요 없어 한 식으로 되돌렸고, 그 식에
닿는 뮤테이션 3종(`--find`·`--show-irrelevant` 강제 제거, 강제 로그 제거)을 더 돌려
잡히는 것을 확인했다.

아직 재지 않은 것은 텔레칩스 캡션의 실제 게시일과, 실물 캡션을 kri 파서가 받는지
였다. 그 판정은 배포 뒤 `--dry-run --since 2026-08-20 --find 텔레칩스` 가 어디로
갔는지(to-forward 면 파싱은 되는데 포워드된 적이 없던 갈래, irrelevant 면 파서가
거부한 갈래, 없음이면 창·게시일 쪽)를 말하고, irrelevant 면 그 출력이 찍는
`diagnose_badonion` 명령이 왜 버려졌는지를 찍게 남겼다.

세 번째 독립 리뷰(abf8fa6..1ecef2f)가 첫 리뷰 문단의 수렴 서술 하나를 반증했다.
첫 회수가 흩어진 실패로 rc 0 을 내 재시도 표식 1 이 남은 뒤 좋은 유닛은 inbox 로
들어갔고, 다음 틱의 후보에는 실패한 유닛만 연달아 남았다. 그 실패들이 연속 실패
상한에 걸려 `BackfillAborted`(rc 1)로 끝났는데, 그 경로는 `finish_recovery` 에 닿지
않아 횟수가 영영 1 이었다 — 6시간마다 40일을 다시 훑고 '❌ 백필 중단 … likely
systemic' 을 보냈다. 그래서 "무한 재시도를 막았다" 는 그 경로에서 거짓이었다(#171).
중단된 회수도 횟수에 세게 했고, 그 사유는 '포워드 실패 N건' 이 아니라 중단 사유로
적게 했다 — FloodWait 중단은 실패가 0건일 수 있었다(#292). 자동 회수 중의 중단
알림에는 'systemic' 이 단정이 아니라는 문장을 붙였다. 프로세스가 죽은 실행(타임아웃
kill)은 여전히 세지 못했다 — 그때도 포워드된 유닛은 inbox 로 들어가 다음 틱의
후보에서 빠졌다.

같은 리뷰가 낮은 결함 넷을 더 잡았다. 안내 명령이 `--grep -KY` 처럼 대시로 시작하는
검색어를 옵션처럼 넘겨 형제의 argparse 가 거부했다 — `--grep=` 로 붙여 썼다. 인증이
풀린 세션의 복사본에 `start()` 하면 전화번호 로그인을 물었고, 그 로그인은 실행 끝에
복사본과 함께 지워졌다 — 복사본은 접속만 하고 인증을 확인해 멈추게 했다. 원본
세션이 없을 때 백필은 조용히 라이브 경로를 썼고 형제는 경고한 뒤 복사본에 로그인해
지웠다 — 두 번째 라운드에 '형제처럼' 이라 적은 복사 규약은 그때 이미 둘로 갈려
있었다(#38). 이제 둘은 같은 함수(`_dry_run_session`·`tg_entities.start_client`)를
썼다. 갈래별로 찾은 수는 세어 놓고 쓰지 않았다 — 한 줄로 찍게 했다. 두 번째
라운드의 결함 수도 세 곳이 서로 달랐다(테스트 머리말의 5건 · 이 절의 '여섯' ·
커밋 본문의 8줄) — 이 절의 그 문장을 리뷰가 잡은 것과 셀프리뷰가 잡은 것으로
갈라 고쳐 적었다.

리뷰가 보고한 생존 뮤테이션 19종 중 동등한 둘과 관측할 수 없는 하나를 뺀 16종에
픽스처를 더했다. 안내 명령은 플래그 이름을 대조하는 데서 그치지 않고, 가짜 telethon
위에서 형제 진단의 `main()` 으로 그대로 실행해 레포 루트로 가는 `cd` · 운영 유닛
ExecStart 와 같은 venv · 가장 이른 irrelevant 날짜 · 대시로 시작하는 검색어까지 그
캡션에 닿는지 쟀다. 가짜 클라이언트는 telethon 처럼 접미가 없을 때만 `.session` 을
붙이게 했다 — 늘 붙이던 흉내는 한 변형을 '잡힌 것' 처럼 보이게 했다.

네 번째 독립 리뷰(c5380dd..9794b0d)가 세 번째 라운드의 그 수정이 너무 넓었다는 것을
보였다. 중단을 전부 세게 하자 긴 FloodWait 으로 첫 유닛에서 끊긴 실행도 한 번으로
셌고, 제한 창 하나에서 알림이 시키는 대로 세 번 다시 돌리는 것만으로 캡션 하나
시도하지 않고 지문을 기록해 회수를 포기했다 — 리뷰가 그걸 R2 로 재현했다. 텔레그램이
실행마다 몇 건만 보내 주고 제한을 걸면 매 실행이 조금씩 나아가는데도 3틱 만에
포기해, 남은 유닛은 한 번도 시도되지 않았다 — R8 이 그걸 재현했다. 포기 문구는
'permanent forward failure' · 'giving up on msgs' 로그 줄을 가리켰는데 FloodWait
중단엔 그 줄이 아예 없었다. 포기는 info 한 줄이라 조용했고, 중단 알림은 판정이 나기
전에 이 중단도 재시도 횟수에 센다고 적었다.

그래서 셈을 `recovery_attempt_counts` 로 갈랐다. 포워드를 끝낸 실행과 진전 없이
연속 실패로 끊긴 실행만 세게 했고, 긴 FloodWait 과 진전 있는 중단은 세지 않게 했다 —
앞의 것은 기다리면 풀렸고, 뒤의 것은 남은 유닛 수로 유계라고 봤다. 그 근거는 다섯
번째 리뷰가 반증했다. #279 에 적어 둔 '더
물어서 답이 바뀌나' 를 나는 이 셈에 적용하지 않았었다. 세지 않는 중단도 재시도 표식은
남기되 횟수는 그대로 두게 했다 — 지문이 이미 기록된 뒤 사람이 돌린 넓은 회수가
FloodWait 에 끊기면, 표식이 없을 때 다음 자동 동기화가 3일로 돌아가 못 보낸 캡션을
영영 잃을 수 있었다. 표식엔 그 실행이 중단이었는지를 싣게 했고, 사유가 '중단돼' 와
'실패가 남아' 를 가르게 했다 — 옛 표식엔 그 칸이 없어 둘 중 무엇인지 단정하지 않게
했다. 포기는 경고로 찍게 했고, 못 보낸 원본 msg id(20개까지, 넘으면 수)와 사람이
돌릴 명령(`cd ~/stock-trade && .backfill-venv/bin/python
trade/scripts/backfill_badonion.py --since <가장 이른 날>`)을 싣게 했다. 그 명령을 가짜
telethon 위에서 dry-run 으로 돌려 그 캡션들이 포워드 후보에 드는지 쟀다. 중단·완료 알림은 `run()` 이
보내지 않고 `main()` 이 판정(재시도 N/3 · 세지 않음 · 포기)을 붙여 한 번 보내게 했다.
판정 기록 자체가 실패하면 그 알림이 판정을 못 붙인 이유를 적게 했다.

같은 리뷰가 낮은 결함 둘을 더 잡았다. 시작 실패 알림이 고치는 법을 두 번 실었다 —
예외 원문을 200자에서 자른 것과, 사유 갈래가 돌려준 같은 원문이었다. 그리고 우리가
고칠 방법을 문장에 담아 던진 예외(세션 형식 · 복사본 미인증)도 트레이스백을 찍어,
dry-run 에서 무엇을 해야 하는지가 그 밑에 묻혔다. 그런 예외는 한 줄로만 남기고
알림엔 원문을 한 번만 싣게 했고, BeOn 동기화도 같은 헬퍼(`tg_entities.prescribed_failure`
· `startup_failure_text`)를 쓰게 했다. 예상 못 한 예외는 여전히 트레이스백을 남기게
했다.

L8 하나는 남겼다. 동기화 유닛의 `TimeoutStartSec=600` 과 FloodWait 상한
(`TRADE_MAX_FLOOD_WAIT_S` 600초)이 겹쳐, 실행 도중 수백 초짜리 FloodWait 을 기다리면
systemd 가 알림 없이 프로세스를 죽일 수 있었다. 이 배치 이전부터 있던 설정이었고,
죽은 실행은 `finish_recovery` 에 닿지 못해 셈도 표식도 남기지 않았다. 포워드된
유닛은 inbox 로 들어가 다음 틱 후보에서 빠졌다. 실행 예산을 스크립트가 알게 하는
수정은 유닛 파일을 바꿔야 해서 별도 작업으로 제안했다.

다섯 번째 독립 리뷰(9794b0d..9745aa2 — 네 번째 라운드 반영분)가 그 셈을 다시
반증했다. 진전 있는 중단을 세지 않게 한 근거는 '남은 유닛 수로 유계' 였는데, 리뷰
B 가 그 근거를 무너뜨렸다. 포워드된 사본이 inbox 에 닿지 않으면(출처를 알 수 없는
포워드 · 트레이드 봇 장애) 같은 유닛이 매 틱 다시 포워드돼 진전처럼 보였고, 그
중단은 영영 세어지지 않았다. 그러면 같은 캡션이 6시간마다 비공개 채널에 다시 들어갈
수 있었다. 리뷰 C 는 자동 회수가 연속 실패 다섯 건에서 끊기는 것을 짚었다. 실패
덩어리 뒤의 유닛은 한 번도 시도되지 않은 채 재시도 셈만 올라 포기됐고, 포기 알림은
그 유닛까지 '포워드 못 한 id' 한 목록에 섞어 실패한 것처럼 적었다.

그래서 자동 회수는 연속 실패로 끊지 않게 했다. 자동 회수는 유닛 상한(100) 안이라
연속 실패 가드가 막으려던 '수천 건을 조용히 갈아 넘기기' 가 없었고, 체계적 장애는
'전체 실패' 완료 알림과 재시도 상한이 잡았다. 사람이 연 창은 전처럼 연속 실패 다섯
건에서 끊게 두었다. 셈에서는 '진전' 갈래를 지웠다 — FloodWait 이 아닌 중단은 포워드
수와 무관하게 셌다. 포기 알림은 실패한 id 와 중단으로 시도 못 한 id 를 두 목록으로
갈라 적게 했다. 자동 회수가 연속 실패로 끊기지 않게 되면서 중단 알림에 붙이던
'systemic' 해명 문장도 지웠다.

같은 리뷰가 셋을 더 잡았다. `run()` 이 알림을 판정 뒤로 미룬 뒤 `finally` 의
disconnect 가 던지면 `run()` 이 예외로 끝나 그 알림이 통째로 사라졌다(E) — 끊기
실패는 경고로 남기고 rc 와 알림을 지키게 했다. 판정 기록이 실패했을 때(F) 로그와
알림은 '다음 동기화가 회수 창을 한 번 더 쓴다' 고 단정했는데, 지문이 이미 기록돼
있고 표식이 없으면 다음 창은 기본 3일이었다. 그 두 문장을 남은 기록에 따라 갈리는
두 갈래로 고쳐 적었다. 재시도 횟수를 읽는 두 곳(셈 · 사유 문구)은 서로 다른 판정을
써, `"0"` 을 셈에서는 0 으로 읽고 사유에서는 '못 읽었다' 고 적었다. 한 판정
(`_count_readable`)으로 모았다. 배포전 셀프리뷰가 하나를 더 찾았다. 네 번째 라운드가
판정을 완료 알림에 붙인 뒤로(L6) 한 통이 '영구실패로 스킵된 unit' 과 '다시 훑는다
(1/3)' 를 같이 적었다 — '영구' 를 뺐다.

### 실수 #404

404. **운영 venv 가 아닌 인터프리터로 한 번 연 것이 운영 세션 파일을 옛 판이 못 읽는
    형식으로 올렸다 — 그 생성자 실패는 알림 밖이었다**(2026-09-24 나쁜양파 동기화
    `too many values to unpack (expected 5)`): telethon 은 세션 SQLite 를 열 때 제
    형식으로 **올린다**(1.45.0 = v8) — 운영 `.backfill-venv`(1.36.0 = v7)는 그 파일을
    생성자에서 못 연다. 유력한 경로는 내가 09-22 에 건넨 `.venv/bin/python` 명령이다
    (그 venv 의 판은 아직 안 쟀다, #165). 상태 파일을 공유하는 의존성은 **판을
    고정하고 여는 쪽이 형식을 잴 것** — `guarded_client` 가 못 읽는 형식은 깔 판을
    말하며 막고, 운영 세션을 올리는 것은 **운영 venv 가 고정판일 때만** 허용한다.
    못 잰 운영 세션은 비운영 인터프리터에 닫을 것 — 핫 저널은 읽기 전용으로 못
    재도 telethon 은 열어 올린다. 판은 숫자로 비교할 것(`1.45` = `1.45.0`).
...
돌렸고, 세션 복사 로그 바로 뒤에서 `telethon/sessions/sqlite.py` 64줄의 `ValueError:
too many values to unpack (expected 5)` 로 끝났다. 사용자는 "이거 고친것 같은데" 라고
물었다. 트레이스백의 경로는 `.backfill-venv` 안이었고, 그 venv 는
`trade/scripts/requirements.txt` 의 고정판 telethon 1.36.0 으로 깔려 있었다 — 고쳐진
적은 없었다.

그 줄은 1.36.0 이 `sessions` 행을 다섯 칸으로 푸는 자리였다. 1.45.0 은 세션 DB
형식을 v8 로 올리며 `tmp_auth_key` 열을 더했고(`_upgrade_database` 가 v7 이면 `alter
table sessions add column tmp_auth_key blob` 을 돌렸다), 파일을 열 때 옛 형식이면
그 자리에서 올렸다. 그래서 더 새 telethon 이 한 번이라도 연 파일은 1.36.0 의
생성자에서 여섯 칸이 되어 죽었다. 샌드박스의 telethon 1.45.0 으로 v7 파일을 열어
v8 로 올라가는 것을 재현했다.

누가 올렸는지는 재지 못했다. 가장 유력한 경로는 09-22 에 내가 건넨 `cd ~/stock-trade
&& .venv/bin/python -m trade.scripts.backfill_badonion --dry-run --show-irrelevant
--lookback-days 14` 였다 — `.venv` 는 루트 `requirements.txt` 의 `telethon>=1.34` 로
상한 없이 깔렸고, 그 무렵 dry-run 은 복사본이 아니라 라이브 세션을 열었다. 운영 유닛
넷(`deploy/trade-bot-{badonion,beon}-{sync,listener}.service`)은 전부
`.backfill-venv` 로 돌았는데, 그 명령은 그걸 확인하지 않고 쓰였다. 이 경로가 맞다면
6시간 동기화는 09-22 부터 매 틱 생성자에서 죽었다 — 그 생성은 알림 `try` 밖이라
폰에는 아무것도 오지 않았다.

고친 것: `trade/tg_entities.py` 에 `guarded_client` 를 두어 세션 파일의 DB 형식(읽기
전용 `mode=ro`)과 설치된 telethon 의 형식을 생성 전에 재게 했다. 파일이 더 새 형식이면
막고 깔 판(`telethon==1.45.0`)을 문장에 담았고, 고정판이 아닌 telethon 이 운영(라이브)
세션을 올리려 하면 막았다. 고정판이 올리는 것(운영이 판을 올린 경우)과 복사본, 재지
못한 경우는 막지 않았다. 고정판은 1.45.0 으로 올렸고 그 출처는
`trade/scripts/requirements.txt` 한 곳으로 두었다. 동기화 두 스크립트는 생성을 알림
`try` 안으로 옮겼고, 리스너 둘은 형식 불일치를 알린 뒤 EX_CONFIG(78)로 끝나게 했다 —
유닛의 `RestartPreventExitStatus=78` 이 재시작 루프를 막았다. 진단 둘
(`diagnose_badonion`·`diagnose_dedup`)도 같은 가드를 거치게 했고, 트레이드 운영
코드에 `TelegramClient(...)` 직접 생성이 남지 않았는지는 회귀가 디렉터리 전수로 쟀다.

아직 재지 않은 것은 VM `.venv` 의 telethon 판과 세션 파일들의 실제 DB 버전이었다.
사용자에게 읽기 전용 측정 명령과, 운영 venv 에 고정판을 까는 명령(`cd ~/stock-trade
&& .backfill-venv/bin/pip install telethon==1.45.0`) 뒤 리스너 둘을 재시작하는 순서를
건넸다. 자동 배포(`deploy/trade-auto-update.sh`)는 pip 를 돌리지 않았으므로 이 커밋의
고정판은 그 설치가 있어야 효력이 있었다 — 설치 전에 이 코드가 배포되면 가드가 설치
명령과 함께 알리게 했다.

네 번째 독립 리뷰(M1)가 가드의 판별 기준 자체를 반증했다. 나는 '고정판이 아닌 판' 을
'운영 venv 가 아닌 인터프리터' 의 대용으로 썼다. 그런데 NOAH `.venv` 는 상한 없이
`telethon>=1.34` 로 깔리므로 마침 고정판과 같은 판(1.45.0)으로 풀릴 수 있었고, 그 venv
로 돈 실행은 판이 같으니 통과해 운영 세션을 v8 로 올렸다 — 리뷰가 실물 telethon 으로
그걸 재현했다. 핀은 `git pull` 로 움직였지만 운영 venv 는 사람이 pip 를 돌려야
움직였으므로, 그 사이 운영 유닛이 옛 판인 창에서 이 사고가 그대로 재발할 수 있었다.
판별을 인터프리터 경로(`sys.executable` 에 `/.backfill-venv/` 가 있나)로 바꿨다. 운영
venv 가 아니면 판이 무엇이든 운영 세션을 올리지 못하게 했고, 운영 venv 는 고정판일
때만 올리게 했다. 실물 telethon 을 고정판으로 둔 채 다른 venv 경로에서 v7 세션을 열어
안 올라가는 것을 재는 회귀를 뒀다.

같은 리뷰의 낮은 결함 셋도 고쳤다. 운영 venv 인데 판이 고정판이 아니면 옛 안내는 그
venv 로 돌리라는 말뿐이었다 — 이미 그 venv 에 있는 사람에게는 소용이 없었다. L1 로
고정판을 깔라고 말하게 했다. 핀을 못 읽으면 안내 문장이 `telethon None` 을 고정판처럼
적었다 — L2 로 고쳤다. 세션 파일을 못 읽거나(핫 저널 · 손상 · version 행 없음)
telethon 판을 못 재면 가드는 조용히 통과했다. L3 로 막지는 않되 경고 로그로 말하게
했다. 그리고 `mode=ro` 는 이 라운드에 처음으로 발화 경로를 얻었다. 쓰다 죽은
트랜잭션의 핫 저널을 자식 프로세스로 만들어, 측정이 그 저널을 되돌리지 않는지(파일
바이트와 `-journal` 이 그대로인지) 쟀다. 앞 라운드에서 그 뮤테이션을 '동등' 으로
분류한 것은 재지 않은 분류였다. 리스너 둘 · BeOn 동기화 · dedup 진단이 운영 세션을
`live=True` 로 재는지 스파이로 고정했다 — 옛 스텁은 그 인자를 무시했다.
`TelegramClient` 의 어떤 별칭·속성 참조도 `guarded_client` 의 첫 인자 밖으로 새지
않는지는, 우회 픽스처로 검사 자체를 잰 AST 검사가 보게 했다.

다섯 번째 독립 리뷰가 가드에서 둘을 더 잡았다. A: 핫 저널이 남은 운영 세션은 읽기
전용 측정으로 형식을 못 쟀는데, 옛 판은 '못 잼 → 경고하고 통과' 라 운영 venv 가 아닌
인터프리터도 그 파일을 열었다. telethon 은 열면서 저널을 되돌린 뒤 제 형식으로
올렸다 — 리뷰가 고정판과 같은 판의 비운영 venv 로 v7 세션이 v8 로 올라가는 것을
재현했다. 비운영 인터프리터가 못 잰 운영 세션을 열려 하면 막게 했고, 운영 venv 와
복사본, 파일이 없는 경우(첫 인증)는 전처럼 통과시켰다. 실물 telethon 으로 핫 저널이
남은 v7 세션을 만들어 가드가 파일과 저널을 건드리지 않는지 쟀다. 운영 venv 는 못 잰
세션을 여전히 경고만 하고 열었다 — 운영 venv 의 telethon 이 고정판이 아니면서 핫
저널이 남은 세션을 여는 조합은 이 가드가 막지 못했고, 그 조합은 재지 않았다.

D: 판을 문자열로 비교해, 핀을 `telethon==1.45` 로 적으면 고정판을 깐 운영
venv(`1.45.0`)가 '고정판이 아니다' 로 막히고 이미 깔린 판을 다시 깔라는 안내가
나왔다. 핀 파서는 그 모양을 받았으므로 실제로 열린 경로였다. 판은 숫자로 비교하게
했고(`1.45` = `1.45.0`), 숫자가 아닌 판(rc 등)은 문자열 그대로 두었다. 파일이 더
새 형식일 때의 안내 두 갈래(운영 venv · 비운영)도 같은 비교를 쓰게 했다. 리뷰가 남긴
생존 뮤테이션(다른 SQLite 의 version 칸 · 운영 venv 경로의 이웃 디렉터리 · 역슬래시
경로)에는 픽스처를 더했다. 동기화 E2E 는 가짜 세션 파일의 형식을 못 재므로, 픽스처의
인터프리터를 운영 venv 경로로 두어 운영이 실제로 도는 모양을 따르게 했다.

### 실수 #406

406. **'수신 0건' 을 '못 받았다' 로 읽었다 — 조용히 버리는 게이트가 있으면 0 은 두
    갈래다**(2026-09-25 나쁜양파 27건 회수): 백필은 `forwarded 27 of 27` 인데 32분 뒤
    inbox 는 그대로였고 trade-bot 저널의 `ingested` 는 0 이었다. 채널·출처 게이트가
    로그 없이 `return` 해 '받고 버렸다' 와 구별되지 않았다(07-11 에 같은 게이트가
    68건을 버렸을 때 목록만 넓히고 버림은 조용히 뒀다, #12). 버림은 사유와 함께 적고,
    그 판인지 시작 줄 표식(`drop_log=on`)으로 밝힐 것 — 옛 판의 '버림 0건' 은 증거가
    아니다(#54). 갈래(미가동·409·웹훅·수신 종류·관리자·게이트·inbox 경로)는 명령
    넷으로 맞추지 말고 `trade.bot_health` 가 가르고(#252), '보냈는데 못 받음' 은
    매시간 health 가 두 저널을 대조해 알린다. ⚠️ allowed_updates 는 비워 두면 마지막
    설정이 남는다 — 매 시작에 명시할 것(#38). 저널 명령을 건넬 땐 토큰을 가릴 것
    (§Secrets) — 가림 이전 판이 찍은 getUpdates 줄엔 토큰이 평문이다. ⚠️ 독립 리뷰:
    '받았다' 는 수신 줄 + 게이트 버림이다(되포워드엔 원래 출처가 붙어 버려진다 — 버림을
    누락으로 세면 거짓 알림) · 판정은 지금 프로세스의 줄로 · 처리 중 예외도 조용한 갈래다.
    ⚠️ 2차: 수 대조는 무관한 수신·버림이 손실을 덮는다 — 직접 증거(예외 줄)는 수와
    무관하게 판정해야 한다.

사용자가 40일 회수(동기화 07:48:52~07:49:09 · 27건)와 수동 재실행(07:49:47~07:50:14 ·
27건) 뒤 두 가지를 돌렸다. 08:21 의 백필 dry-run 은 `already ingested: 15790` 으로
회수 전과 같았고 텔레칩스 유닛을 여전히 `to-forward` 로 찍었다. 같은 창의 trade-bot
저널에서 `ingested msg=` 줄은 0 개였다.

나는 직전 답의 판정표에 "to-forward + 0 이면 봇이 못 받은 것" 이라고 적어 두었다. 그런데
`trade/bot.py` 의 `on_channel_post` 는 채널 게이트(`_allowed_channel`)와 출처 게이트
(`_origin_matches`)에서 로그 없이 `return` 했다 — 그 0 은 '못 받았다' 와 '받고 버렸다' 를
가르지 못했다. 같은 게이트는 2026-07-11 에 나쁜양파 포워드 68건을 조용히 버린 적이 있었고
그때는 출처 목록만 넓혔다. 그 기록은 `trade/tests/test_bot_origin.py` 독스트링에 있었다.
#405 가 '비동기로 쓰는 inbox 가 아직 못 따라잡았다' 로 적은 것도 같은 전제 위에 있었다 —
32분 뒤에도 0 이었으니 지연이 아니었다.

두 게이트가 버릴 때 사유를 적게 했다. 출처 게이트는 건마다 origin 종류·chat·username·허용
목록을 적었고, 채널 게이트는 채널마다 프로세스당 한 번 적었다. 봇 시작 줄에 `drop_log=on`
을 붙여 그 판인지 저널이 말하게 했다. `run_polling` 에는 `allowed_updates=Update.ALL_TYPES`
를 넘겼다. 비워 두면 텔레그램이 마지막 설정을 계속 쓰므로, 같은 토큰으로 누가 한 번
channel_post 를 뺀 목록을 보냈다면 재시작해도 채널 글이 오지 않았다 — NOAH 봇은 이미
그렇게 넘기고 있었다.

`trade/bot_health.py`(`cd ~/stock-trade && .venv/bin/python -m trade.bot_health`)를
만들었다. 유닛 상태, 텔레그램(getMe·getWebhookInfo·getChatMember·원천 getChat), 실행 중
설정(지금 PID 의 시작 줄), 저널 창(폴링 상태코드·수신·버림·409), 릴레이 포워드와 수신의
대조, inbox 원천별 마지막 수신을 한 번에 재어 갈래를 말하게 했다. getUpdates 는 부르지
않았다 — 돌고 있는 봇과 409 로 부딪히고, 여기서 받아 간 업데이트는 봇이 못 보기
때문이었다. 원천을 getChat 으로 물은 이유는 출처 게이트가 사용자명으로 대조하기
때문이었다 — 원천이 사용자명을 바꾸면 릴레이는 세션 캐시(숫자 ID)로 계속 포워드하는데
봇은 그 글을 전부 버리게 되어 있었다. 같은 대조(`delivery_check`)를
`trade.scripts.health_check` 가 매시간 돌려, 보낸 만큼 못 받았으면 알리게 했다. 같은
누락은 6시간에 한 번만 알리게 했다.

그러다 트레이드 봇 저널의 getUpdates 줄에 봇 토큰이 평문으로 있다는 것을 코드로 확인했다.
httpx 가 요청 URL 을 INFO 로 찍었고 NOAH 봇과 달리 가리는 필터가 없었다 — watchdog 가 바로
그 줄로 폴링 생존을 재었으니 줄은 실재했다. 그래서 진단의 모든 출력에서 토큰 리터럴과 토큰
모양을 지웠고, 리스너 판정이 권하는 명령도 `systemctl status`(저널 꼬리를 같이 찍는다)
대신 `systemctl show` 로 바꿨다.

아직 재지 않은 것은 봇이 그 27건을 받았는지 받고 버렸는지였다. 운영 중인 봇은 버림을 적지
않는 옛 판이라 저널로는 답이 안 나왔고, 배포 전 텔레그램 쪽 상태(웹훅·수신 종류·관리자·원천
사용자명)는 사용자가 읽기 전용 명령으로 재야 했다.

독립 리뷰(a814a77..6f407d9)가 진단 자체의 결함을 짚었다. 첫째로 대조가 '포워드 한 건 = 수신
줄 한 줄' 을 전제했는데, BeOn 은 AWAKE 플러스 같은 다른 채널의 글을 그대로 되포워드했고
텔레그램은 포워드의 포워드에도 원래 출처를 달았다. 봇은 그 글을 출처 게이트에서 버렸고, 버림을
'못 받음' 으로 센 대조는 매번 거짓 누락이 되었다. BeOn 동기화는 inbox 에 없는 글을 다시
포워드하므로, 캡션 무시 목록에 안 걸리는 되포워드는 동기화마다 다시 포워드되게 되어
있었다(코드로 확인한 것이고 운영에서 몇 번 그랬는지는 재지 않았다). 버림을 받음으로 세고, 릴레이 원천의 글을 버린 것만 따로
세어 알리게 했다. 원천이 사용자명을 바꿨을 때를 위해 릴레이가 접속 직후 찍는
`source(marked)=` 줄에서 원천 ID 를 배웠다.

둘째로 재시작 전 프로세스의 409·버림을 지금 판정에 섞었다. 판정은 MainPID 의 줄로 하고 그 전
것은 메모로 말하게 했다. 막 뜬 프로세스는 첫 폴링 전이라 멈춤과 같은 창만큼 봐 주었다. 셋째로
받은 글을 처리하다 예외가 나면 그 글은 수신 줄도 버림 줄도 없이 사라졌고, 에러 핸들러가 없어
PTB 기본 문구만 남았다. 봇에 에러 핸들러를 달아 핸들러 예외(메시지 번호 포함)와 폴링 오류를
한 줄 표식으로 가르게 했고, 수신 줄이 없는 번호의 예외는 누락의 원인으로 읽게 했다.

넷째로 백필 'done' 줄은 실행 끝에 찍히는데 봇 저널을 같은 창으로 읽어, 창 첫머리에서 끝난
실행의 수신을 못 셌다. 수신은 같은 PID 의 접속 줄부터 세고 봇 저널은 30분 앞부터 읽게 했다.
매시간 알림의 중복 방지 표식이 하나라 6시간 동안 다른 누락도 막혔으므로 누락마다 표식을 두었다.

비밀값 쪽에서는 NOAH 봇이 몇 달 전부터 httpx 로그의 토큰을 가리고 있었는데 트레이드 봇에는
그 선례가 옮겨지지 않았다. 포매터로 옮겨 트레이스백까지 가렸고, PTB 가 토큰 거절 예외 문구에
토큰을 싣는 경로(인터프리터가 직접 찍는 트레이스백)도 진입점에서 받아 같은 포매터를 거치게
했다. ⑦ 명령줄은 가린 뒤에 자르게 했고, `systemctl status trade-bot` 을 권하던 두 곳은 진단
명령으로 바꿨다. 리뷰가 남긴 생존 뮤테이션 13종에는 픽스처를 더했다 — 그중 셋(릴레이 로그
형식·로거 이름·유닛 목록)은 손으로 적은 픽스처가 형식 변경을 축복하던 것이라 소스와 `deploy/`
에서 파생하게 했다.

2차 독립 리뷰(6b005d9..e5b528a)가 넷을 더 짚었다. 첫째로 예외로 놓친 글(수신 줄이 없는
번호)을 누락이 보일 때만 ❌ 로 읽었는데, 수 대조는 번호로 짝을 짓지 않아 같은 창의 다른 글 —
아직 기다리는 새 포워드의 수신이나, 첫 리뷰 반영으로 받음에 넣은 남의 글 버림 — 이 그 손실을
덮었다. 리뷰 재현에서 진단은 통과 판정 옆에 '지금 대조엔 그 글로 빠진 것이 없다' 는 거짓 메모를
달았고 매시간 알림은 조용했다. 그 줄 자체가 증거이므로 수 대조와 무관하게 판정하게 했다. 봇이
예외 줄에 포워드 출처를 버림 줄과 같은 규약으로 싣게 했고, 진단은 릴레이 원천이면 ❌, 출처를
적지 않는 판의 줄이면 ❓, 직접 쓴 글·다른 출처면 메모로 말하게 했다. 매시간 알림도 그 글을
알리게 했다.

둘째로 재시작을 걸친 백필(배포가 봇을 재시작하는 동안 sync 가 돈 경우)에서, 사건은 끝 시각으로
'지금 몫' 에 넣고 수신은 재시작 뒤부터 세어 옛 프로세스가 받은 몫을 버렸다. 27건이 다 inbox 에
있는데 판정은 '텔레그램이 일부를 전달하지 않았다' 로 끝났다. 잰 시작(같은 PID 의 접속 줄)이
재시작 전이면 그 시작부터 두 프로세스의 수신을 같이 세게 했다. 다만 접속 줄이 창 밖이라 30분을
짐작한 시작은 경계를 넘지 않게 했다 — 처음엔 짐작도 넘게 짰다가, 재시작 전에 잃은 글이 지금
프로세스의 누락으로 둔갑하는 것을 기존 회귀가 잡았다.

셋째로 매시간 알림의 표식을 '창 안 첫 포워드' 로 만들어, 끊김이 이어지면 표식이 매시간 바뀌어
같은 포워드를 두 번씩 알렸다. 리뷰 재현에서는 12시간에 12통이었고 포워드마다 두 통이었다. 알린
사실(포워드 사건·버림·예외)마다 신원을 파일에 적고 다음 실행은 아직 안 알린 사건만으로 다시
대조하게 했고, 그러자 각 포워드가 정확히 한 알림에만 실렸다. 버림 때문에 나간 알림에 그 창의
포워드 사건까지 '알렸다' 로 적으면 기다리는 새 포워드의 수신이 잠시 덮은 누락이 영영 안
알려졌으므로, 포워드 사건은 누락을 알렸을 때만 적게 했다. 넷째로 수신을 세려고 30분 앞부터
읽은 봇 저널로 판정까지 해, 사용자가 `--since` 로 뺀 409 가 '지금' 의 ❌ 가 됐다. 저널을 두 번
읽어 판정은 사용자의 창으로, 넓힌 것은 수신 대조에만 쓰게 했다.

그 밖에 문서 둘이 코드와 달랐다. 봇의 로그 틀(`_LOG_FORMAT`)을 진단의 계약이라 적었지만 진단은
메시지 본문만 읽었고, '재시작 전 예외는 메모로 말한다' 고 적었지만 그런 메모가 없었다(이번에
더했다). 백필 'done' 사건엔 구조적으로 0 인 '마지막 포워드 뒤 수신' 수는 뺐다. 리뷰가 남긴 생존
뮤테이션(B07·B12·B13·B21·H01·V04·T07·KF02 등)에는 픽스처를 더했다 — KF02 는 가림 검사가 낱말
'sed' 를 부분문자열로 찾아 'used' 같은 낱말에 속던 것이었다. 반영분에 뮤테이션 50종을 태웠고

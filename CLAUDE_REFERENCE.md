...
- 1주기 40건 수집 · 2주기 저장분 40 + 새로 40 = 80 · 3주기 100% · 그 뒤 네트워크 0
  (회귀가 네 주기를 실제로 태워 값으로 고정).
- 성공은 30일, **실패는 15분만** 믿는다(#303·#161·#152 — 길게 믿으면 원천 장애
  한 번이 그 리포트를 한 달 내내 빈칸으로 굳힌다).
...
가짜 nid 하나가 오래 산다. `tests/conftest.py` 의 세션 리다이렉트에 이 모듈을
추가하고 **메모리 사본도 같이 비운다**(파일만 돌려놓으면 이미 읽어 둔 맵이 다음

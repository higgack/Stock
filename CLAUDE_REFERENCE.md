...
403. **사용자가 이미 본 캡션을 '원천 미게시'로 판정했다 — 반증이 레포에 있었다**
    ...
    있는 반증부터 볼 것(#392). 뿌리는 "파서 배포 = 백필 수동 회수"(#261)를 사람
    손에 둔 것이라 구조로 옮겼다(#119): 필터 코드 지문이 바뀌면 동기화가 40일을
    한 번 훑는다.

사용자가 "두번째도 해줘" 라고 해 다시 봤다. 09-22 에 나는 kri(한국 수입 회사별)
보드가 비어 있는 이유를 '원천 미게시' 로 확정했는데, 근거는 `backfill_badonion
--dry-run --show-irrelevant --lookback-days 14 | grep -E '▶️|수입 한국'` 가 0줄이었다는
것뿐이었다. 그런데 `trade/kr_stock_imports.py` 독스트링은 사용자가 2026-09-16 에
"현재는 8월부터 텔레칩스 하나야" 라며 첨부한 캡션 스크린샷을 적고 있었다 — 원천은
이미 냈다.
...
뿌리는 따로 있었다. 관련성 필터가 곧 파서라 파서가 생기기 전 캡션은 리스너가
버렸다. 되찾는 길은 백필이었는데 6시간 동기화 타이머는 최근 3일만 봤다. #261·#330 은
"파서 배포는 백필 수동 회수까지가 한 세트" 라고 적어 사람 손에 맡겼고, #370 배포
뒤 그 수동 단계는 telethon 미설치와 지어낸 `--days` 플래그로 두 번 실패했다(#371).
그래서 캡션과 배포 사이가 3일을 넘은 채 영영 회수되지 않았다.

고친 내용: `badonion_sources.relevance_fingerprint` 가 레지스트리에서 파생한 뿌리
(레지스트리 + 각 파서 모듈)에서 패키지 안 import 를 따라간 전이 폐포(실측 27개
모듈, 약 190ms)를 독스트링을 걷어낸 AST 로 재게 했다. `sync_plan` 은 그 지문이
`~/.trade/badonion_sync_state.json` 의 기록과 다르면(또는 기록이 없으면) 40일 창을,
같으면 3일을 고르게 했다. 백필은 성공한 비 dry-run 기본 창 실행에서만 기록하고,
명시 창(`--since`·`--lookback-days`·`--to`)은 지금까지 40일을 덮을 때만 기록하게
했다. dry-run
이든 아니든 포워드할 유닛의 머리와 소스 이름도 찍게 했다.

기각한 가설: kri 파서는 스크린샷 재구성이라 `▶️` 의 변형 선택자(U+FE0F)를
스크린샷으로는 확인할 수 없었다. 그러나 같은 채널의 품목판 파서 9개(cn·jp2·mx·my·
ph·th·tw·us_imports·us_ppi)가 같은 바이트로 실물 캡션을 운영 중 파싱하고 있어,
재지 않은 가설로 파서를 바꾸지 않았다.

검증: 형제 `test_backfill_beon` 은 telethon 이 없으면 통째로 skip 이라 게이트에서
한 번도 안 돌았다. 그래서 가짜 telethon 을 monkeypatch 로만 꽂고 스크립트를
sys.modules 에 등록하지 않는 사설 모듈로 읽어 `main()` 을 태웠다. 첫 실행의 40일
회수와 기록, 둘째 실행의 3일 창(20일 전 캡션에 닿지 않음), dry-run·실패·명시 창의
무기록, 지문이 바뀐 뒤의 재회수를 값으로 쟀고 #397 누수 가드는 조용했다.

아직 재지 않은 것은 텔레칩스 캡션의 실제 게시일과, 재구성 픽스처가 실물 캡션과
같은 모양인지였다. 그 첫 실물 대조는 배포 뒤 첫 동기화의 `to-forward unit …
[한국 수입(회사별)]` 줄과 보드로 남겼다.

...
(2026-09-21 거래량 상위 배너, 사용자 "이것도 코멘트 확인해줘"): #391 이
급등·급락에서 고친 그 증상인데, `public_reason` 이 마커 3개짜리 열거로
경계를 **다시 추론**해 목록 밖 상세(zod 해설·pykrx 함수명·예외)는 머리에
남고 목록 안 어구가 `원문: {…}` **안**에서 걸려 `원문: ·` 만 남았다(541자
중 63자만 잘림). `compose_reason` 은 경계를 **알면서 버리고** 있었다
(#123 계열·#24). **아는 쪽이 표식을 찍고 자르는 쪽은 읽기만 할 것**(#86).
⚠️ 그렇다고 **무조건 찍으면 줄이는 것이 지우는 것이 된다** — 그 슬롯엔
덤프 없는 짧은 사람 문장(`shape_reason`)도 와서 급등·급락이 다시 침묵했다.
덤프를 품은 것에만 긋고 덤프인 줄 아는 생산부는 **스스로** 찍는다(#395·#43).
⚠️ 상세가 **사람 문장 안**에 있는 생산부도 있다 — 기계 상세를 그 뒤에 이어
붙이면 사이의 사람 문장이 먹힌다. **이어 붙이지 말고 가를 것**(경계는 언제나
하나 · 앞은 전부 사람 · 뒤는 전부 상세). 그 표식은 **다른 축의 뮤테이션도
가린다**(#75 벌크 층 없는 픽스처로).
⚠️ 표식은 **경계 자리**를 가리키고 뒤쪽에 남긴다 — 벗기는 한 줄을 지우는
변형이 화면에 기호를 흘린다(#119 구조로).
⚠️ 표본을 3건으로 자르면 머리는 6거래일인데 몸통이 3건이라(#45) **자른
사실을 말해야** 한다.
→ REFERENCE §실수 #398
...
결과는 725자 중 175자였다. 화면에는 무엇으로 대체했는지, 기준일이

...
- **부동산 Byte — 광주 대표구 미등록 (2026-08-08 사용자 요청, 보류)**.
  대전 유성구(30200)·세종시(36110)는 VM 라이브 API 조회로 응답 법정동명
  일치 확인 후 `bot/realestate_client.py` `_REGIONS` 등록 완료. 광주는
  2026-04-28/7-1 광주광역시+전라남도가 "전남광주통합특별시"(전국 최초
  광역통합)로 출범하며 舊 광주 시도코드(29xxx)가 전부 무효화된 것으로
  실측 확인(29155 및 인접 29150~29159 라이브 조회 전부 0건) — 광주의
  5개 구(동구/서구/남구/북구/광산구) 자체는 존속한다고 뉴스로 확인됐지만
  통합특별시의 새 시도코드는 검색으로 못 찾음(mois.go.kr/code.go.kr 등
  공식 코드조회 사이트 전부 이 세션에서 접근 불가). 착수 시: (a) 사용자가
  code.go.kr 직접 조회해 새 코드 확인 후 알려주거나 (b) VM 에서 더 넓은
  시도코드 범위(예: 마지막 배정된 특별자치시 세종=36 이후 미사용 번호대)
  를 실거래가 API 로 스캔해 응답 법정동명(봉선동/주월동/진월동 등 남구
  관할동)으로 자체 확인 — 화성 동탄구·대전/세종과 동일 패턴.

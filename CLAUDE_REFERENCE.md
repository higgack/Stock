...
399. **딥링크는 '앵커가 붙었나'가 아니라 '그 주소가 목적지에서 해석되나'까지
    재야 한다**(2026-09-22 수출입 종목 딥링크): 보드 카드 제목·회사별 섹션을
    `/lookup/<질의>` 로 걸며 href 만 보고 끝냈으면 **국내 전 종목이 죽은
    링크**였다 — NOAH `resolve_name_to_ticker` 가 접미사 없는 6자리를 US 심볼로
    떨어뜨려(`detect_market` 기본값) `/lookup/005930` 이 미국 조회가 된다.
    목적지 리졸버를 같은 커밋에서 고치고(6자리 → `normalize_kr_ticker_suffix`
    로 KS/KQ) 회귀가 그 해석을 **값으로** 잰다.
    ⚠️ 질의 규칙은 **관측된 티커 모양**에서 정한다(보드별 실측: 미국 영문심볼·
    KR 6자리·JP 로컬코드) — 못 푼 이름은 평문 그대로다(#144·#43).
    ⚠️ 접두는 상대 `../lookup/` 이고 프록시가 토큰 포함 절대로 바꾼다 — 절대로
    적으면 토큰이 떨어져 404 다(형제 `../media/` 와 **같은 자리**, #38·#359).
    ⚠️ `"sl-link" in html` 은 **스타일시트**가 대신 만족시킨다 — 앵커를 집을 것
    (#300·#75). 그리고 헬퍼만 재면 배선을 떼는 변형이 통과한다(#20) — 실측으로
    `stockHref(name)` → `''` 가 통과했고 뷰를 통째로 태워서야 잡혔다.
    → REFERENCE §실수 #399

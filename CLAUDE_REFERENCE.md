...
403. **사용자가 이미 본 캡션을 '원천 미게시'로 판정했다 — 반증이 레포에 있었다**
    ...
    기다리면 풀리는 FloodWait·진전 있는 중단까지 세면 시도도 못 한 채 포기한다.
...
앞의 것은 기다리면 풀렸고 뒤의 것은 남은 유닛 수로 유계였다. #279 에 적어 둔 '더
...
telethon 위에서 그대로 돌려 그 캡션들에 닿는지 쟀다. 중단·완료 알림은 `run()` 이
...
죽은 실행은 `finish_recovery` 에 닿지 못해 세지 않는 중단과 같게 됐다. 포워드된

...
403. **사용자가 이미 본 캡션을 '원천 미게시'로 판정했다 — 반증이 레포에 있었다**
    ...
    40일을 한 번 훑는다. 원인 갈래는 추측하지 않고 `--find` 가 가른다.
...
kri 캡션이 그 9건에 있었는지 알 수 없었다. 그래서 원인 갈래는 끝까지 둘이었다 —
캡션이 파싱은 되는데 포워드된 적이 없었거나(이 변경이 회수한다), 실물 캡션을 kri
파서가 거부했거나(이 변경으로는 안 채워진다). 09-22 의 빈 grep 은 두 갈래와 다
모순되지 않았다.
...
있었다. 대신 같은 지문으로 3회째에도 남는 실패는 영구 실패로 보고 기록하게 해
무한 재시도를 막았다. `--since` 와 `--lookback-days` 를 같이 주면 판정이
...
였다. 그 판정은 배포 뒤 `--dry-run --since 2026-08-20 --find 텔레칩스` 한 번으로
남겼다.

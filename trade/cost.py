...
def format_dashboard_line(snap: dict | None = None) -> str:
    ...
    # 비용 3창 표기(오늘/이번달/누적, KST — 사용자 2026-07-05). month/total
    # 키 없는 옛 snap 은 기존 30일 표기 폴백(구버전 스냅 호환).
    llm = s.get("llm")
    if llm and llm.get("total_calls"):
        _mn, _tt = llm.get("month"), llm.get("total")
        if _mn is not None and _tt is not None:
            _td = llm.get("today_kst") or llm.get("today") or {}
            parts.append(
                f"LLM 오늘 {_td.get('cost_krw', 0):,}원 · "
                f"이번달 {_mn['cost_krw']:,}원 · 누적 {_tt['cost_krw']:,}원"
            )
        else:
            d30 = llm["d30"]

...
def _load_industry_html(customs_db_path: Path | str | None) -> str:
    ...
    try:
        ...
        # 잠정 '발표'(1일)와 세부 API '적재'(익월 상순, 수일 지연·유동)는
        # 다름 — 2026-07-04 실측: 6월 세부가 7/4에도 미적재(정상 범위).
        # '1일 적재'로 적으면 상순마다 "왜 안 나와?"가 됨(사용자 지적).
        zone_div = (
            "<div class='ind-zone-div'><span>📅 여기부터 <b>월간 산업트렌드</b>"
            " — 잠정 세부는 익월 상순 적재(발표는 1일, API 반영 수일 지연)"
            " → ~15일 확정 정제 · 매일 갱신</span></div>"
        ) if prov_html else ""

...
IGNORED_CONTAINS: tuple[str, ...] = (
    ...
    "이달의 주요 기업 수출데이터",
    # IPO/투자설명서 분석 릴레이 (BeOn, msg 9249 '세레브라스(Cerebras) 투자설명서
    # 분석' 2026-06-15 확인). 미국 상장 예정 기업의 증권신고서/사업설명 equity
    # 노트 — 한국 수출입 데이터가 아닌 무관 자료. 제목이 'X가 선택한 Y, Z(영문)
    # 투자설명서 분석' 으로 회사마다 달라 prefix 미스 → '투자설명서 분석' 본문
    # 키워드로 시리즈(회사 무관) 전체 skip. 수출입 알림은 이 어구를 안 쓴다.
    "투자설명서 분석",
)

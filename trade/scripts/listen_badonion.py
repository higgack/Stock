...
async def _run_listener() -> int:
    ...
    # 실측 2026-08-27, #258). resolve_peer = 세션 캐시 우선.
...
def main() -> None:
    ...
    try:
        ...
        # ⚠️ 재인증 대상이 아니다 — 시작 시(엔티티 해석 등) 요청 제한이면
        # 기다렸다 재시작해야 한다. 곧장 exit 하면 systemd 가 즉시 재시작해
        # 해석 요청을 또 쏘고, 그 루프가 한도를 더 태운다(#258 증폭기).
        # 대기(상한 1h)를 여기서 소화한 뒤 종료 → 다음 재시작이 성공한다.
        from trade.tg_entities import startup_failure_note
        log.error("flood wait at startup: %s", e)
        _notify(
            "⚠️ <b>나쁜양파 리스너 — 요청 제한</b>\n"

...
# 회계 라인·대조계정·합계·주석 — 제품이 아닌데 이름이 한글이라 품목으로 오매핑되던 행
# (audit '이름 의심' 클래스, 사용자 2026-06-18). '빈칸 > 오매핑' 정책 — **명백한 비제품만**
# 차단하고 상품/제품/용역/임대/기타 '매출' 카테고리는 정상 매출구성이라 보존(공백 제거 후
# 부분일치). 손익라인·매출 대조차감·연결내부거래·비율주석·총계 leak 만 표적.
_NON_PRODUCT_SUBSTR = (
    # 손익 라인 (제품 아님)
    "영업손익", "영업이익", "영업손실", "당기순손익", "당기순이익", "당기순손실",
    "당기손익", "주당",      # bare '주당' = EPS 전 변형(주당순이익/주당이익/희석주당 — 2026-06-19 audit 67건)
    "계속사업손익", "중단사업손익", "포괄손익", "법인세",
    # 매출 대조/차감 계정
    "매출에누리", "매출할인", "매출환입", "에누리", "매출차감", "차감매출", "매출조정",
    # 연결/내부거래 제거
    "내부거래", "연결조정", "연결제거", "내부매출",
    # 비율·발생여부 등 표 부속 텍스트
    "대비비율", "대비비중", "발생여부", "매출액비율",
    # 헤더/총계가 셀로 샌 것
    "총매출", "전체매출", "매출액계", "매출계", "매출총계", "수익(매출액)",
    "매출액(백만원)", "매출액(비율)", "매출액(비중)",
)


def _is_non_product_label(s: str) -> bool:
    """제품 아닌 회계/대조/총계/주석 행 라벨인가 (오매핑 차단). 순수·단위테스트."""
    return any(k in (s or "").replace(" ", "") for k in _NON_PRODUCT_SUBSTR)
...
def _is_texty(s: str) -> bool:
    ...
    if not s or s in _TOTAL_NAMES or s.replace(" ", "") in _TOTAL_NAMES_NS:
        return False
    # 회계 라인·대조계정·총계 leak 오매핑 차단 ('빈칸 > 오매핑', 사용자 2026-06-18 audit).
    if _is_non_product_label(s):
        return False
...
_ACCT_KW = ("재무제", "내용연수", "회계정책", "회계정", "주석", "상각방법",
            ...
            # 연구개발비(R&D 비용) 표 — '연구개발비/매출액 비율' 이 '매출액'+'%' 신호로
            # 오선택돼 진짜 매출구성표를 가렸다(parse_empty 002710 류, 2026-06-20). R&D
            # 비용표는 매출구성이 아니므로 제외 → 차순위 매출표가 선택되게.
            "연구개발비", "연구개발비용", "경상연구개발")
...
def collect_score(rows: list[list[str]]) -> int:
    ...
    if "매출" not in joined and any(k in joined for k in ("매입액", "매입처", "매입유형")):
        return 0
    # 목차(TOC)·섹션 목록 등 '데이터 없는' 표 — 매출구성표는 반드시 금액(콤마 천단위)
    # 또는 비중(%) 셀을 갖는다. 본문에 콤마금액·% 신호가 0 이면 매출구성표가 아님
    # (건설 002290 류: '사업의 내용' 목차 표가 '매출/사업' 키워드로 오선택, 2026-06-20).
    # 회귀 0 — 현 working 추출은 _to_amount(콤마 필수)·_to_pct(% 또는 비중컬럼)에
    # 의존하므로, 콤마금액·% 가 전혀 없는 표는 애초에 제품을 못 뽑던 표.
    if not re.search(r"\d{1,3}(?:,\d{3})+|\d{1,3}(?:\.\d+)?\s*%",

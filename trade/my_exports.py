...
.my-sum{list-style:none;padding:13px 16px;
  display:flex;flex-direction:column;gap:7px}
.my-sum::-webkit-details-marker{display:none}
details.my-card > .my-sum{cursor:pointer}
details.my-card > .my-sum::after{content:"▸ 펼치기(차트·월별)";color:var(--muted);font-size:11px;margin-top:2px}
...
def _card_html(r: dict, hist: list[dict], media_prefix: str) -> str:
    ...
    return card_html("my", summary, detail)
...
def render_html(conn: sqlite3.Connection, media_url_prefix: str = "../") -> str:
    ...
    return (
        ...
        + asof_footer(len(items), "품목", latest or None,
                      max_ingest_iso(conn, "my_exports"))

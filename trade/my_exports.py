"""말레이시아 수출 데이터 (나쁜양파/Badonion) — 파서 + 저장 + 대시보드 (사용자 2026-07-26).

나쁜양파(t.me/Badonions) 채널이 발행하는 *말레이시아* 관세 수출 통계 메시지는
`trade/tw_exports.py`(대만)·`trade/cn_exports.py`(중국)·`trade/jp2_exports.py`
(일본, 나쁜양파 2차 소스)와 완전히 동일한 포맷 — 같은 채널, 같은 구조,
마커 단어("말레이시아")만 다르다. KR·JP(BeOn)·TW·CN·JP2·TH 파서가 모두 None 을 낸 뒤의
7차 fallback:

  ingest_inbox._ingest_group → (KR·JP(BeOn)·TW·CN·JP2·TH 전부 None) →
  parse_my_export() → my.db upsert
  trade.dashboard main() (sibling-regen) → regenerate() → ~/.trade/dashboard/my.html

저장은 별도 my.db(store.db·jp.db·tw.db·cn.db·jp2.db·th.db 전부 무영향). TW/CN/JP2 와 동일하게 메시지
...
반영해도 무방(사용자 지침). 향후 채널에 새로 올라오는 말레이시아 수출 메시지도
...
_MARKER_RE = re.compile(r"\d+\s*월\s*수출\s*말레이시아")
...
def parse_my_export(caption: str) -> dict | None:
    """나쁜양파 말레이시아 수출 메시지 → dict(item, companies, months[]).
    마커/품목/월라인 없으면 None(진짜 미인식 — KR·JP(BeOn)·TW·CN·JP2·TH 파서 다음
    7차 fallback이라 여기서 None 이면 그냥 unparseable로 버려짐)."""
...
# ─────────────────────────────────────────────────────────────────────────
# Storage (별도 my.db — store.db·jp.db·tw.db·cn.db·jp2.db·th.db 전부 무영향)
# ─────────────────────────────────────────────────────────────────────────
_SCHEMA = """
...
def open_my_db(path: str | Path) -> sqlite3.Connection:
...
def upsert_my(conn: sqlite3.Connection, item: str, month_row: dict, *,
    ...
    if not item or not month:
        return False
    ex = conn.execute(
        "SELECT * FROM my_exports WHERE item=? AND month=?",
        (item, month)).fetchone()
    ...
    conn.execute(
        f"INSERT OR REPLACE INTO my_exports ({','.join(_COLS)}) "
        f"VALUES ({placeholders})", merged)
    return True


def list_my(conn: sqlite3.Connection) -> list[dict]:
    """대시보드용 — 품목별 최신월 1행."""
    rows = conn.execute(
        "SELECT * FROM my_exports t WHERE month = "
        "(SELECT MAX(month) FROM my_exports WHERE item = t.item) "
...
def history(conn: sqlite3.Connection, item: str) -> list[dict]:
    """품목의 월별 전체 이력(오름차순)."""
    rows = conn.execute(
        "SELECT * FROM my_exports WHERE item=? ORDER BY month ASC",
        (item,)).fetchall()
    return [dict(r) for r in rows]
...
def ingest(conn: sqlite3.Connection, caption: str, *, source_message_id=None,
    ...
    parsed = parse_my_export(caption)
    if parsed is None:
        return False
    ...
    for mrow in parsed["months"]:
        ok = upsert_my(conn, parsed["item"], mrow, companies=companies_str,
...
# ─────────────────────────────────────────────────────────────────────────
# Dashboard render (my.html) — tw_exports.py/jp2_exports.py 톤·CSS 미러
# ─────────────────────────────────────────────────────────────────────────
_CSS = """
...
.my-card{background:var(--card);border:1px solid var(--border);border-radius:10px;overflow:hidden}
.my-card[open]{border-color:var(--accent)}
.my-sum{list-style:none;cursor:pointer;padding:13px 16px;
  display:flex;flex-direction:column;gap:7px}
.my-sum::-webkit-details-marker{display:none}
.my-sum::after{content:"▸ 펼치기(차트·월별)";color:var(--muted);font-size:11px;margin-top:2px}
.my-card[open] .my-sum::after{content:"▾ 접기"}
.my-hd{display:flex;align-items:baseline;gap:8px;flex-wrap:wrap}
.my-item{font-weight:680;font-size:15px;color:var(--item)}
.my-mo{font-size:12px;color:var(--muted);margin-left:auto}
.my-metric{display:flex;align-items:baseline;gap:8px;font-size:13px;flex-wrap:wrap}
.my-mlabel{color:var(--muted);min-width:62px}
.my-mval{font-weight:600;color:var(--text)}
.my-delta{font-size:12px}
.up{color:#e5484d}.down{color:#3b82f6}.flat{color:var(--muted)}
.my-co{font-size:11.5px;color:var(--muted);line-height:1.6}
.my-detail{padding:0 16px 14px;border-top:1px solid var(--border)}
.my-chart{margin:12px 0;border:1px solid var(--chartbd);border-radius:8px;overflow:hidden}
.my-chart img{display:block;width:100%;height:auto}
.my-htbl{width:100%;border-collapse:collapse;font-size:12px}
.my-htbl th,.my-htbl td{padding:4px 8px;text-align:right;border-bottom:1px solid var(--row)}
.my-htbl th{color:var(--muted);font-weight:500}
.my-htbl td:first-child,.my-htbl th:first-child{text-align:left;color:var(--text)}
...
def _delta_str(yoy, mom) -> str:
    ...
    for tag, v in (("YoY", yoy), ("MoM", mom)):
        ...
        parts.append(f'<span class="my-delta {cls}">{tag} {arr}{v:+.1f}%</span>')
...
def _hist_table(hist: list[dict]) -> str:
    ...
    return ('<table class="my-htbl"><tr><th>월</th><th>수출액(USD M$)</th>'
...
def _card_html(r: dict, hist: list[dict], media_prefix: str) -> str:
    ...
    co_html = (f'<div class="my-co">🏭 {_html.escape(co)}</div>' if co else "")
    summary = [f'<summary class="my-sum">'
               f'<div class="my-hd"><span class="my-item">{item}</span>'
               f'<span class="my-mo">📅 {mo}</span></div>']
    ev = r.get("export_value_musd")
    if ev is not None:
        summary.append(
            f'<div class="my-metric"><span class="my-mlabel">💰 수출액</span>'
            f'<span class="my-mval">${ev:,.1f}M</span>'
    ...
    if chart:
        detail.append(f'<div class="my-chart"><img loading="lazy" '
                      f'src="{_html.escape(media_prefix + chart)}" alt="{item}"></div>')
    detail.append(_hist_table(hist))
    detail_html = (f'<div class="my-detail">{"".join(d for d in detail if d)}</div>'
                   if any(detail) else "")
    return f'<details class="my-card">{"".join(summary)}{detail_html}</details>'
...
def render_html(conn: sqlite3.Connection, media_url_prefix: str = "../") -> str:
    ...
    if items:
        ...
        body = ('<div class="empty">아직 수집된 말레이시아 수출 데이터(나쁜양파)가 없습니다. '
                '나쁜양파 채널의 "N월 수출 말레이시아" 메시지를 포워드하면 표시됩니다.</div>')
    return (
        "<!doctype html><html lang='ko'><head><meta charset='UTF-8'>"
        "<meta name='viewport' content='width=device-width,initial-scale=1'>"
        "<title>🐯 말레이시아 수출 데이터 (나쁜양파)</title>"
        f"<style>{_CSS}</style></head><body><div class='wrap'>"
        '<div class="nav"><a href="./">← 수출입 대시보드</a></div>'
        "<h1>🐯 말레이시아 수출 데이터 (나쁜양파)</h1>"
        f'<p class="sub">출처 나쁜양파(관세 말레이시아) · 품목별 월 수출액(USD M$) · '
        f'{len(items)}개 품목'
...
def regenerate(my_db_path: str | Path, out_path: str | Path,
               media_url_prefix: str = "../") -> None:
    """my.db → my.html. 데이터 없어도 빈 페이지 생성(링크 404 방지)."""
    conn = open_my_db(my_db_path)
    try:

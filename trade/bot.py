...
• <b>📈 산업트렌드</b> — 20산업+하위(MTI6): 분류보드(초고성장/턴어라운드/부진·가속/둔화·📥수입모멘텀) + 품목 카드(수출·MA·YoY막대·TTM·8지표·해석·원자료) + 하위품목 TOP(급등률·급증액 MoM·수출≥2억) + 🔍추가신호 + 🚀🔄산업내부동학 + 🟢잠정속보(관세청10일단위·확정보다선행·capex선행) + 🗄아카이브 + 🔗export. HSK-MTI 기준
...
🔴 확정 D+N 지연 (예정일 초과)
🆕 NEW — 카드: 7일내 게시 / 헤더: 7일내 첫 등장 품목·회사

<b>7. 부가 기능</b>
• 검색: 품목/회사/국가 부분일치 (회사명 정확 일치 시 회사 뷰 자동 좁힘)
• 칩 필터: 수출/수입, 잠정/확정, 🆕 신규(최근 7일 게시 카드만)
• 📥 CSV — 현재 필터 결과 풀필드 다운로드 (전 메타데이터 + media_urls 절대경로)
...
<i>최종 갱신: 2026-06-03 — 산업트렌드 🟢잠정 속보(관세청 10일 단위·확정보다 선행)</i>
"""

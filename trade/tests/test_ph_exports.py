"""필리핀 수출 데이터(나쁜양파) 파서 + 저장 회귀 (사용자 2026-07-26).

tw_exports/cn_exports/jp2_exports 와 완전히 동일한 채널·포맷(마커 단어만
"필리핀")이므로 테스트 구조도 그대로 미러링. 회사 링크 포맷(마크다운 링크 vs
...
_REAL_FULL = (
    ...
    "관련기업: #Sumitomo Electric Industries\n\n"
    "최근 추이 (단위: USD M$)\n"
...
class ParseTests(unittest.TestCase):
    def test_full_message_headline_and_history_plain_hashtag(self):
        p = ph.parse_ph_export(_REAL_FULL)
        self.assertIsNotNone(p)
        self.assertEqual(p["item"], "동축케이블 및 동축 전기도체")
        self.assertEqual(p["companies"], ["Sumitomo Electric Industries"])
        self.assertEqual(len(p["months"]), 3)  # 헤드라인 1 + 히스토리 2
        self.assertEqual(p["months"][0], {
            "month": "2026-05", "export_value_musd": 12.2,
            "export_yoy": 219.5, "export_mom": 96.2,
        })
        self.assertEqual(p["months"][-1]["month"], "2026-03")

    def test_markdown_link_companies(self):
        p = ph.parse_ph_export(_MARKDOWN_LINK_CO)
    ...
    def test_multi_plain_hashtag_companies_split_correctly(self):
        p = ph.parse_ph_export(_MULTI_PLAIN_CO)
        self.assertEqual(p["companies"], ["Co One", "Co Two", "Co Three"])

    def test_no_company_line_graceful(self):
        p = ph.parse_ph_export(_NO_COMPANY)
        self.assertIsNotNone(p)
        self.assertEqual(p["companies"], [])
        self.assertEqual(len(p["months"]), 1)

    def test_other_source_captions_return_none(self):
        # 8차 폴백이므로 KR/JP(BeOn)/TW/CN/JP2/TH/MY 포맷은 절대 PH 로 오인되면 안 됨.
        self.assertIsNone(ph.parse_ph_export(_JP1_CAPTION))
        self.assertIsNone(ph.parse_ph_export(_KR_CAPTION))
        self.assertIsNone(ph.parse_ph_export(_TW_CAPTION))
        self.assertIsNone(ph.parse_ph_export(_CN_CAPTION))
        self.assertIsNone(ph.parse_ph_export(_JP2_CAPTION))
        self.assertIsNone(ph.parse_ph_export(_MX_CAPTION))
        self.assertIsNone(ph.parse_ph_export(""))
        self.assertIsNone(ph.parse_ph_export("그냥 잡담 메시지"))

    def test_no_month_line_returns_none(self):
        self.assertIsNone(ph.parse_ph_export("🇵🇭 05월 수출 필리핀\n\n▶️ 품목"))


class StorageTests(unittest.TestCase):
    def setUp(self):
        self.tmp = tempfile.TemporaryDirectory()
        self.db_path = Path(self.tmp.name) / "ph.db"

    def tearDown(self):
        self.tmp.cleanup()

    def test_ingest_stores_all_months_and_latest_snapshot(self):
        conn = ph.open_ph_db(self.db_path)
        ok = ph.ingest(conn, _REAL_FULL, source_message_id=1,
                        posted_at="2026-07-26T07:03:00Z",
                        media_paths=["2026-07-26/abc.jpg"])
        self.assertTrue(ok)
        latest = ph.list_ph(conn)
        self.assertEqual(len(latest), 1)
        self.assertEqual(latest[0]["month"], "2026-05")
        self.assertEqual(latest[0]["export_value_musd"], 12.2)
        self.assertIn("Sumitomo Electric Industries", latest[0]["companies"])
        hist = ph.history(conn, "동축케이블 및 동축 전기도체")
    ...
    def test_reingest_preserves_media_when_new_media_absent(self):
        conn = ph.open_ph_db(self.db_path)
        ph.ingest(conn, _REAL_FULL, source_message_id=1,
    ...
    def test_other_source_caption_not_ingested(self):
        conn = ph.open_ph_db(self.db_path)
        self.assertFalse(ph.ingest(conn, _JP1_CAPTION))
        self.assertFalse(ph.ingest(conn, _TW_CAPTION))
        self.assertFalse(ph.ingest(conn, _CN_CAPTION))
        self.assertFalse(ph.ingest(conn, _JP2_CAPTION))
        self.assertFalse(ph.ingest(conn, _MX_CAPTION))
        self.assertEqual(ph.list_ph(conn), [])

    def test_multiple_items_independent(self):
        conn = ph.open_ph_db(self.db_path)
        ph.ingest(conn, _REAL_FULL, source_message_id=1, posted_at="t")
        ph.ingest(conn, _MARKDOWN_LINK_CO, source_message_id=2, posted_at="t")
        latest = ph.list_ph(conn)
...
class RenderTests(unittest.TestCase):
    ...
    def test_render_html_with_data(self):
        ...
        ph.ingest(conn, _REAL_FULL, source_message_id=1, posted_at="t",
                  media_paths=["2026-07-26/abc.jpg"])
        html = ph.render_html(conn)
        self.assertIn("필리핀 수출 데이터", html)
        self.assertIn("12.2", html)
        self.assertIn("Sumitomo Electric Industries", html)
        self.assertIn("ph-card", html)
        self.assertIn("2026-07-26/abc.jpg", html)

    def test_render_html_empty_graceful(self):
        conn = ph.open_ph_db(self.db_path)
        html = ph.render_html(conn)
        self.assertIn("아직 수집된 필리핀 수출 데이터(나쁜양파)가 없습니다", html)
        self.assertNotIn('<details class="ph-card"', html)

    def test_regenerate_writes_file(self):
        out = Path(self.tmp.name) / "ph.html"
        ph.regenerate(self.db_path, out)
        self.assertTrue(out.exists())
        self.assertIn("필리핀 수출 데이터", out.read_text(encoding="utf-8"))

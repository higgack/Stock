"""필리핀 수출 데이터 (나쁜양파/Badonion) — 파서 + 저장 + 대시보드 (사용자 2026-07-26).

나쁜양파(t.me/Badonions) 채널이 발행하는 *필리핀* 관세 수출 통계 메시지는
`trade/tw_exports.py`(대만)·`trade/cn_exports.py`(중국)·`trade/jp2_exports.py`
(일본, 나쁜양파 2차 소스)와 완전히 동일한 포맷 — 같은 채널, 같은 구조,
마커 단어("필리핀")만 다르다. KR·JP(BeOn)·TW·CN·JP2·TH·MY 파서가 모두 None 을 낸 뒤의
8차 fallback:

  ingest_inbox._ingest_group → (KR·JP(BeOn)·TW·CN·JP2·TH·MY 전부 None) →
  parse_ph_export() → ph.db upsert
  trade.dashboard main() (sibling-regen) → regenerate() → ~/.trade/dashboard/ph.html

저장은 별도 ph.db(store.db·jp.db·tw.db·cn.db·jp2.db·th.db·my.db 전부 무영향). TW/CN/JP2 와 동일하게 메시지
...
반영해도 무방(사용자 지침). 향후 채널에 새로 올라오는 필리핀 수출 메시지도
...
2026-07-26 스크린샷 기반):
    ...
    **26년05월: $12.2M  (+219.5% YoY)  (+96.2% MoM)**

    관련기업: #Sumitomo Electric Industries #Furukawa Electric

    최근 추이 (단위: USD M$)
    26년04월: $6.2M  (+93.9% YoY)  (-11.1% MoM)
    26년03월: $7.0M  (+49.6% YoY)  (+53.4% MoM)
"""
...
_MARKER_RE = re.compile(r"\d+\s*월\s*수출\s*필리핀")
...
def parse_ph_export(caption: str) -> dict | None:
    """나쁜양파 필리핀 수출 메시지 → dict(item, companies, months[]).
    마커/품목/월라인 없으면 None(진짜 미인식 — KR·JP(BeOn)·TW·CN·JP2·TH·MY 파서 다음
    8차 fallback이라 여기서 None 이면 그냥 unparseable로 버려짐)."""
...
# ─────────────────────────────────────────────────────────────────────────
# Storage (별도 ph.db — store.db·jp.db·tw.db·cn.db·jp2.db·th.db·my.db 전부 무영향)
# ─────────────────────────────────────────────────────────────────────────
_SCHEMA = """
CREATE TABLE IF NOT EXISTS ph_exports (
  item TEXT NOT NULL,
...
def open_ph_db(path: str | Path) -> sqlite3.Connection:
...
def upsert_ph(conn: sqlite3.Connection, item: str, month_row: dict, *,
    ...
    if not item or not month:
        return False
    ex = conn.execute(
        "SELECT * FROM ph_exports WHERE item=? AND month=?",
        (item, month)).fetchone()
    ...
    conn.execute(
        f"INSERT OR REPLACE INTO ph_exports ({','.join(_COLS)}) "
        f"VALUES ({placeholders})", merged)
    return True


def list_ph(conn: sqlite3.Connection) -> list[dict]:
    """대시보드용 — 품목별 최신월 1행."""
    rows = conn.execute(
        "SELECT * FROM ph_exports t WHERE month = "
        "(SELECT MAX(month) FROM ph_exports WHERE item = t.item) "
...
def history(conn: sqlite3.Connection, item: str) -> list[dict]:
    """품목의 월별 전체 이력(오름차순)."""
    rows = conn.execute(
        "SELECT * FROM ph_exports WHERE item=? ORDER BY month ASC",
        (item,)).fetchall()
    return [dict(r) for r in rows]
...
def ingest(conn: sqlite3.Connection, caption: str, *, source_message_id=None,
    ...
    parsed = parse_ph_export(caption)
    if parsed is None:
        return False
    ...
    for mrow in parsed["months"]:
        ok = upsert_ph(conn, parsed["item"], mrow, companies=companies_str,
...
# ─────────────────────────────────────────────────────────────────────────
# Dashboard render (ph.html) — tw_exports.py/jp2_exports.py 톤·CSS 미러
# ─────────────────────────────────────────────────────────────────────────
_CSS = """
...
.ph-card{background:var(--card);border:1px solid var(--border);border-radius:10px;overflow:hidden}
.ph-card[open]{border-color:var(--accent)}
.ph-sum{list-style:none;cursor:pointer;padding:13px 16px;
  display:flex;flex-direction:column;gap:7px}
.ph-sum::-webkit-details-marker{display:none}
.ph-sum::after{content:"▸ 펼치기(차트·월별)";color:var(--muted);font-size:11px;margin-top:2px}
.ph-card[open] .ph-sum::after{content:"▾ 접기"}
.ph-hd{display:flex;align-items:baseline;gap:8px;flex-wrap:wrap}
.ph-item{font-weight:680;font-size:15px;color:var(--item)}
.ph-mo{font-size:12px;color:var(--muted);margin-left:auto}
.ph-metric{display:flex;align-items:baseline;gap:8px;font-size:13px;flex-wrap:wrap}
.ph-mlabel{color:var(--muted);min-width:62px}
.ph-mval{font-weight:600;color:var(--text)}
.ph-delta{font-size:12px}
.up{color:#e5484d}.down{color:#3b82f6}.flat{color:var(--muted)}
.ph-co{font-size:11.5px;color:var(--muted);line-height:1.6}
.ph-detail{padding:0 16px 14px;border-top:1px solid var(--border)}
.ph-chart{margin:12px 0;border:1px solid var(--chartbd);border-radius:8px;overflow:hidden}
.ph-chart img{display:block;width:100%;height:auto}
.ph-htbl{width:100%;border-collapse:collapse;font-size:12px}
.ph-htbl th,.ph-htbl td{padding:4px 8px;text-align:right;border-bottom:1px solid var(--row)}
.ph-htbl th{color:var(--muted);font-weight:500}
.ph-htbl td:first-child,.ph-htbl th:first-child{text-align:left;color:var(--text)}
...
def _delta_str(yoy, mom) -> str:
    ...
    for tag, v in (("YoY", yoy), ("MoM", mom)):
        ...
        parts.append(f'<span class="ph-delta {cls}">{tag} {arr}{v:+.1f}%</span>')
...
def _hist_table(hist: list[dict]) -> str:
    ...
    return ('<table class="ph-htbl"><tr><th>월</th><th>수출액(USD M$)</th>'
...
def _card_html(r: dict, hist: list[dict], media_prefix: str) -> str:
    ...
    co_html = (f'<div class="ph-co">🏭 {_html.escape(co)}</div>' if co else "")
    summary = [f'<summary class="ph-sum">'
               f'<div class="ph-hd"><span class="ph-item">{item}</span>'
               f'<span class="ph-mo">📅 {mo}</span></div>']
    ev = r.get("export_value_musd")
    if ev is not None:
        summary.append(
            f'<div class="ph-metric"><span class="ph-mlabel">💰 수출액</span>'
            f'<span class="ph-mval">${ev:,.1f}M</span>'
    ...
    if chart:
        detail.append(f'<div class="ph-chart"><img loading="lazy" '
                      f'src="{_html.escape(media_prefix + chart)}" alt="{item}"></div>')
    detail.append(_hist_table(hist))
    detail_html = (f'<div class="ph-detail">{"".join(d for d in detail if d)}</div>'
                   if any(detail) else "")
    return f'<details class="ph-card">{"".join(summary)}{detail_html}</details>'
...
def render_html(conn: sqlite3.Connection, media_url_prefix: str = "../") -> str:
    ...
    if items:
        ...
        body = ('<div class="empty">아직 수집된 필리핀 수출 데이터(나쁜양파)가 없습니다. '
                '나쁜양파 채널의 "N월 수출 필리핀" 메시지를 포워드하면 표시됩니다.</div>')
    return (
        "<!doctype html><html lang='ko'><head><meta charset='UTF-8'>"
        "<meta name='viewport' content='width=device-width,initial-scale=1'>"
        "<title>🥭 필리핀 수출 데이터 (나쁜양파)</title>"
        f"<style>{_CSS}</style></head><body><div class='wrap'>"
        '<div class="nav"><a href="./">← 수출입 대시보드</a></div>'
        "<h1>🥭 필리핀 수출 데이터 (나쁜양파)</h1>"
        f'<p class="sub">출처 나쁜양파(관세 필리핀) · 품목별 월 수출액(USD M$) · '
        f'{len(items)}개 품목'
...
def regenerate(ph_db_path: str | Path, out_path: str | Path,
               media_url_prefix: str = "../") -> None:
    """ph.db → ph.html. 데이터 없어도 빈 페이지 생성(링크 404 방지)."""
    conn = open_ph_db(ph_db_path)
    try:

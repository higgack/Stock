...
# ⚠️ 원천이 **두 레이아웃을 오간다**(2026-08-28 실측): 6월분은
# `이름 (티커) 일본 수출 Update` 한 줄 + 다음 줄 연월, 7월분은
# `이름 (티커)` / `일본 수출` / `26년 7월 Update` 세 줄이다. 옛 판은 1줄만
# 받아 7월분이 통째로 드랍됐다(저장·미매칭 알림 둘 다 없음 — 이 파서가 곧
# 관련성 필터라 그렇다, #83). 형제(mys·cns/cni)가 이미 쓰던 유연 형태로
# 수렴시킨다 — 티커 자리에서만 줄바꿈을 허용하고 `Update` 는 양쪽 선택.
...
# ⚠️ 원천이 `동시상관`·`방향 일치율` 을 추가하자(2026-08-28) 품목 슬롯이
# 그 첫 줄을 집어 카드에 `동시상관: 0.96` 이 품목으로 실렸다 — 형제(mys·
# cns/cni)는 이미 걸러내고 있었다(#27 형제 설정 불일치). 불릿(`- `/`* `)도
# 각주지 품목이 아니다.
_SKIP_LINE = re.compile(
    r"(수출액|단가|YoY|상관|방향\s*일치율|매출|https?://|맵핑|Update"
    r"|일본\s*수출|^\s*[-•*]|^\s*\d{2}\s*년)", re.I)
...
def _card_html(r: dict, hist: list[dict], media_prefix: str,
    ...
    name = _sl.linked_name(raw_name, _sl.lookup_href(
        raw_tk, raw_name, local="JP", kr_master=kr_master))
...
def render_html(conn: sqlite3.Connection, *, media_url_prefix: str = "../") -> str:
    ...
    # ⚠️ 실측한 건 **말레이시아 보드의 `삼성SDI (006400)`** 하나다 — 이 보드에서
    # 봤다는 뜻이 아니다(#165). 전시장 동일하게 걸 뿐 한 보드용 게이트를 두지

...
종료코드는 0 이상 없음 · 1 문제(❌) · 2 판정 불가(❓)다. 버림을 적지 않거나 재게시 보증을
모르는(시작 줄에 `relay_vouch=on` 이 없는) **옛 판** 봇이 돌면 증상이 없어도 2 다 — 그래서
다시 포워드는 `bot_health && 백필` 로 이으면 봇이 새 판으로 재시작되기 전엔 나가지 않는다
(실수 #409·#411).

나쁜양파가 다른 채널에서 퍼 온 **재게시** 글은 텔레그램이 원래 출처를 달아 보내, 봇의 출처
목록(`TRADE_SOURCE_ORIGIN`)에 안 걸린다(2026-09-25 27건이 그렇게 버려졌다 — 실수 #411).
그 채널을 목록에 더하지 않는다: 나쁜양파 릴레이(리스너·동기화)가 관련 글을 포워드하기
**전에** 그 글의 (원래 채널, 원래 글번호)를 `~/.trade/relay_origins.json` 에 보증하고, 봇은
목록에 없는 포워드라도 보증된 **그 글**만 받는다(같은 채널의 다른 글 — BeOn 이 되포워드한
무관 글 등 — 은 계속 버린다). 보증을 못 쓰면 릴레이는 그 글을 포워드하지 않고 알린다(주기
동기화가 다시 시도). 버림 줄의 `vouch=` 칸(`miss`·`unreadable`·`n/a`)과 `origin_title` 이 그
글이 어느 채널의 무엇이고 왜 못 받았는지 말하고, 보증으로 받은 글은 `accepted …
reason=relay_vouch` 줄로 남는다.
...
릴레이가 **보증한** 재게시 글을 보증 뒤에 버린 것도 알린다(보증 경로가 깨졌다 — 실수
#411). 보증 없는 다른 출처 포워드의 버림은 알리지 않는다 — BeOn 이 되포워드한 남의 글이
대부분이라서다. ⚠️ 그래서 보증하기 **전의** 판이 포워드한 재게시 글(다른 채널에서 퍼 온 글
— 텔레그램은 원래 출처를 단다)을 버린 손실은 알림에 안 잡힌다. 진단의 ⚠️ 메모가 그
출처·건수와 가르는 명령(`--find` 로 그 글이 여전히 to-forward 인가)을 적는다 — 그 글은
보증하는 판의 릴레이로 다시 포워드하면 받는다. 같은 사실은 한 번만 알린다 — **전달된** 알림의

...
# ── 큐레이션 테마 행 (사용자 2026-06-19 'MTI 품목표에 없는 카테고리는 테마로').
# 무역협회 HSK-MTI 1,294품목에 **행 자체가 없는** 카테고리(피부과 미용기기·MLCC·
# 건기식·의료기기·반도체 소재/테스트 등) — 알림엔 자주 나오나 붙일 품목이 없어
# 미매칭으로 남던 것. (테마명, 산업, 기업들) 큐레이션. 기업은 채널 알림에서
# 검증된 종목만(환각 0). 정확성 > 커버리지 — 확신 없는 종목 미수록. 레퍼런스북이
# '🏷️ 테마' 배지로 MTI 행과 구분 + surfaced 처리(미매칭 패널서 자동 제거). ──────
_THEME_ROWS: list[tuple[str, str, tuple[str, ...]]] = [
    ("피부과·미용 의료기기 (HIFU·레이저·RF)", "바이오헬스",
     ("클래시스", "비올", "원텍", "이루다", "루트로닉", "제이시스메디칼",
      "텐텍", "하이로닉", "레이저옵텍")),
    ("MLCC·콘덴서 (적층세라믹·필름)", "전기전자",
     ("삼성전기", "삼화콘덴서", "아모텍", "아바코", "아바텍", "삼영", "삼화전기")),
    ("건강기능식품 (펩타이드·식이보충)", "바이오헬스",
     ("케어젠", "노바렉스", "서흥", "코스맥스엔비티")),
    ("진단·치료 의료기기 (스텐트·지혈·창상·체성분·혈당)", "바이오헬스",
     ("엠아이텍", "넥스트바이오메디컬", "티앤엘", "인바디", "셀바스헬스케어",
      "아이센스", "오스테오닉")),
    ("반도체 소재 (SiC·쿼츠·펠리클·마스크·PR)", "반도체",
     ("티씨케이", "하나머티리얼즈", "케이엔제이", "원익QnC", "비씨엔씨",
      "월덱스", "에프에스티", "에스앤에스텍", "동진쎄미켐", "켐트로닉스")),
    ("반도체 테스트 (프로브카드·소켓)", "반도체",
     ("리노공업", "티에스이", "ISC", "마이크로컨텍솔", "마이크로투나노",
      "타이거일렉", "샘씨엔에스", "한솔테크닉스")),
    ("카메라 모듈·OIS 액추에이터", "전기전자",
...
def theme_rows() -> list[dict]:
    """큐레이션 테마 행 [{name, industry, companies(표기통일·중복제거)}] — MTI
    품목표에 없는 카테고리 보강(사용자 2026-06-19). 순수."""
    return [{"name": nm, "industry": ind, "companies": dedup_companies(cos)}
            for nm, ind, cos in _THEME_ROWS]

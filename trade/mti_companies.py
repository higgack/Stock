...
_MAP: list[tuple[tuple[str, ...], tuple[str, ...]]] = [
    ...
    # 큐레이션 후보 알림(2026-07-21, 최신월 34.3억$) — DART 자동추천 없어
    # 수기 조사(사용자 확인 후 등재): 프로세서(AP)·MCU 전문 팹리스 도메인 일치.
    (("프로세서와컨트롤러",), ("삼성전자", "어보브반도체", "텔레칩스")),
    ...
    # ── 보고서 매핑 _MAP (실제 MTI품목명=reinforce 키와 동기화, 사용자 2026-06-20).
    #    companies_for 첫매치라 끝에 추가 — 기존 우선·신규 품목만 보강.
    (("D램",), ("SK하이닉스",)),
    (("OLED",), ("이엠앤아이", "아바코", "덕산네오룩스")),
    (("TV카메라",), ("아이디스", "팅크웨어")),
    (("X선및방사선기기",), ("레이", "바텍")),
    (("건전지",), ("비츠로셀",)),
    (("계전기",), ("LS이모빌리티솔루션", "와이엠텍")),
    (("고무플라스틱가공기계",), ("우진플라임",)),
    (("고압케이블",), ("티씨머티리얼즈", "일진전기")),
    (("고정식축전기",), ("다이요유덴", "삼성전기", "삼화콘덴서", "아모텍", "삼화전기")),
    (("골프용품",), ("골프존",)),
    (("공기조절기",), ("SNT에너지", "한텍")),
    (("과산화수소",), ("OCI", "한솔케미칼")),
    (("광케이블",), ("가온전선", "대한광통신", "머큐리")),
    (("기타집적회로반도체",), ("삼성전자", "제주반도체", "네페스")),
    (("기타개별소자반도체",), ("신성이엔지", "한화솔루션")),
    (("기타건설중장비",), ("HD건설기계", "볼보그룹코리아", "포스코인터내셔널")),
    (("기타계측기",), ("고영", "인텍플러스", "펨트론")),
    (("기타광학기기부품",), ("LS전선",)),
    (("기타기계류",), ("제이엔케이히터",)),
    (("기타농산가공품",), ("쎌바이오텍", "케어젠", "코스맥스엔비티")),
    (("기타무선통신기기부품",), ("제이케이시냅스", "오이솔루션", "라이콤")),
    (("기타사진영화용재료",), ("에스앤에스텍", "동진쎄미켐")),
    (("기타산업기계",), ("에스티아이", "대보마그네틱")),
    (("기타산업기계부품",), ("GST", "유니셈", "지앤비에스 에코")),
    (("기타석유화학제품",), ("카프로", "현대코퍼레이션", "롯데정밀화학", "이수화학")),
    (("기타선박",), ("HD현대중공업", "대한조선", "삼성중공업", "케이조선", "한화오션")),
    (("기타용의전기로",), ("동일스틸럭스", "HPSP")),
    (("기타유리제품",), ("비씨엔씨", "원익QnC")),
    (("기타의료위생용품",), ("넥스트바이오메디컬",)),
    (("기타정밀화학원료",), ("솔브레인", "케이씨텍", "POSCO홀딩스", "STX", "에코프로이노베이션", "이녹스리튬", "한국유미코아", "유니드", "케이엔제이", "새빗켐")),
    (("기타정밀화학제품",), ("송원산업", "엘티씨", "선진뷰티사이언스")),
    (("기타철구조물",), ("SK오션플랜트",)),
    (("기타프린터",), ("빅솔론", "아이디피")),
    (("기타합성수지",), ("진양화학", "그래피")),
    (("기타항공기부품",), ("아스트", "켄코아에어로스페이스", "하이즈항공")),
    (("기타화장품",), ("졸스", "콜마홀딩스", "차AI헬스케어", "에스티씨라이프", "유한코스메틱", "코스나인", "넥스트아이", "에코글로우", "내츄럴엔도텍", "콜마비앤에이치", "잇츠한불", "바이오인프라생명과학", "씨엔에프", "에이피알", "예스아시아홀딩스", "이미인", "잉글우드랩", "코스메카코리아", "한국콜마", "한국화장품제조")),
    (("기타회로보호용기기",), ("ISC", "마이크로컨텍솔", "리노공업")),
    (("내연기관트랙터신차",), ("TYM",)),
    (("데이터디스플레이장치",), ("쿠콘", "코텍", "토비스")),
    (("도금강판",), ("TCC스틸",)),
    (("동박",), ("롯데에너지머티리얼즈",)),
    (("리튬이온전지",), ("삼성SDI", "LG에너지솔루션")),
    (("메이크업기초화장품",), ("졸스", "콜마홀딩스", "차AI헬스케어", "에스티씨라이프", "유한코스메틱", "코스나인", "넥스트아이", "에코글로우", "내츄럴엔도텍", "콜마비앤에이치", "잇츠한불", "나우코스", "바이오인프라생명과학", "달바글로벌", "실리콘투")),
    (("면류",), ("농심", "삼양식품", "오뚜기")),
    (("무계목강관",), ("세아제강", "넥스틸", "아주베스틸")),
    (("무기류부품",), ("LIG넥스원",)),
    (("미용및조직수복용",), ("메디톡스", "바이오플러스", "파마리서치", "원텍", "제이시스메디칼", "비올", "클래시스")),
    (("밀링공구",), ("와이지-원",)),
    (("바이오원료",), ("그린리소스", "녹십자", "바디텍메드")),
    (("바이오의약품",), ("현대약품", "서린바이오", "큐리언트", "메타케어", "대웅제약", "휴젤")),
    (("반도체제조용장비",), ("이오테크닉스", "로체시스템즈", "한미반도체", "SEMES", "피에스케이홀딩스", "제우스", "오로스테크놀로지", "원익IPS", "주성엔지니어링")),
    (("반도체제조용장비부품",), ("피에스케이", "티에스이", "티에프이", "에프에스티", "코미코", "티씨케이", "월덱스", "하나머티리얼즈", "한솔아이원스")),
    (("반창고",), ("티앤엘",)),
    (("밸브",), ("성광벤드", "태광", "하이록코리아")),
    (("분리막",), ("더블유씨피",)),
    (("비스킷",), ("오리온홀딩스", "롯데웰푸드")),
    ...
    (("소주",), ("하이트진로",)),
    (("스마트폰부품",), ("재영솔루텍",)),
    (("아라미드사",), ("도레이첨단소재", "코오롱인더스트리")),
    (("안경렌즈",), ("폴라리스세원", "인터로조")),
    (("안료",), ("씨큐브",)),
    (("알루미늄박",), ("율촌화학", "DI동일", "동원시스템즈", "롯데케미칼", "삼아알미늄")),
    (("양극재",), ("에코프로비엠", "엘앤에프", "코스모신소재", "포스코퓨처엠")),
    (("유입식변압기",), ("제룡전기", "LS ELECTRIC")),
    (("은",), ("덕산하이메탈",)),
    (("인쇄회로",), ("에이엔피", "아비코전자", "디에이피", "뉴프렉스", "두산", "대덕전자", "샘씨엔에스", "이수페타시스", "코리아써키트", "티엘비", "비에이치", "심텍", "인터플렉스")),
    (("일반수술및처치용",), ("아이센스", "리브스메드", "엠아이텍", "인바디")),
    (("자동차부품",), ("영화금속", "팬스타엔터프라이즈", "현대모비스", "성우하이텍")),
    (("전자현미경",), ("코셈", "파크시스템스")),
    (("전해액",), ("에이아이코리아", "솔브레인홀딩스")),
    (("제조담배",), ("KT&G",)),
    (("주철제관연결구류",), ("비엠티",)),
    (("집적회로반도체부품",), ("해성디에스",)),
    (("철및비합금강H형강",), ("동국제강", "현대제철")),
    (("철및비합금강용접강관대",), ("EEW KHPC", "EEW코리아", "KBI동양철관", "휴스틸")),
    (("체외진단용시약",), ("프로티아",)),
    (("축전지부품",), ("한국앤컴퍼니", "상신이디피", "신흥에스이씨", "신성에스티")),
    (("측정공구",), ("코메론",)),
    (("치과용기기및재료",), ("엘앤케이바이오", "덴티움", "디오", "메가젠임플란트", "오스템임플란트", "메타바이오메드")),
    (("커피",), ("큐로홀딩스", "SPC그룹")),
    (("타이어코드",), ("넥센타이어", "금호타이어")),
    (("탄소섬유사",), ("효성첨단소재",)),
    (("태양광모듈",), ("현대에너지솔루션",)),
    (("폴리아미드필름",), ("PI첨단소재", "넥스플렉스")),
    (("프로필렌중합체필름",), ("삼영",)),
    (("현미경",), ("토모큐브",)),
    (("화학원료",), ("CJ제일제당", "대상", "에스티팜", "하이텍팜")),
]
...
_COMPANY_CANON = {
    ...
    # 키 = 공백제거·소문자. 가격은 price_provider 가 별도 보유(공식명 기존 해석 or
    # _DIRECT_CODES 보강분 참조).
    "금호석유": "금호석유화학",                 # 통상표기 → KRX 금호석유화학(011780)
    "코오롱인더": "코오롱인더스트리",            # 약어 → 공식 코오롱인더스트리(120110)
    "한국앤컴퍼니(아트라스bx)": "한국앤컴퍼니",   # 주석표기 제거 → 한국앤컴퍼니(000240)
    "현대에너지솔루션": "HD현대에너지솔루션",     # 사명변경 → HD현대에너지솔루션(322000)
    "현대차": "현대자동차",                     # 통상약어 → 공식 현대자동차(005380)
}
...
_COMPANY_TYPO = {
    ...
    "네페스": "네패스",                  # BeOn 오타(페↔패) → KRX 네패스(033640)
}
...
_THEME_MTI_PIN: dict[str, tuple[str, ...]] = {
    ...
    # 기존 데이터 오귀착 교정 (2026-06-20 카탈로그 대조):
    # 피팅 HS 7307.29 → 카탈로그상 617110(주강)으로 떨어짐(stainless 피팅 전용 MTI 부재).
    # 의미상 철강제 관연결구류 = 692620 주철제관연결구류 핀(reinforce '주철제관연결구류'와 일치).
    "피팅 (철강제 관연결구류)": ("692620",),
    # 줄자 HS 9017.80-2000 leaf 미존재 → 901780(6자리)이 515190·732100(반도체장비!)·763610
    # 3개 MTI 로 과잉부착 → 측정공구 763610 만 핀(반도체장비 오부착 제거).
    "줄자 (척도자·길이측정구)": ("763610",),
}

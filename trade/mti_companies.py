...
_MAP: list[tuple[tuple[str, ...], tuple[str, ...]]] = [
    ...
    # ── 보고서(5월 수출 스크리닝) 품목↔기업 매핑 반영 (사용자 2026-06-20) —
    #    실존 MTI품목명 기준(새 품목명 생성 X). companies_for 첫매치라 끝에 추가 =
    #    기존 우선·신규 품목만 보강. 같은 MTI는 reinforce_approved.csv 와 동일 집합.
    (("디램모듈",), ("SK하이닉스",)),
    (("OLED", "OLED발광소재"), ("덕산네오룩스",)),
    (("TV카메라", "CCTV", "차량용영상녹화장치"), ("아이디스", "팅크웨어")),
    (("X선및방사선기기", "치과용X레이장비"), ("레이", "바텍")),
    (("리튬1차전지",), ("비츠로셀",)),
    (("EVRelays계전기전기제어용전압1000V이하",), ("LS이모빌리티솔루션", "와이엠텍")),
    (("고무플라스틱가공기계", "플라스틱성형사출기"), ("우진플라임",)),
    (("고정식축전기", "MLCC", "알루미늄전해콘덴서"), ("다이요유덴", "삼성전기", "삼화콘덴서", "아모텍", "삼화전기")),
    (("골프용품", "기타골프용품"), ("골프존",)),
    (("기타열교환기",), ("SNT에너지", "한텍")),
    (("과산화수소",), ("OCI", "한솔케미칼")),
    (("광케이블", "광섬유광케이블"), ("가온전선", "대한광통신", "머큐리")),
    (("기타집적회로반도체", "기타전자집적회로"), ("삼성전자", "제주반도체", "네페스")),
    (("기타개별소자반도체", "태양광셀"), ("신성이엔지", "한화솔루션")),
    (("기타건설중장비", "불도저앵글도저"), ("HD건설기계", "볼보그룹코리아", "포스코인터내셔널")),
    (("기타계측기", "광학식검사및계측기기"), ("고영", "인텍플러스", "펨트론")),
    (("기타광학기기부품", "광섬유ㆍ광섬유다발과광섬유케이블"), ("LS전선",)),
    (("기타기계류", "산업용가열로"), ("제이엔케이히터",)),
    (("기타농산가공품", "건강기능식품"), ("쎌바이오텍", "케어젠", "코스맥스엔비티")),
    (("기타무선통신기기부품", "그밖의무선통신용-기타송수신용장치", "통신및위성장비부분품"), ("오이솔루션", "라이콤")),
    (("기타사진영화용재료", "디스플레이용블랭크마스크", "반도체용감광액"), ("에스앤에스텍", "동진쎄미켐")),
    (("기타산업기계", "기타개별기능기계장치", "전자석탈철기"), ("에스티아이", "대보마그네틱")),
    (("기타산업기계부품", "스크러버"), ("GST", "유니셈", "지앤비에스에코")),
    (("기타석유화학제품", "알킬벤젠"), ("롯데정밀화학", "이수화학")),
    (("기타선박", "유조선및LNG운반선"), ("HD현대중공업", "대한조선", "삼성중공업", "케이조선", "한화오션")),
    (("기타용의전기로", "고압수소열처리장비"), ("HPSP",)),
    (("기타유리제품", "반도체제조용쿼츠", "반도체용쿼츠웨어"), ("비씨엔씨", "원익QnC")),
    (("기타의료위생용품",), ("넥스트바이오메디컬",)),
    (("기타정밀화학원료", "CMP슬러리", "수산화칼륨", "실리콘카바이드포커스링"), ("솔브레인", "케이씨텍", "POSCO홀딩스", "STX", "에코프로이노베이션", "이녹스리튬", "한국유미코아", "유니드", "케이엔제이", "새빗켐")),
    (("기타정밀화학제품", "기타페놀산화방지제", "반도체용신너", "화학공업조제품"), ("송원산업", "엘티씨", "선진뷰티사이언스")),
    (("기타철구조물", "기타철강구조물"), ("SK오션플랜트",)),
    (("기타프린터",), ("빅솔론", "아이디피")),
    (("기타합성수지", "폴리우레탄"), ("그래피",)),
    (("기타항공기부품", "항공기부품"), ("아스트", "켄코아에어로스페이스", "하이즈항공")),
    (("기타화장품",), ("씨엔에프", "에이피알", "예스아시아홀딩스", "이미인", "잉글우드랩", "코스메카코리아", "한국콜마", "한국화장품제조")),
    (("기타회로보호용기기", "반도체검사용핀접속기기", "인쇄회로반도체검사용소켓"), ("ISC", "마이크로컨텍솔", "리노공업")),
    (("내연기관트랙터신차", "농업용트랙터"), ("TYM",)),
    (("데이터디스플레이장치", "카지노모니터"), ("코텍", "토비스")),
    (("니켈도금",), ("TCC스틸",)),
    (("ESS에너지저장장치용", "기타축전지리튬이온배터리"), ("삼성SDI", "LG에너지솔루션")),
    (("메이크업기초화장품", "기초화장품"), ("달바글로벌", "실리콘투")),
    (("무계목강관", "철강제관및중공프로파일", "케이싱강관"), ("세아제강", "넥스틸", "아주베스틸")),
    (("무기류부품", "탄약및군수품"), ("LIG넥스원",)),
    (("미용및조직수복용", "미용필러", "의료기기부분품및부속품", "피부과용의료기기"), ("메디톡스", "바이오플러스", "파마리서치", "원텍", "제이시스메디칼", "비올", "클래시스")),
    (("밀링공구", "밀링커터"), ("와이지-원",)),
    (("바이오원료", "혈액분획물및진단용제품"), ("녹십자", "바디텍메드")),
    (("바이오의약품", "보툴리눔톡신"), ("대웅제약", "휴젤")),
    (("12인치레이저마커레이저그루빙", "FPD및반도체이송장치", "TCBONDER", "건식식각장비"), ("이오테크닉스", "로체시스템즈", "한미반도체", "SEMES", "피에스케이홀딩스", "제우스", "오로스테크놀로지", "원익IPS", "주성엔지니어링")),
    (("반도체제조용장비부품", "반도체검사장비부분품", "반도체공정용부분품", "반도체제조장비부분품"), ("티에스이", "티에프이", "에프에스티", "코미코", "티씨케이", "월덱스", "하나머티리얼즈", "한솔아이원스")),
    (("창상피복재",), ("티앤엘",)),
    (("배터리분리막",), ("더블유씨피",)),
    (("고양이사료",), ("오에스피",)),
    (("기타베이커리제품",), ("우양",)),
    (("스마트폰부품", "스마트폰부분품"), ("재영솔루텍",)),
    (("승용차용타이어",), ("금호타이어", "넥센타이어")),
    (("아라미드사", "아라미드실"), ("도레이첨단소재", "코오롱인더스트리")),
    (("안경렌즈",), ("인터로조",)),
    (("진주광택안료",), ("씨큐브",)),
    (("알루미늄박", "알루미늄파우치필름"), ("율촌화학", "DI동일", "동원시스템즈", "롯데케미칼", "삼아알미늄")),
    (("NCA니켈코발트알루미늄산화물의리튬염", "NCM니켈코발트망간산화물의리튬염"), ("에코프로비엠", "엘앤에프", "코스모신소재", "포스코퓨처엠")),
    (("유입식변압기", "초고압변압기"), ("LS ELECTRIC",)),
    (("CCL동박적층판", "기타인쇄회로기판", "패키지용회로기판"), ("두산", "대덕전자", "샘씨엔에스", "이수페타시스", "코리아써키트", "티엘비", "비에이치", "심텍", "인터플렉스")),
    (("일반수술및처치용", "마이크로톰혈당측정기", "외과수술용의료기기", "의료용스텐트"), ("아이센스", "리브스메드", "엠아이텍", "인바디")),
    (("BUMPERRAIL", "CARBODYASSY등"), ("현대모비스", "성우하이텍")),
    (("전자현미경", "정밀현미경"), ("코셈", "파크시스템스")),
    (("제조담배",), ("KT&G",)),
    (("주철제관연결구류", "배관피팅"), ("비엠티",)),
    (("집적회로반도체부품", "리드프레임"), ("해성디에스",)),
    (("철및비합금강H형강",), ("동국제강", "현대제철")),
    (("철및비합금강용접강관대", "철강제대구경강관"), ("EEW KHPC", "EEW코리아", "KBI동양철관", "휴스틸")),
    (("체외진단용시약", "전체진단시약"), ("프로티아",)),
    (("축전지부품", "2차전지팩모듈및CapAssy", "축전지부분품"), ("상신이디피", "신흥에스이씨", "신성에스티")),
    (("측정공구",), ("코메론",)),
    (("치과용기기및재료", "스파이럴임플란트", "치과용임플란트", "치과용충전재"), ("엘앤케이바이오", "덴티움", "디오", "메가젠임플란트", "오스템임플란트", "메타바이오메드")),
    (("커피원두",), ("SPC그룹",)),
    (("탄소섬유사",), ("효성첨단소재",)),
    (("태양광모듈", "태양광모듈패널"), ("현대에너지솔루션",)),
    (("고압케이블", "저압케이블", "전력선"), ("일진전기",)),   # 초고압 전력케이블(HS 8544.60)
    (("폴리아미드필름", "PIFilmPISheet등", "연성동박적층판"), ("PI첨단소재", "넥스플렉스")),
    (("프로필렌중합체필름", "콘덴서용필름"), ("삼영",)),
    (("생물현미경",), ("토모큐브",)),
    (("화학원료", "라이신염", "항생물질원료"), ("CJ제일제당", "대상", "에스티팜", "하이텍팜")),
]
...
_COMPANY_CANON = {
    ...
    # BeOn 알림이 'LS ELECTRIC' 을 split 해 남긴 고아 토큰 'ELECTRIC' → LS ELECTRIC
    # (운영자 미매칭 회사명수정, 2026-06-20). _MAP 변압기/차단기/배전반에 LS일렉트릭이
    # 이미 surfaced 라, 이 한 줄로 전 전력기기 미매칭(변압기·차단기·배전반·계전기·변환기
    # 등 ~106빈도)이 드롭. 'ELECTRIC' 단독은 이 데이터셋에선 LS ELECTRIC 만.
    "electric": "LS ELECTRIC",
    "포스코홀딩스": "POSCO홀딩스",
    "sk하이닉스": "SK하이닉스", "하이닉스": "SK하이닉스",
    "머큐리케이블": "머큐리",   # 알림 표기 '머큐리 케이블' → KRX 상장 머큐리(동일사)
...
_COMPANY_TYPO = {
    ...
    "삼성디플레이": "삼성디스플레이",     # BeOn 오타(디플레이) → 삼성디스플레이(OLED 큐레이션 surfaced)
}
...
_THEME_ROWS: list[tuple[str, str, str | tuple[str, ...], tuple[str, ...]]] = [
    ...
    ("리튬 1차전지", "2차전지/에너지/화학", "8506.50-0000",
     ("비츠로셀", "LS머트리얼즈")),
    # CNT 도전재: 파우더는 2803.00(탄소→228900), 배터리 납품 슬러리(NMP/물 분산)는
    # 화학조제품 3824.99 로 신고될 수 있어(운영자 2026-06-19) 둘 다 묶음. 단 3824.99 는
    # 농약원제·전해액까지 분산되므로 _THEME_MTI_PIN 으로 228900+290090(기타화학)만 고정.
    ("CNT 도전재 (탄소나노튜브)", "2차전지/에너지/화학", ("2803.00-0000", "3824.99-9000"),
     ("나노신소재", "제이오")),
    ...
    ("대구경 철강제 관 (라인파이프)", "기계/조선/철강", "7305.11-1000",
     ("EEW코리아", "KBI동양철관")),   # EEW KHPC=비상장 → price_provider._IGNORED_COMPANIES 영구 무시
    ...
    ("반도체 챔버 부품 (정전척·가스켓 가공)", "반도체/디스플레이", "8486.90-9000",
     ("한솔아이원스",)),
    # 삼성디스플레이는 대형 TV LCD 철수·QD-OLED 집중 → 8524.11(LCD,837110) 아닌
    # 8524.12(OLED,837120)로 타겟팅해야 대형 OLED 출하 펀더멘털 왜곡 없음(운영자 2026-06-19).
    ("평판디스플레이 모듈 (OLED·QD-OLED)", "반도체/디스플레이", "8524.12-1000",
     ("삼성디스플레이",)),
    # 기타 평판디스플레이(837130 = LCD·OLED 이외) = Micro-LED·e-Paper·무기발광 등 차세대.
    # HS 8524.19(구동기기無)/8524.99(有) 둘 다 837130. 절대액은 미미하나 OLED 노이즈
    # 제거된 선행기술 개화 선행지표(운영자 2026-06-19 '서울바이오시스같이 Micro-LED 발굴').
    ("기타 평판디스플레이 (Micro-LED·e-Paper)", "반도체/디스플레이",
     ("8524.19-1000", "8524.99-1000"),
     ("서울바이오시스", "서울반도체", "루멘스", "사파이어테크놀로지",
      "삼성디스플레이", "LG디스플레이")),
